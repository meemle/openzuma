...
def test_openrouter_base_url_applies_or_headers(mock_openai):
    ...
    assert headers["HTTP-Referer"] == "https://openzuma-agent.nousresearch.com"
    assert headers["X-OpenRouter-Title"] == "Openzuma Agent"
...
def test_ai_gateway_base_url_applies_attribution_headers(mock_openai):
    ...
    headers = agent._client_kwargs["default_headers"]
    assert headers["HTTP-Referer"] == "https://openzuma-agent.nousresearch.com"
    assert headers["X-Title"] == "Openzuma Agent"
    assert headers["User-Agent"].startswith("OpenzumaAgent/")
