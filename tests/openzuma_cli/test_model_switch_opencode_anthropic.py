...
``openzuma_cli.runtime_provider.resolve_runtime_provider`` already strips
``/v1`` at fresh agent init (PR #4918), but the ``/model`` mid-session
switch path in ``openzuma_cli.model_switch.switch_model`` was missing the
...
def _run_opencode_switch(
    ...
    with (
        patch("openzuma_cli.model_switch.resolve_alias", return_value=None),
        patch("openzuma_cli.model_switch.list_provider_models", return_value=[]),
        patch(
        ...
        patch(
            "openzuma_cli.models.validate_requested_model",
            return_value=_MOCK_VALIDATION,
        ),
        patch("openzuma_cli.model_switch.get_model_info", return_value=None),
        patch("openzuma_cli.model_switch.get_model_capabilities", return_value=None),
        patch("openzuma_cli.models.detect_provider_for_model", return_value=None),
    ):
        return switch_model(
            raw_input=raw_input,
