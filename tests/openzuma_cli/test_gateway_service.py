...
import openzuma_cli.gateway as gateway_cli
...
class TestSystemdServiceRefresh:
    def test_systemd_install_repairs_outdated_unit_without_force(self, tmp_path, monkeypatch):
        unit_path = tmp_path / "openzuma-gateway.service"
...
class TestLaunchdServiceRecovery:
    def test_get_restart_drain_timeout_prefers_env_then_config_then_default(self, monkeypatch):
        monkeypatch.delenv("OPENZUMA_RESTART_DRAIN_TIMEOUT", raising=False)
        ...
        monkeypatch.setenv("OPENZUMA_RESTART_DRAIN_TIMEOUT", "9")
        assert gateway_cli._get_restart_drain_timeout() == 9.0

        monkeypatch.setenv("OPENZUMA_RESTART_DRAIN_TIMEOUT", "invalid")
        assert (
            gateway_cli._get_restart_drain_timeout()
            == DEFAULT_GATEWAY_RESTART_DRAIN_TIMEOUT
        )
    ...
    def test_launchd_install_repairs_outdated_plist_without_force(self, tmp_path, monkeypatch):
        plist_path = tmp_path / "ai.openzuma.gateway.plist"
...
class TestGatewaySystemServiceRouting:
    ...
    def test_systemd_status_surfaces_planned_restart_failure(self, monkeypatch, capsys):
        ...
        monkeypatch.setattr(gateway_cli, "has_legacy_openzuma_units", lambda: False)
    ...
    def test_gateway_install_reports_termux_manual_mode(self, monkeypatch, capsys):
        ...
        assert "Run manually: openzuma gateway" in out
    ...
    def test_gateway_status_on_termux_shows_manual_guidance(self, monkeypatch, capsys):
        ...
        assert "nohup openzuma gateway" in out
    ...
    def test_gateway_restart_does_not_fallback_to_foreground_when_launchd_restart_fails(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda: (_ for _ in ()).throw(
                gateway_cli.subprocess.CalledProcessError(5, ["launchctl", "kickstart", "-k", "gui/501/ai.openzuma.gateway"])
            ),
        )
...
class TestSystemUnitOpenzumaHome:
    """OPENZUMA_HOME in system units must reference the target user, not root."""
    ...
    def test_system_unit_uses_target_user_home_not_calling_user(self, monkeypatch):
        ...
        assert 'OPENZUMA_HOME=/home/alice/.openzuma' in unit
        assert '/root/.openzuma' not in unit

    def test_system_unit_remaps_profile_to_target_user(self, monkeypatch):
        # Simulate sudo with a profile: OPENZUMA_HOME was resolved under root
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("OPENZUMA_HOME", "/root/.openzuma/profiles/coder")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'OPENZUMA_HOME=/home/alice/.openzuma/profiles/coder' in unit
        assert '/root/' not in unit

    def test_system_unit_preserves_custom_openzuma_home(self, monkeypatch):
        # Custom OPENZUMA_HOME not under any user's home — keep as-is
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("OPENZUMA_HOME", "/opt/openzuma-shared")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'OPENZUMA_HOME=/opt/openzuma-shared' in unit

    def test_user_unit_unaffected_by_change(self):
        # User-scope units should still use the calling user's OPENZUMA_HOME
        unit = gateway_cli.generate_systemd_unit(system=False)

        openzuma_home = str(gateway_cli.get_openzuma_home().resolve())
        assert f'OPENZUMA_HOME={openzuma_home}' in unit


class TestOpenzumaHomeForTargetUser:
    """Unit tests for _openzuma_home_for_target_user()."""

    def test_remaps_default_home(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.delenv("OPENZUMA_HOME", raising=False)

        result = gateway_cli._openzuma_home_for_target_user("/home/alice")
        assert result == "/home/alice/.openzuma"
    ...
    def test_remaps_profile_path(self, monkeypatch):
        ...
        result = gateway_cli._openzuma_home_for_target_user("/home/alice")
        assert result == "/home/alice/.openzuma/profiles/coder"

    def test_keeps_custom_path(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("OPENZUMA_HOME", "/opt/openzuma")

        result = gateway_cli._openzuma_home_for_target_user("/home/alice")
        assert result == "/opt/openzuma"
...
class TestProfileArg:
    ...
    def test_default_openzuma_home_returns_empty(self, tmp_path, monkeypatch):
        """Default ~/.openzuma should not produce a --profile flag."""
        openzuma_home = tmp_path / ".openzuma"
        openzuma_home.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("OPENZUMA_HOME", str(openzuma_home))
        result = gateway_cli._profile_arg(str(openzuma_home))
        assert result == ""

    def test_named_profile_returns_flag(self, tmp_path, monkeypatch):
        """~/.openzuma/profiles/mybot should return '--profile mybot'."""
        profile_dir = tmp_path / ".openzuma" / "profiles" / "mybot"
        profile_dir.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("OPENZUMA_HOME", str(tmp_path / ".openzuma"))
    ...
    def test_hash_path_returns_empty(self, tmp_path, monkeypatch):
        """Arbitrary non-profile OPENZUMA_HOME should return empty string."""
        custom_home = tmp_path / "custom" / "openzuma"
    ...
    def test_nested_profile_path_returns_empty(self, tmp_path, monkeypatch):
        """~/.openzuma/profiles/mybot/subdir should NOT match — too deep."""
        nested = tmp_path / ".openzuma" / "profiles" / "mybot" / "subdir"
    ...
    def test_invalid_profile_name_returns_empty(self, tmp_path, monkeypatch):
        ...
        bad_profile = tmp_path / ".openzuma" / "profiles" / "My Bot!"
    ...
    def test_systemd_unit_includes_profile(self, tmp_path, monkeypatch):
        ...
        profile_dir.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("OPENZUMA_HOME", str(profile_dir))
        monkeypatch.setattr(gateway_cli, "get_openzuma_home", lambda: profile_dir)
        unit = gateway_cli.generate_systemd_unit(system=False)
    ...
    def test_launchd_plist_path_uses_real_user_home_not_profile_home(self, tmp_path, monkeypatch):
        profile_dir = tmp_path / ".openzuma" / "profiles" / "orcha"
        profile_dir.mkdir(parents=True)
        ...
        assert plist_path == machine_home / "Library" / "LaunchAgents" / "ai.openzuma.gateway-orcha.plist"
...
class TestRemapPathForUser:
    ...
    def test_remaps_path_under_current_home(self, monkeypatch, tmp_path):
        ...
        result = gateway_cli._remap_path_for_user(
            str(tmp_path / "root" / ".openzuma" / "openzuma-agent"),
            str(tmp_path / "alice"),
        )
        assert result == str(tmp_path / "alice" / ".openzuma" / "openzuma-agent")

    def test_keeps_system_path_unchanged(self, monkeypatch, tmp_path):
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "root")
        (tmp_path / "root").mkdir()
        result = gateway_cli._remap_path_for_user("/opt/openzuma", str(tmp_path / "alice"))
        assert result == "/opt/openzuma"

    def test_noop_when_same_user(self, monkeypatch, tmp_path):
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "alice")
        (tmp_path / "alice").mkdir()
        original = str(tmp_path / "alice" / ".openzuma" / "openzuma-agent")
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        ...
        root_home.mkdir()
        project = root_home / ".openzuma" / "openzuma-agent"
        ...
        monkeypatch.setenv("OPENZUMA_HOME", str(root_home / ".openzuma"))
        monkeypatch.setattr(gateway_cli, "get_openzuma_home", lambda: root_home / ".openzuma")
        ...
        assert "WorkingDirectory=/home/alice/.openzuma/openzuma-agent" in unit
...
class TestDockerAwareGateway:
    ...
    def test_run_systemctl_raises_runtimeerror_when_missing(self, monkeypatch):
        ...
        with pytest.raises(RuntimeError, match="systemctl is not available"):
            gateway_cli._run_systemctl(["start", "openzuma-gateway"])
    ...
    def test_run_systemctl_passes_through_on_success(self, monkeypatch):
        ...
        result = gateway_cli._run_systemctl(["status", "openzuma-gateway"])
        assert result.returncode == 0
        assert len(calls) == 1
        assert "status" in calls[0]

    def test_install_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'openzuma gateway install' inside Docker exits 0 with container guidance."""
        import pytest
    ...
    def test_uninstall_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'openzuma gateway uninstall' inside Docker exits 0 with container guidance."""
        import pytest
    ...
    def test_start_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'openzuma gateway start' inside Docker exits 0 with container guidance."""
        import pytest
        ...
        out = capsys.readouterr().out
        assert "docker" in out.lower()
        assert "openzuma gateway run" in out


class TestLegacyOpenzumaUnitDetection:
    """Tests for _find_legacy_openzuma_units / has_legacy_openzuma_units.

    These guard against the scenario that tripped Luis in April 2026: an
    older install left a ``openzuma.service`` unit behind when the service was
    renamed to ``openzuma-gateway.service``. After PR #5646 (signal recovery
    via systemd), the two services began SIGTERM-flapping over the same
    Telegram bot token in a 30-second cycle.

    The detector must flag ``openzuma.service`` ONLY when it actually runs our
    gateway, and must NEVER flag profile units
    (``openzuma-gateway-<profile>.service``) or unrelated third-party services.
    """

    # Minimal ExecStart that looks like our gateway
    _OUR_UNIT_TEXT = (
        "[Unit]\nDescription=Openzuma Gateway\n[Service]\n"
        "ExecStart=/usr/bin/python -m openzuma_cli.main gateway run --replace\n"
    )
    ...
    def test_detects_legacy_openzuma_service_in_user_scope(self, tmp_path, monkeypatch):
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        legacy = user_dir / "openzuma.service"
        legacy.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")

        results = gateway_cli._find_legacy_openzuma_units()

        assert len(results) == 1
        name, path, is_system = results[0]
        assert name == "openzuma.service"
        assert path == legacy
        assert is_system is False
        assert gateway_cli.has_legacy_openzuma_units() is True

    def test_detects_legacy_openzuma_service_in_system_scope(self, tmp_path, monkeypatch):
        _, system_dir = self._setup_search_paths(tmp_path, monkeypatch)
        legacy = system_dir / "openzuma.service"
        legacy.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
    ...
    def test_ignores_profile_unit_openzuma_gateway_coder(self, tmp_path, monkeypatch):
        """CRITICAL: profile units must NOT be flagged as legacy.

        Teknium's concern — ``openzuma-gateway-coder.service`` is our standard
        ...
        for base in (user_dir, system_dir):
            (base / "openzuma-gateway-coder.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
            (base / "openzuma-gateway-orcha.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
            (base / "openzuma-gateway.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
        ...
        assert results == []
        assert gateway_cli.has_legacy_openzuma_units() is False

    def test_ignores_unrelated_openzuma_service(self, tmp_path, monkeypatch):
        """Third-party ``openzuma.service`` that isn't ours stays untouched.

        If a user has some other package named ``openzuma`` installed as a
        service, we must not flag it.
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        (user_dir / "openzuma.service").write_text(
            "[Unit]\nDescription=Some Other Openzuma\n[Service]\n"
            "ExecStart=/opt/other-openzuma/bin/daemon --foreground\n",
            encoding="utf-8",
        )
    ...
    def test_returns_empty_when_no_legacy_files_exist(self, tmp_path, monkeypatch):
        ...
        assert gateway_cli._find_legacy_openzuma_units() == []
        assert gateway_cli.has_legacy_openzuma_units() is False
    ...
    def test_detects_both_scopes_simultaneously(self, tmp_path, monkeypatch):
        ...
        user_dir, system_dir = self._setup_search_paths(tmp_path, monkeypatch)
        (user_dir / "openzuma.service").write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        (system_dir / "openzuma.service").write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
    ...
    def test_accepts_alternate_execstart_formats(self, tmp_path, monkeypatch):
        ...
        ExecStart variants we've seen in the wild:
          - python -m openzuma_cli.main gateway run
          - python path/to/openzuma_cli/main.py gateway run
          - openzuma gateway run   (direct binary)
          - python path/to/gateway/run.py
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        variants = [
            "ExecStart=/venv/bin/python -m openzuma_cli.main gateway run --replace",
            "ExecStart=/venv/bin/python /opt/openzuma/openzuma_cli/main.py gateway run",
            "ExecStart=/usr/local/bin/openzuma gateway run --replace",
            "ExecStart=/venv/bin/python /opt/openzuma/gateway/run.py",
        ]
        for i, execstart in enumerate(variants):
            name = f"openzuma.service" if i == 0 else f"openzuma.service"  # same name
            # Test each variant fresh
            (user_dir / "openzuma.service").write_text(
                f"[Unit]\nDescription=Old Openzuma\n[Service]\n{execstart}\n",
                encoding="utf-8",
            )
            results = gateway_cli._find_legacy_openzuma_units()
    ...
    def test_print_legacy_unit_warning_shows_migration_hint(self, tmp_path, monkeypatch, capsys):
        ...
        assert "openzuma.service" in out
        assert "openzuma gateway migrate-legacy" in out
    ...
    def test_handles_unreadable_unit_file_gracefully(self, tmp_path, monkeypatch):
        ...
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        unreadable = user_dir / "openzuma.service"
...
class TestRemoveLegacyOpenzumaUnits:
    """Tests for remove_legacy_openzuma_units (the migration action)."""
    ...
    def test_returns_zero_when_no_legacy_units(self, tmp_path, monkeypatch, capsys):
        ...
        removed, remaining = gateway_cli.remove_legacy_openzuma_units(interactive=False)
    ...
    def test_dry_run_lists_without_removing(self, tmp_path, monkeypatch, capsys):
        ...
        removed, remaining = gateway_cli.remove_legacy_openzuma_units(
    ...
    def test_removes_user_scope_legacy_unit(self, tmp_path, monkeypatch, capsys):
        ...
        assert any("--user stop openzuma.service" in c for c in cmds_joined)
        assert any("--user disable openzuma.service" in c for c in cmds_joined)
    ...
    def test_system_scope_without_root_defers_removal(self, tmp_path, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "sudo openzuma gateway migrate-legacy" in out
    ...
    def test_system_scope_with_root_removes(self, tmp_path, monkeypatch, capsys):
        ...
        assert any(
            c.startswith("systemctl stop openzuma.service") for c in cmds_joined
        )
        assert any(
            c.startswith("systemctl disable openzuma.service") for c in cmds_joined
        )
    ...
    def test_removes_both_scopes_with_root(self, tmp_path, monkeypatch, capsys):
        ...
        user_legacy = user_dir / "openzuma.service"
        system_legacy = system_dir / "openzuma.service"
    ...
    def test_does_not_touch_profile_units_during_migration(
        ...
        """Teknium's constraint: profile units (openzuma-gateway-coder.service)
        ...
        profile_unit = user_dir / "openzuma-gateway-coder.service"
        profile_unit.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        default_unit = user_dir / "openzuma-gateway.service"
    ...
    def test_interactive_prompt_no_skips_removal(self, tmp_path, monkeypatch, capsys):
        ...
        removed, remaining = gateway_cli.remove_legacy_openzuma_units(interactive=True)
...
class TestMigrateLegacyCommand:
    """Tests for the `openzuma gateway migrate-legacy` subcommand dispatch."""
    ...
    def test_migrate_legacy_subparser_accepts_dry_run_and_yes(self):
        ...
        import openzuma_cli.main as cli_main
        ...
        # public, just confirm that `openzuma gateway --help` shows it.
        import subprocess
        import sys

        project_root = cli_main.PROJECT_ROOT if hasattr(cli_main, "PROJECT_ROOT") else None
        if project_root is None:
            import openzuma_cli.gateway as gw
            project_root = gw.PROJECT_ROOT

        result = subprocess.run(
            [sys.executable, "-m", "openzuma_cli.main", "gateway", "--help"],
    ...
    def test_gateway_command_migrate_legacy_dispatches(
        ...
        monkeypatch.setattr(gateway_cli, "remove_legacy_openzuma_units", fake_remove)
        monkeypatch.setattr(gateway_cli, "supports_systemd_services", lambda: True)
        monkeypatch.setattr(gateway_cli, "is_macos", lambda: False)
...
class TestGatewayStatusParser:
    def test_gateway_status_subparser_accepts_full_flag(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "openzuma_cli.main", "gateway", "status", "-l", "--help"],
...
class TestSystemdInstallOffersLegacyRemoval:
    ...
    def test_install_offers_removal_when_legacy_detected(
        ...
        # has_legacy_openzuma_units must return True
        monkeypatch.setattr(gateway_cli, "has_legacy_openzuma_units", lambda: True)
        monkeypatch.setattr(gateway_cli, "remove_legacy_openzuma_units", fake_remove)
