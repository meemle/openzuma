...
def container_env(tmp_path, monkeypatch):
    """Set up a fake OPENZUMA_HOME with .container-mode file."""
    openzuma_home = tmp_path / ".openzuma"
    openzuma_home.mkdir()
    monkeypatch.setenv("OPENZUMA_HOME", str(openzuma_home))
    monkeypatch.delenv("OPENZUMA_DEV", raising=False)

    container_mode = openzuma_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=openzuma-agent\n"
        "exec_user=openzuma\n"
        "openzuma_bin=/data/current-package/bin/openzuma\n"
    )
    return openzuma_home


def test_get_container_exec_info_returns_metadata(container_env):
    """Reads .container-mode and returns all fields including exec_user."""
    with patch("openzuma_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "openzuma-agent"
    assert info["exec_user"] == "openzuma"
    assert info["openzuma_bin"] == "/data/current-package/bin/openzuma"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("openzuma_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_openzuma_dev(container_env, monkeypatch):
    """Returns None when OPENZUMA_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("OPENZUMA_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_openzuma_dev_zero(container_env, monkeypatch):
    """OPENZUMA_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("OPENZUMA_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        openzuma_home = Path(tmpdir) / ".openzuma"
        openzuma_home.mkdir()
        (openzuma_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("openzuma_constants.is_container", return_value=False), \
             patch("openzuma_cli.config.get_openzuma_home", return_value=openzuma_home), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("OPENZUMA_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=openzuma-custom\n"
        "exec_user=myuser\n"
        "openzuma_bin=/opt/openzuma/bin/openzuma\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "openzuma-custom"
    assert info["exec_user"] == "myuser"
    assert info["openzuma_bin"] == "/opt/openzuma/bin/openzuma"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from openzuma_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "openzuma"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "openzuma-agent" in cmd
    assert "/data/current-package/bin/openzuma" in cmd
    assert "chat" in cmd
