"""Tests for ``openzuma debug`` CLI command and debug utilities."""
...
@pytest.fixture
def openzuma_home(tmp_path, monkeypatch):
    """Set up an isolated OPENZUMA_HOME with minimal logs."""
...
class TestUploadPasteRs:
    ...
    def test_upload_paste_rs_success(self):
        from openzuma_cli.debug import _upload_paste_rs
        ...
        with patch("openzuma_cli.debug.urllib.request.urlopen", return_value=mock_resp):
    ...
    def test_upload_paste_rs_network_error(self):
        ...
        with patch(
            "openzuma_cli.debug.urllib.request.urlopen",
...
class TestUploadDpasteCom:
    ...
    def test_upload_dpaste_com_success(self):
        from openzuma_cli.debug import _upload_dpaste_com
...
class TestUploadToPastebin:
    ...
    def test_tries_paste_rs_first(self):
        from openzuma_cli.debug import upload_to_pastebin

        with patch("openzuma_cli.debug._upload_paste_rs",
    ...
    def test_falls_back_to_dpaste_com(self):
        ...
        with patch("openzuma_cli.debug._upload_paste_rs",
             ...
             patch("openzuma_cli.debug._upload_dpaste_com",
...
class TestCaptureLogSnapshot:
    ...
    def test_reads_small_file(self, openzuma_home):
        from openzuma_cli.debug import _capture_log_snapshot
    ...
    def test_empty_primary_reports_file_empty(self, openzuma_home):
        """Empty primary (no .1 fallback) surfaces as '(file empty)', not missing."""
        (openzuma_home / "logs" / "agent.log").write_text("")
    ...
    def test_race_truncate_after_resolve_reports_empty(self, openzuma_home, monkeypatch):
        """If the log is truncated between resolve and stat, say 'empty', not 'missing'."""
        log_path = openzuma_home / "logs" / "agent.log"
        from openzuma_cli import debug
    ...
    def test_truncates_large_file(self, openzuma_home):
        """Files larger than max_bytes get tail-truncated."""
        from openzuma_cli.debug import _capture_log_snapshot

        # Write a file larger than 1KB
        big_content = "x" * 100 + "\n"
        (openzuma_home / "logs" / "agent.log").write_text(big_content * 200)

        snap = _capture_log_snapshot("agent", tail_lines=10, max_bytes=1024)
        assert snap.full_text is not None
        assert "truncated" in snap.full_text

    def test_keeps_first_line_when_truncation_on_boundary(self, openzuma_home):
        ...
        (openzuma_home / "logs" / "agent.log").write_text(line * num_lines)
    ...
    def test_drops_partial_when_truncation_mid_line(self, openzuma_home):
    ...
    def test_unknown_log_returns_none(self, openzuma_home):
        from openzuma_cli.debug import _capture_log_snapshot
        snap = _capture_log_snapshot("nonexistent", tail_lines=10)
        assert snap.full_text is None

    def test_falls_back_to_rotated_file(self, openzuma_home):
        """When gateway.log doesn't exist, falls back to gateway.log.1."""
        from openzuma_cli.debug import _capture_log_snapshot

        logs_dir = openzuma_home / "logs"
    ...
    def test_prefers_primary_over_rotated(self, openzuma_home):
    ...
    def test_falls_back_when_primary_empty(self, openzuma_home):
...
class TestCollectDebugReport:
    ...
    def test_report_includes_dump_output(self, openzuma_home):
        from openzuma_cli.debug import collect_debug_report

        with patch("openzuma_cli.dump.run_dump") as mock_dump:
            mock_dump.side_effect = lambda args: print(
                "--- openzuma dump ---\nversion: 0.8.0\n--- end dump ---"
            )
            report = collect_debug_report(log_lines=50)

        assert "--- openzuma dump ---" in report
        assert "version: 0.8.0" in report

    def test_report_includes_agent_log(self, openzuma_home):
        from openzuma_cli.debug import collect_debug_report

        with patch("openzuma_cli.dump.run_dump"):
            report = collect_debug_report(log_lines=50)

        assert "--- agent.log" in report
        assert "session started" in report

    def test_report_includes_errors_log(self, openzuma_home):
        from openzuma_cli.debug import collect_debug_report
    ...
    def test_report_includes_gateway_log(self, openzuma_home):
        from openzuma_cli.debug import collect_debug_report
...
class TestRunDebugShare:
    ...
    def test_share_sweeps_expired_pastes(self, openzuma_home, capsys):
        ...
        with patch("openzuma_cli.dump.run_dump"), \
             patch("openzuma_cli.debug._sweep_expired_pastes", return_value=(0, 0)) as mock_sweep, \
             patch("openzuma_cli.debug.upload_to_pastebin",
    ...
    def test_share_survives_sweep_failure(self, openzuma_home, capsys):
        ...
        with patch("openzuma_cli.dump.run_dump"), \
             patch(
                 "openzuma_cli.debug._sweep_expired_pastes",
    ...
    def test_local_flag_prints_full_logs(self, openzuma_home, capsys):
    ...
    def test_share_uploads_three_pastes(self, openzuma_home, capsys):
        ...
        with patch("openzuma_cli.dump.run_dump") as mock_dump, \
            ...
            mock_dump.side_effect = lambda a: print("--- openzuma dump ---\nversion: test\n--- end dump ---")
            run_debug_share(args)
        ...
        assert "--- openzuma dump ---" in agent_paste
        assert "--- full agent.log ---" in agent_paste
        gateway_paste = uploaded_content[2]
        assert "--- openzuma dump ---" in gateway_paste
        assert "--- full gateway.log ---" in gateway_paste

    def test_share_keeps_report_and_full_log_on_same_snapshot(self, openzuma_home, capsys):
        """A mid-run rotation must not make full agent.log older than the report."""
        from openzuma_cli.debug import run_debug_share, collect_debug_report as real_collect_debug_report
        ...
        with patch("openzuma_cli.dump.run_dump"), \
             patch("openzuma_cli.debug.collect_debug_report", side_effect=_wrapped_collect_debug_report), \
             patch("openzuma_cli.debug.upload_to_pastebin", side_effect=_mock_upload):
            run_debug_share(args)
    ...
    def test_share_continues_on_log_upload_failure(self, openzuma_home, capsys):
    ...
    def test_share_exits_on_report_upload_failure(self, openzuma_home, capsys):
...
class TestRunDebug:
    def test_no_subcommand_shows_usage(self, capsys):
        ...
        out = capsys.readouterr().out
        assert "openzuma debug" in out
        assert "share" in out
        assert "delete" in out

    def test_share_subcommand_routes(self, openzuma_home):
        from openzuma_cli.debug import run_debug
...
class TestExtractPasteId:
    def test_paste_rs_url(self):
        from openzuma_cli.debug import _extract_paste_id
...
class TestDeletePaste:
    def test_delete_sends_delete_request(self):
        from openzuma_cli.debug import delete_paste
        ...
        with patch("openzuma_cli.debug.urllib.request.urlopen",
...
class TestScheduleAutoDelete:
    ...
    ``~/.openzuma/pastes/pending.json`` and lets ``_sweep_expired_pastes``
    handle the DELETE requests synchronously on the next ``openzuma debug``
    invocation.
    """

    def test_does_not_spawn_subprocess(self, openzuma_home):
        ...
        import ast
        import inspect
        from openzuma_cli.debug import _schedule_auto_delete
    ...
    def test_records_pending_to_json(self, openzuma_home):
        """Scheduled URLs are persisted to pending.json with expiration."""
        from openzuma_cli.debug import _schedule_auto_delete, _pending_file
        import json
    ...
    def test_skips_non_paste_rs_urls(self, openzuma_home):
    ...
    def test_merges_with_existing_pending(self, openzuma_home):
        """Subsequent calls merge into existing pending.json."""
        from openzuma_cli.debug import _schedule_auto_delete, _load_pending
    ...
    def test_dedupes_same_url(self, openzuma_home):
...
class TestSweepExpiredPastes:
    ...
    def test_sweep_empty_is_noop(self, openzuma_home):
        from openzuma_cli.debug import _sweep_expired_pastes

        deleted, remaining = _sweep_expired_pastes()
        assert deleted == 0
        assert remaining == 0

    def test_sweep_deletes_expired_entries(self, openzuma_home):
        ...
        with patch("openzuma_cli.debug.delete_paste", side_effect=fake_delete):
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_leaves_future_entries_alone(self, openzuma_home):
        from openzuma_cli.debug import _sweep_expired_pastes, _save_pending
        import time
        ...
        with patch("openzuma_cli.debug.delete_paste") as mock_delete:
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_survives_network_failure(self, openzuma_home):
        ...
        with patch(
            "openzuma_cli.debug.delete_paste",
    ...
    def test_sweep_drops_entries_past_grace_window(self, openzuma_home):
...
class TestRunDebugSweepsOnInvocation:
    ...
    def test_run_debug_calls_sweep(self, openzuma_home):
        from openzuma_cli.debug import run_debug

        args = MagicMock()
        args.debug_command = None  # default → prints help

        with patch("openzuma_cli.debug._sweep_expired_pastes") as mock_sweep:
            run_debug(args)

        mock_sweep.assert_called_once()

    def test_run_debug_survives_sweep_failure(self, openzuma_home, capsys):
        ...
        out = capsys.readouterr().out
        assert "Usage: openzuma debug" in out


class TestRunDebugDelete:
    def test_deletes_valid_url(self, capsys):
        from openzuma_cli.debug import run_debug_delete

        args = MagicMock()
        args.urls = ["https://paste.rs/abc"]

        with patch("openzuma_cli.debug.delete_paste", return_value=True):
    ...
    def test_handles_delete_failure(self, capsys):
        ...
        with patch("openzuma_cli.debug.delete_paste",
...
class TestShareIncludesAutoDelete:
    ...
    def test_share_schedules_auto_delete(self, openzuma_home, capsys):
        from openzuma_cli.debug import run_debug_share
        ...
        with patch("openzuma_cli.dump.run_dump"), \
             ...
             patch("openzuma_cli.debug._schedule_auto_delete") as mock_sched:
            run_debug_share(args)
    ...
    def test_share_shows_privacy_notice(self, openzuma_home, capsys):
        from openzuma_cli.debug import run_debug_share
        ...
        with patch("openzuma_cli.dump.run_dump"), \
             patch("openzuma_cli.debug.upload_to_pastebin",
                    return_value="https://paste.rs/test"), \
             patch("openzuma_cli.debug._schedule_auto_delete"):
            run_debug_share(args)

        out = capsys.readouterr().out
        assert "public paste service" in out

    def test_local_no_privacy_notice(self, openzuma_home, capsys):
        from openzuma_cli.debug import run_debug_share
