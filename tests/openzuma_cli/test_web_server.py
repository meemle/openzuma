"""Tests for openzuma_cli.web_server and related config utilities."""
...
class TestReloadEnv:
    ...
    def test_adds_new_vars(self, tmp_path):
        ...
        with patch("openzuma_cli.config.get_env_path", return_value=env_file):
    ...
    def test_removes_deleted_known_vars(self, tmp_path):
        """reload_env() removes known Openzuma vars not present in .env."""
        env_file = tmp_path / ".env"
    ...
    def test_does_not_remove_unknown_vars(self, tmp_path):
        """reload_env() preserves non-Openzuma env vars even when absent from .env."""
        env_file = tmp_path / ".env"
        env_file.write_text("")
        with patch("openzuma_cli.config.get_env_path", return_value=env_file):
...
class TestWebServerEndpoints:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_openzuma_home):
        """Create a TestClient and isolate the state DB under the test OPENZUMA_HOME."""
        try:
            from starlette.testclient import TestClient
        except ImportError:
            pytest.skip("fastapi/starlette not installed")

        import openzuma_state
        from openzuma_constants import get_openzuma_home
        from openzuma_cli.web_server import app, _SESSION_TOKEN

        monkeypatch.setattr(openzuma_state, "DEFAULT_DB_PATH", get_openzuma_home() / "state.db")
    ...
    def test_get_status(self):
        ...
        assert "openzuma_home" in data
        assert "active_sessions" in data

    def test_get_status_filters_unconfigured_gateway_platforms(self, monkeypatch):
        import gateway.config as gateway_config
        import openzuma_cli.web_server as web_server
    ...
    def test_reveal_env_var(self, tmp_path):
        ...
        from openzuma_cli.config import save_env_value
        from openzuma_cli.web_server import _SESSION_TOKEN
    ...
    def test_reveal_env_var_no_token(self, tmp_path):
        ...
        from starlette.testclient import TestClient
        from openzuma_cli.web_server import app
        from openzuma_cli.config import save_env_value
    ...
    def test_path_traversal_dotdot_blocked(self):
        ...
        resp = self.client.get("/%2e%2e/openzuma_cli/web_server.py")
        assert resp.status_code in (200, 404)
        if resp.status_code == 200:
...
class TestBuildSchemaFromConfig:
    def test_produces_expected_field_count(self):
        from openzuma_cli.web_server import CONFIG_SCHEMA
    ...
    def test_empty_prefix_produces_correct_keys(self):
        from openzuma_cli.web_server import _build_schema_from_config
...
class TestNewEndpoints:
    ...
    @pytest.fixture(autouse=True)
    def _setup(self, monkeypatch, _isolate_openzuma_home):
        try:
            from starlette.testclient import TestClient
        except ImportError:
            pytest.skip("fastapi/starlette not installed")
    ...
    def test_skills_list_includes_disabled_skills(self, monkeypatch):
        ...
        import openzuma_cli.skills_config as skills_config
        import openzuma_cli.web_server as web_server
    ...
    def test_toolsets_list_matches_cli_enabled_state(self, monkeypatch):
        import openzuma_cli.tools_config as tools_config
...
class TestModelContextLength:
    ...
    def test_normalize_extracts_context_length_from_dict(self):
        ...
        from openzuma_cli.web_server import _normalize_config_for_web
    ...
    def test_denormalize_writes_context_length_into_model_dict(self):
        ...
        from openzuma_cli.web_server import _denormalize_config_from_web
...
class TestModelInfoEndpoint:
    ...
    def test_model_info_with_dict_config(self, monkeypatch):
        import openzuma_cli.web_server as ws
...
class TestProbeGatewayHealth:
    ...
    def test_normalizes_url_with_health_suffix(self, monkeypatch):
        ...
        import openzuma_cli.web_server as ws
        monkeypatch.setattr(ws, "_GATEWAY_HEALTH_URL", "http://gw:8742/health")
        ...
        alive, body = ws._probe_gateway_health()
        assert alive is False
        assert "http://gw:8742/health/detailed" in calls
        assert "http://gw:8742/health" in calls
    ...
    def test_normalizes_url_with_health_detailed_suffix(self, monkeypatch):
        ...
        import openzuma_cli.web_server as ws
        monkeypatch.setattr(ws, "_GATEWAY_HEALTH_URL", "http://gw:8742/health/detailed")
        monkeypatch.setattr(ws, "_GATEWAY_HEALTH_TIMEOUT", 1)
        calls = []
    ...
    def test_successful_detailed_probe(self, monkeypatch):
        ...
        import openzuma_cli.web_server as ws
        monkeypatch.setattr(ws, "_GATEWAY_HEALTH_URL", "http://gw:8742")
        monkeypatch.setattr(ws, "_GATEWAY_HEALTH_TIMEOUT", 1)
...
class TestStatusRemoteGateway:
    ...
    def test_status_falls_back_to_remote_probe(self, monkeypatch):
        ...
        assert data["gateway_health_url"] == "http://gw:8742"
