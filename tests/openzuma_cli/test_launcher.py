"""Tests for the top-level `./openzuma` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./openzuma` should use `openzuma_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "openzuma"
    called = []

    fake_main_module = types.ModuleType("openzuma_cli.main")

    def fake_main():
        called.append("openzuma_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "openzuma_cli.main", fake_main_module)
    ...
    assert called == ["openzuma_cli.main"]
