"""Tests for the Command Installation check in openzuma doctor."""
...
import openzuma_cli.doctor as doctor_mod


def _setup_doctor_env(monkeypatch, tmp_path, venv_name="venv"):
    """Create a minimal OPENZUMA_HOME + PROJECT_ROOT for doctor tests."""
    ...
    openzuma_bin = venv_bin_dir / "openzuma"
    openzuma_bin.write_text("#!/usr/bin/env python\n# entry point\n")
    openzuma_bin.chmod(0o755)

    monkeypatch.setattr(doctor_mod, "OPENZUMA_HOME", home)
    ...
    # Stub auth checks
    try:
        from openzuma_cli import auth as _auth_mod
    ...
    return home, project, openzuma_bin
...
class TestDoctorCommandInstallation:
    ...
    def test_correct_symlink_shows_ok(self, monkeypatch, tmp_path):
        home, project, openzuma_bin = _setup_doctor_env(monkeypatch, tmp_path)

        # Create the command link dir with correct symlink
        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "openzuma"
        cmd_link.symlink_to(openzuma_bin)
    ...
    def test_missing_symlink_shows_fail(self, monkeypatch, tmp_path):
        ...
        assert "openzuma doctor --fix" in out
    ...
    def test_fix_creates_missing_symlink(self, monkeypatch, tmp_path):
        ...
        cmd_link = tmp_path / ".local" / "bin" / "openzuma"
        assert cmd_link.is_symlink()
        assert cmd_link.resolve() == openzuma_bin.resolve()
    ...
    def test_wrong_target_symlink_shows_warn(self, monkeypatch, tmp_path):
        ...
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "openzuma"
        wrong_target = tmp_path / "wrong_openzuma"
    ...
    def test_dot_venv_dir_is_found(self, monkeypatch, tmp_path):
        ...
        openzuma_bin = project / ".venv" / "bin" / "openzuma"
        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "openzuma"
        cmd_link.symlink_to(openzuma_bin)
        ...
        out = _run_doctor(fix=False)
        assert "Venv entry point exists" in out
        assert ".venv/bin/openzuma" in out

    @pytest.mark.skipif(sys.platform == "win32", reason="Symlink check is Unix-only")
    def test_non_symlink_regular_file_shows_ok(self, monkeypatch, tmp_path):
        """If ~/.local/bin/openzuma is a regular file (not symlink), accept it."""
        home, project, openzuma_bin = _setup_doctor_env(monkeypatch, tmp_path)

        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "openzuma"
        cmd_link.write_text("#!/bin/sh\nexec python -m openzuma_cli.main \"$@\"\n")
