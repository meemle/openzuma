"""Tests for the `openzuma memory reset` CLI command.
...
- Profile-scoped reset (uses OPENZUMA_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake OPENZUMA_HOME with memory files."""
    openzuma_home = tmp_path / ".openzuma"
    memories = openzuma_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("OPENZUMA_HOME", str(openzuma_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nOpenzuma repo is at ~/.openzuma/openzuma-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `openzuma memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        openzuma_home = tmp_path / ".openzuma"
        (openzuma_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("OPENZUMA_HOME", str(openzuma_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's OPENZUMA_HOME."""
        ...
        monkeypatch.setenv("OPENZUMA_HOME", str(profile_home))
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        openzuma_home = tmp_path / ".openzuma"
        openzuma_home.mkdir(parents=True)
        # No memories dir
        monkeypatch.setenv("OPENZUMA_HOME", str(openzuma_home))

        # The memories dir won't exist; get_openzuma_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
