...
from openzuma_cli.auth import AuthError, get_provider_auth_state, resolve_nous_runtime_credentials
...
class TestResolveVerifyFallback:
    ...
    def test_missing_ca_bundle_in_auth_state_falls_back(self):
        from openzuma_cli.auth import _resolve_verify
    ...
    def test_missing_ssl_cert_file_env_falls_back(self, monkeypatch):
        ...
        monkeypatch.delenv("OPENZUMA_CA_BUNDLE", raising=False)
        result = _resolve_verify(auth_state={"tls": {}})
        assert result is True

    def test_missing_openzuma_ca_bundle_env_falls_back(self, monkeypatch):
        from openzuma_cli.auth import _resolve_verify

        monkeypatch.setenv("OPENZUMA_CA_BUNDLE", "/nonexistent/openzuma-ca.pem")
...
def _setup_nous_auth(
    ...
    *,
    ...
    (openzuma_home / "auth.json").write_text(json.dumps(auth_store, indent=2))
...
def test_get_nous_auth_status_checks_credential_pool(tmp_path, monkeypatch):
    ...
    """
    from openzuma_cli.auth import get_nous_auth_status
...
def test_get_nous_auth_status_auth_store_fallback(tmp_path, monkeypatch):
    ...
    openzuma_home = tmp_path / "openzuma"
    _setup_nous_auth(openzuma_home, access_token="at-123")
    monkeypatch.setenv("OPENZUMA_HOME", str(openzuma_home))
...
def test_refresh_token_persisted_when_mint_returns_insufficient_credits(tmp_path, monkeypatch):
    openzuma_home = tmp_path / "openzuma"
    _setup_nous_auth(openzuma_home, refresh_token="refresh-old")
    monkeypatch.setenv("OPENZUMA_HOME", str(openzuma_home))
    ...
    monkeypatch.setattr("openzuma_cli.auth._refresh_access_token", _fake_refresh_access_token)
    monkeypatch.setattr("openzuma_cli.auth._mint_agent_key", _fake_mint_agent_key)
...
class TestLoginNousSkipKeepsCurrent:
    """When a user runs `openzuma model` → Nous Portal → Skip (keep current) after
    ...
    def _setup_home_with_openrouter(self, tmp_path, monkeypatch):
        ...
        auth_path = openzuma_home / "auth.json"
        auth_path.write_text(json.dumps({
            "version": 1,
            "active_provider": "openrouter",
            "providers": {"openrouter": {"api_key": "sk-or-fake"}},
        }))
        return openzuma_home, config_path, auth_path
    ...
    def _patch_login_internals(self, monkeypatch, *, prompt_returns):
        ...
        import openzuma_cli.auth as auth_mod
        import openzuma_cli.models as models_mod
        import openzuma_cli.nous_subscription as ns
    ...
    def test_skip_keep_current_preserves_provider_and_model(self, tmp_path, monkeypatch):
        ...
        import argparse
        import yaml
        from openzuma_cli.auth import PROVIDER_REGISTRY, _login_nous

        openzuma_home, config_path, auth_path = self._setup_home_with_openrouter(
...
def test_persist_nous_credentials_writes_both_pool_and_providers(tmp_path, monkeypatch):
    ...
    Regression guard: before this helper existed, `openzuma auth add nous`
    ...
    """
    from openzuma_cli.auth import persist_nous_credentials, NOUS_DEVICE_CODE_SOURCE
...
def test_persist_nous_credentials_allows_recovery_from_401(tmp_path, monkeypatch):
    ...
    must succeed (not raise "Openzuma is not logged into Nous Portal").
    ...
    """
    from openzuma_cli.auth import persist_nous_credentials, resolve_nous_runtime_credentials
    ...
    # doesn't raise "Openzuma is not logged into Nous Portal".
    def _fake_refresh_access_token(*, client, portal_base_url, client_id, refresh_token):
        return {
...
def test_persist_nous_credentials_embeds_custom_label(tmp_path, monkeypatch):
    ...
    Previously `openzuma auth add nous --type oauth --label <name>` silently
