"""Tests for the Openzuma plugin system (openzuma_cli.plugins)."""
...
from openzuma_cli.plugins import (
    ENTRY_POINTS_GROUP,
    VALID_HOOKS,
    LoadedPlugin,
    PluginContext,
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    ``<openzuma_home>/config.yaml`` under ``plugins.enabled``. Plugins are
    ...
    *base* is expected to be ``<openzuma_home>/plugins/``; we derive
    ``<openzuma_home>`` from it by walking one level up.
    """
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <OPENZUMA_HOME>/config.yaml.
        # Config is always read from OPENZUMA_HOME (not from the project
        # dir for project plugins), so that's where we opt in.
        import os
        openzuma_home_str = os.environ.get("OPENZUMA_HOME")
        if openzuma_home_str:
            openzuma_home = Path(openzuma_home_str)
        else:
            openzuma_home = base.parent
        openzuma_home.mkdir(parents=True, exist_ok=True)
        cfg_path = openzuma_home / "config.yaml"
        cfg: dict = {}
        if cfg_path.exists():
            try:
...
class TestPluginDiscovery:
    ...
    def test_discover_user_plugins(self, tmp_path, monkeypatch):
        """Plugins in ~/.openzuma/plugins/ are discovered."""
        plugins_dir = tmp_path / "openzuma_test" / "plugins"
        _make_plugin_dir(plugins_dir, "hello_plugin")
        monkeypatch.setenv("OPENZUMA_HOME", str(tmp_path / "openzuma_test"))
    ...
    def test_discover_project_plugins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("OPENZUMA_ENABLE_PROJECT_PLUGINS", "true")
        plugins_dir = project_dir / ".openzuma" / "plugins"
...
class TestPluginLoading:
    ...
    def test_load_missing_init(self, tmp_path, monkeypatch):
        ...
        # missing-init error.
        openzuma_home = tmp_path / "openzuma_test"
        (openzuma_home / "config.yaml").write_text(
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under openzuma_plugins.<name>."""
        plugins_dir = tmp_path / "openzuma_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("OPENZUMA_HOME", str(tmp_path / "openzuma_test"))

        # Clean up any prior namespace module
        sys.modules.pop("openzuma_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "openzuma_plugins.ns_plugin" in sys.modules
...
class TestPluginHooks:
    ...
    def test_invalid_hook_name_warns(self, tmp_path, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="openzuma_cli.plugins"):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestPreToolCallBlocking:
    ...
    def test_block_message_returned_for_valid_directive(self, monkeypatch):
        monkeypatch.setattr(
            "openzuma_cli.plugins.invoke_hook",
...
class TestPluginToolVisibility:
    ...
    def test_plugin_tools_in_definitions(self, tmp_path, monkeypatch):
        ...
        import openzuma_cli.plugins as plugins_mod
...
class TestPluginCommands:
    ...
    def test_get_plugin_command_handler_found(self):
        ...
        with patch("openzuma_cli.plugins._plugin_manager", mgr):
    ...
    def test_get_plugin_context_engine_discovers_plugins_lazily(self, tmp_path, monkeypatch):
        ...
        openzuma_home = tmp_path / "openzuma_test"
        plugins_dir = openzuma_home / "plugins"
    ...
    def test_commands_in_list_plugins_output(self, tmp_path, monkeypatch):
        ...
        plugins_dir = tmp_path / "openzuma_test" / "plugins"
        # Set OPENZUMA_HOME BEFORE _make_plugin_dir so auto-enable targets
...
class TestPluginDispatchTool:
    ...
    def test_dispatch_tool_calls_registry(self):
        ...
        with patch("openzuma_cli.plugins.PluginContext.dispatch_tool.__module__", "openzuma_cli.plugins"):
