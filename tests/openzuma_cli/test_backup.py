"""Tests for openzuma backup and import commands."""
...
def _make_openzuma_tree(root: Path) -> None:
    """Create a realistic ~/.openzuma directory structure for testing."""
    (root / "config.yaml").write_text("model:\n  provider: openrouter\n")
    (root / ".env").write_text("OPENROUTER_API_KEY=sk-test-123\n")
    (root / "memory_store.db").write_bytes(b"fake-sqlite")
    (root / "openzuma_state.db").write_bytes(b"fake-state")
    ...
    # openzuma-agent repo (should be EXCLUDED)
    (root / "openzuma-agent").mkdir(exist_ok=True)
    (root / "openzuma-agent" / "run_agent.py").write_text("# big file\n")
    (root / "openzuma-agent" / ".git").mkdir()
    (root / "openzuma-agent" / ".git" / "HEAD").write_text("ref: refs/heads/main\n")
...
class TestShouldExclude:
    def test_excludes_openzuma_agent(self):
        from openzuma_cli.backup import _should_exclude
        assert _should_exclude(Path("openzuma-agent/run_agent.py"))
        assert _should_exclude(Path("openzuma-agent/.git/HEAD"))
...
class TestBackup:
    def test_creates_zip(self, tmp_path, monkeypatch):
        ...
        openzuma_home.mkdir()
        _make_openzuma_tree(openzuma_home)

        monkeypatch.setenv("OPENZUMA_HOME", str(openzuma_home))
        # get_default_openzuma_root needs this
    ...
    def test_excludes_openzuma_agent(self, tmp_path, monkeypatch):
        """Backup does NOT include openzuma-agent/ directory."""
        openzuma_home = tmp_path / ".openzuma"
        openzuma_home.mkdir()
        _make_openzuma_tree(openzuma_home)
        ...
        with zipfile.ZipFile(out_zip, "r") as zf:
            names = zf.namelist()
            agent_files = [n for n in names if "openzuma-agent" in n]
            assert agent_files == [], f"openzuma-agent files leaked into backup: {agent_files}"
    ...
    def test_default_output_path(self, tmp_path, monkeypatch):
        """When no output path given, zip goes to ~/openzuma-backup-*.zip."""
        openzuma_home = tmp_path / ".openzuma"
        openzuma_home.mkdir()
        (openzuma_home / "config.yaml").write_text("model: test\n")
        ...
        zips = list(tmp_path.glob("openzuma-backup-*.zip"))
        assert len(zips) == 1
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        """A zip containing state.db is accepted as a valid Openzuma backup."""
        from openzuma_cli.backup import _validate_backup_zip
    ...
    def test_old_wrong_db_name_fails(self, tmp_path):
        """A zip with only openzuma_state.db (old wrong name) is rejected."""
        from openzuma_cli.backup import _validate_backup_zip
        zip_path = tmp_path / "old.zip"
        self._make_zip(zip_path, ["openzuma_state.db", "memory_store.db"])
        with zipfile.ZipFile(zip_path, "r") as zf:
            ok, reason = _validate_backup_zip(zf)
        assert not ok
...
class TestImport:
    ...
    def test_restores_files(self, tmp_path, monkeypatch):
        """Import extracts files into openzuma home."""
        openzuma_home = tmp_path / ".openzuma"
        openzuma_home.mkdir()
        monkeypatch.setenv("OPENZUMA_HOME", str(openzuma_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (openzuma_home / "config.yaml").read_text() == "model:\n  provider: openrouter\n"
        assert (openzuma_home / ".env").read_text() == "OPENROUTER_API_KEY=sk-test\n"
        assert (openzuma_home / "skills" / "my-skill" / "SKILL.md").read_text() == "# My Skill\n"
        assert (openzuma_home / "profiles" / "coder" / "config.yaml").exists()

    def test_strips_openzuma_prefix(self, tmp_path, monkeypatch):
        """Import strips .openzuma/ prefix if all entries share it."""
        openzuma_home = tmp_path / ".openzuma"
        openzuma_home.mkdir()
        monkeypatch.setenv("OPENZUMA_HOME", str(openzuma_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        zip_path = tmp_path / "backup.zip"
        self._make_backup_zip(zip_path, {
            ".openzuma/config.yaml": "model: test\n",
            ".openzuma/skills/a/SKILL.md": "# A\n",
        })
        ...
        assert (openzuma_home / "config.yaml").read_text() == "model: test\n"
        assert (openzuma_home / "skills" / "a" / "SKILL.md").read_text() == "# A\n"
    ...
    def test_rejects_non_openzuma_zip(self, tmp_path, monkeypatch):
        """Import rejects a zip that doesn't look like a openzuma backup."""
        openzuma_home = tmp_path / ".openzuma"
        openzuma_home.mkdir()
        monkeypatch.setenv("OPENZUMA_HOME", str(openzuma_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_blocks_path_traversal(self, tmp_path, monkeypatch):
        ...
        assert (openzuma_home / "config.yaml").exists()
        # traversal file should NOT exist outside openzuma home
    ...
    def test_confirmation_prompt_abort(self, tmp_path, monkeypatch):
        ...
        (openzuma_home / "config.yaml").write_text("existing: true\n")
        monkeypatch.setenv("OPENZUMA_HOME", str(openzuma_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (openzuma_home / "config.yaml").read_text() == "existing: true\n"
    ...
    def test_force_skips_confirmation(self, tmp_path, monkeypatch):
        ...
        assert (openzuma_home / "config.yaml").read_text() == "model: restored\n"
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        # Source
        src_home = tmp_path / "source" / ".openzuma"
        src_home.mkdir(parents=True)
        _make_openzuma_tree(src_home)

        monkeypatch.setenv("OPENZUMA_HOME", str(src_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "source")

        # Backup
        out_zip = tmp_path / "roundtrip.zip"
        from openzuma_cli.backup import run_backup, run_import

        run_backup(Namespace(output=str(out_zip)))
        assert out_zip.exists()

        # Import into a different location
        dst_home = tmp_path / "dest" / ".openzuma"
        dst_home.mkdir(parents=True)
        monkeypatch.setenv("OPENZUMA_HOME", str(dst_home))
        ...
        # openzuma-agent should NOT be present
        assert not (dst_home / "openzuma-agent").exists()
...
class TestFormatSize:
    def test_bytes(self):
        from openzuma_cli.backup import _format_size
...
class TestValidation:
    ...
    def test_validate_rejects_random(self):
        """Zip without openzuma markers fails validation."""
        import io
        from openzuma_cli.backup import _validate_backup_zip
    ...
    def test_detect_prefix_openzuma(self):
        """Detects .openzuma/ prefix wrapping all entries."""
        import io
        from openzuma_cli.backup import _detect_prefix

        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w") as zf:
            zf.writestr(".openzuma/config.yaml", "test")
            zf.writestr(".openzuma/skills/a/SKILL.md", "skill")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
            assert _detect_prefix(zf) == ".openzuma/"
    ...
    def test_detect_prefix_only_dirs(self):
        ...
        with zipfile.ZipFile(buf, "w") as zf:
            ...
            zf.writestr(".openzuma/", "")
            zf.writestr(".openzuma/skills/", "")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
            assert _detect_prefix(zf) == ""
...
class TestBackupEdgeCases:
    def test_nonexistent_openzuma_home(self, tmp_path, monkeypatch):
        """Backup exits when openzuma home doesn't exist."""
        fake_home = tmp_path / "nonexistent" / ".openzuma"
        monkeypatch.setenv("OPENZUMA_HOME", str(fake_home))
    ...
    def test_output_is_directory(self, tmp_path, monkeypatch):
        ...
        zips = list(out_dir.glob("openzuma-backup-*.zip"))
        assert len(zips) == 1
    ...
    def test_empty_openzuma_home(self, tmp_path, monkeypatch):
        """Backup handles empty openzuma home (no files to back up)."""
        openzuma_home = tmp_path / ".openzuma"
        openzuma_home.mkdir()
        # Only excluded dirs, no actual files
        (openzuma_home / "__pycache__").mkdir()
        (openzuma_home / "__pycache__" / "foo.pyc").write_bytes(b"\x00")
    ...
    def test_permission_error_during_backup(self, tmp_path, monkeypatch):
        ...
        bad_file = openzuma_home / "secret.db"
    ...
    def test_pre1980_timestamp_skipped(self, tmp_path, monkeypatch):
        ...
        old_file = openzuma_home / "ancient.txt"
    ...
    def test_skips_output_zip_inside_openzuma(self, tmp_path, monkeypatch):
        """Backup skips its own output zip if it's inside openzuma root."""
        openzuma_home = tmp_path / ".openzuma"
        openzuma_home.mkdir()
        (openzuma_home / "config.yaml").write_text("model: test\n")
        ...
        # Output inside openzuma home
        out_zip = openzuma_home / "backup.zip"
        args = Namespace(output=str(out_zip))
...
class TestImportEdgeCases:
    ...
    def test_eof_during_confirmation(self, tmp_path, monkeypatch):
        ...
        openzuma_home = tmp_path / ".openzuma"
        openzuma_home.mkdir()
        (openzuma_home / "config.yaml").write_text("existing\n")
        monkeypatch.setenv("OPENZUMA_HOME", str(openzuma_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_keyboard_interrupt_during_confirmation(self, tmp_path, monkeypatch):
        ...
        openzuma_home = tmp_path / ".openzuma"
        openzuma_home.mkdir()
        (openzuma_home / ".env").write_text("KEY=val\n")
        monkeypatch.setenv("OPENZUMA_HOME", str(openzuma_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_permission_error_during_import(self, tmp_path, monkeypatch):
        ...
        locked_dir = openzuma_home / "locked"
        locked_dir.mkdir()
    ...
    def test_progress_with_many_files(self, tmp_path, monkeypatch):
        ...
        assert (openzuma_home / "config.yaml").exists()
        assert (openzuma_home / "sessions" / "s0599.json").exists()
...
class TestProfileRestoration:
    ...
    def test_import_creates_profile_wrappers(self, tmp_path, monkeypatch):
        ...
        assert (openzuma_home / "profiles" / "coder" / "config.yaml").exists()
        assert (openzuma_home / "profiles" / "researcher" / "config.yaml").exists()
        ...
        assert "openzuma -p coder" in coder_wrapper
    ...
    def test_import_without_profiles_module(self, tmp_path, monkeypatch):
        ...
        import openzuma_cli.backup as backup_mod
        original_import = __builtins__.__import__ if hasattr(__builtins__, '__import__') else __import__

        def fake_import(name, *a, **kw):
            if name == "openzuma_cli.profiles":
...
class TestSafeCopyDb:
    def test_copies_valid_database(self, tmp_path):
        from openzuma_cli.backup import _safe_copy_db
...
class TestQuickSnapshot:
    @pytest.fixture
    def openzuma_home(self, tmp_path):
        """Create a fake OPENZUMA_HOME with critical state files."""
        home = tmp_path / ".openzuma"
        home.mkdir()
    ...
    def test_creates_snapshot(self, openzuma_home):
        from openzuma_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(openzuma_home=openzuma_home)
        assert snap_id is not None
        snap_dir = openzuma_home / "state-snapshots" / snap_id
        assert snap_dir.is_dir()
        assert (snap_dir / "manifest.json").exists()

    def test_label_in_id(self, openzuma_home):
        from openzuma_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(label="before-upgrade", openzuma_home=openzuma_home)
        assert "before-upgrade" in snap_id

    def test_state_db_safely_copied(self, openzuma_home):
        from openzuma_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(openzuma_home=openzuma_home)
        db_copy = openzuma_home / "state-snapshots" / snap_id / "state.db"
    ...
    def test_copies_nested_files(self, openzuma_home):
        from openzuma_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(openzuma_home=openzuma_home)
        assert (openzuma_home / "state-snapshots" / snap_id / "cron" / "jobs.json").exists()

    def test_missing_files_skipped(self, openzuma_home):
        from openzuma_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(openzuma_home=openzuma_home)
        with open(openzuma_home / "state-snapshots" / snap_id / "manifest.json") as f:
            meta = json.load(f)
    ...
    def test_empty_home_returns_none(self, tmp_path):
        ...
        empty.mkdir()
        assert create_quick_snapshot(openzuma_home=empty) is None

    def test_list_snapshots(self, openzuma_home):
        from openzuma_cli.backup import create_quick_snapshot, list_quick_snapshots
        id1 = create_quick_snapshot(label="first", openzuma_home=openzuma_home)
        id2 = create_quick_snapshot(label="second", openzuma_home=openzuma_home)

        snaps = list_quick_snapshots(openzuma_home=openzuma_home)
        assert len(snaps) == 2
        assert snaps[0]["id"] == id2  # most recent first
        assert snaps[1]["id"] == id1

    def test_list_limit(self, openzuma_home):
        from openzuma_cli.backup import create_quick_snapshot, list_quick_snapshots
        for i in range(5):
            create_quick_snapshot(label=f"s{i}", openzuma_home=openzuma_home)
        snaps = list_quick_snapshots(limit=3, openzuma_home=openzuma_home)
        assert len(snaps) == 3

    def test_restore_config(self, openzuma_home):
        from openzuma_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(openzuma_home=openzuma_home)

        (openzuma_home / "config.yaml").write_text("model:\n  provider: anthropic\n")
        assert "anthropic" in (openzuma_home / "config.yaml").read_text()

        result = restore_quick_snapshot(snap_id, openzuma_home=openzuma_home)
        assert result is True
        assert "openrouter" in (openzuma_home / "config.yaml").read_text()

    def test_restore_state_db(self, openzuma_home):
        from openzuma_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(openzuma_home=openzuma_home)

        conn = sqlite3.connect(str(openzuma_home / "state.db"))
        conn.execute("INSERT INTO sessions VALUES ('s2', 'new')")
        conn.commit()
        conn.close()

        restore_quick_snapshot(snap_id, openzuma_home=openzuma_home)
    ...
    def test_restore_nonexistent(self, openzuma_home):
        from openzuma_cli.backup import restore_quick_snapshot
        assert restore_quick_snapshot("nonexistent", openzuma_home=openzuma_home) is False

    def test_auto_prune(self, openzuma_home):
        from openzuma_cli.backup import create_quick_snapshot, list_quick_snapshots, _QUICK_DEFAULT_KEEP
        for i in range(_QUICK_DEFAULT_KEEP + 5):
            create_quick_snapshot(label=f"snap-{i:03d}", openzuma_home=openzuma_home)
        snaps = list_quick_snapshots(limit=100, openzuma_home=openzuma_home)
        assert len(snaps) <= _QUICK_DEFAULT_KEEP

    def test_manual_prune(self, openzuma_home):
        from openzuma_cli.backup import create_quick_snapshot, prune_quick_snapshots, list_quick_snapshots
        for i in range(10):
            create_quick_snapshot(label=f"s{i}", openzuma_home=openzuma_home)
        deleted = prune_quick_snapshots(keep=3, openzuma_home=openzuma_home)
        assert deleted == 7
        assert len(list_quick_snapshots(openzuma_home=openzuma_home)) == 3
