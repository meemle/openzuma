...
def main_mod(monkeypatch):
    import openzuma_cli.main as mod
...
def test_print_tui_exit_summary_includes_resume_and_token_totals(monkeypatch, capsys):
    ...
    monkeypatch.setitem(sys.modules, "openzuma_state", types.SimpleNamespace(SessionDB=lambda: _FakeDB()))

    main_mod._print_tui_exit_summary("20260409_000001_abc123")
    out = capsys.readouterr().out

    assert "Resume this session with:" in out
    assert "openzuma --tui --resume 20260409_000001_abc123" in out
    assert 'openzuma --tui -c "demo title"' in out
