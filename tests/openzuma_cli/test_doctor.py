...
import openzuma_cli.doctor as doctor
import openzuma_cli.gateway as gateway_cli
from openzuma_cli import doctor as doctor_mod
from openzuma_cli.doctor import _has_provider_env_config
...
def test_run_doctor_sets_interactive_env_for_tool_checks(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(doctor_mod, "OPENZUMA_HOME", openzuma_home)
    monkeypatch.delenv("OPENZUMA_INTERACTIVE", raising=False)

    seen = {}

    def fake_check_tool_availability(*args, **kwargs):
        seen["interactive"] = os.getenv("OPENZUMA_INTERACTIVE")
        raise SystemExit(0)
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_openzuma_home(self, tmp_path, provider=""):
        """Create a minimal OPENZUMA_HOME with config.yaml."""
    ...
    def _run_doctor_and_capture(self, monkeypatch, tmp_path, provider=""):
        ...
        home = self._make_openzuma_home(tmp_path, provider)
