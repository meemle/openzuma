...
def _write_auth_store(tmp_path, payload: dict) -> None:
    ...
    (openzuma_home / "auth.json").write_text(json.dumps(payload, indent=2))
...
def test_auth_add_api_key_persists_manual_entry(tmp_path, monkeypatch):
    ...
    payload = json.loads((tmp_path / "openzuma" / "auth.json").read_text())
...
def test_auth_add_anthropic_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "agent.anthropic_adapter.run_openzuma_oauth_login_pure",
        lambda: {
            "access_token": token,
    ...
    entry = next(item for item in entries if item["source"] == "manual:openzuma_pkce")
    assert entry["label"] == "claude@example.com"
    assert entry["source"] == "manual:openzuma_pkce"
...
def test_auth_add_nous_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "openzuma_cli.auth._nous_device_code_login",
        lambda **kwargs: {
            "portal_base_url": "https://portal.example.com",
            "inference_base_url": "https://inference.example.com/v1",
            "client_id": "openzuma-cli",
    ...
    # `openzuma auth add nous` must also populate providers.nous so the
    ...
    # raises "Openzuma is not logged into Nous Portal" and the agent dies.
...
def test_auth_add_nous_oauth_honors_custom_label(tmp_path, monkeypatch):
    """`openzuma auth add nous --type oauth --label <name>` must preserve the
...
def test_auth_add_codex_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "openzuma_cli.auth._codex_device_code_login",
        lambda: {
            "tokens": {
                "access_token": token,
                "refresh_token": "refresh-token",
...
def test_auth_remove_reindexes_priorities(tmp_path, monkeypatch):
    ...
    from openzuma_cli.auth_commands import auth_remove_command
...
def test_auth_reset_clears_provider_statuses(tmp_path, monkeypatch, capsys):
    ...
    from openzuma_cli.auth_commands import auth_reset_command
...
def test_auth_list_does_not_call_mutating_select(monkeypatch, capsys):
    from openzuma_cli.auth_commands import auth_list_command
    ...
    monkeypatch.setattr(
        "openzuma_cli.auth_commands.load_pool",
...
def test_auth_list_shows_exhausted_cooldown(monkeypatch, capsys):
    ...
    monkeypatch.setattr("openzuma_cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr("openzuma_cli.auth_commands.time.time", lambda: 1030.0)
...
def test_auth_list_prefers_explicit_reset_time(monkeypatch, capsys):
    ...
    monkeypatch.setattr("openzuma_cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr(
        "openzuma_cli.auth_commands.time.time",
...
def test_auth_remove_claude_code_suppresses_reseed(tmp_path, monkeypatch):
    ...
    (openzuma_home / "auth.json").write_text(json.dumps(auth_store))

    from types import SimpleNamespace
    from openzuma_cli.auth_commands import auth_remove_command
    auth_remove_command(SimpleNamespace(provider="anthropic", target="1"))

    updated = json.loads((openzuma_home / "auth.json").read_text())
...
def test_unsuppress_credential_source_clears_marker(tmp_path, monkeypatch):
    ...
    from openzuma_cli.auth import suppress_credential_source, unsuppress_credential_source, is_source_suppressed
...
def test_unsuppress_credential_source_returns_false_when_absent(tmp_path, monkeypatch):
    ...
    from openzuma_cli.auth import unsuppress_credential_source
...
def test_auth_add_codex_clears_suppression_marker(tmp_path, monkeypatch):
    """Re-linking codex via `openzuma auth add openai-codex` must clear any suppression marker."""
    monkeypatch.setenv("OPENZUMA_HOME", str(tmp_path / "openzuma"))
    monkeypatch.setattr(
        "agent.credential_pool._seed_from_singletons",
        lambda provider, entries: (False, set()),
    ...
    # Pre-existing suppression (simulating a prior `openzuma auth remove`)
    (openzuma_home / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {},
    ...
    payload = json.loads((openzuma_home / "auth.json").read_text())
...
def test_seed_from_singletons_respects_codex_suppression(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("openzuma_cli.auth._import_codex_cli_tokens", _fake_import)
    ...
    after = json.loads((openzuma_home / "auth.json").read_text())
...
def test_auth_remove_env_seeded_suppresses_shell_exported_var(tmp_path, monkeypatch, capsys):
    """`openzuma auth remove xai 1` must stick even when the env var is exported
    by the shell (not written into ~/.openzuma/.env).  Before PR for #13371 the
...
def test_auth_remove_env_seeded_dotenv_only_no_shell_hint(tmp_path, monkeypatch, capsys):
    """When the env var lives only in ~/.openzuma/.env (not the shell), the
    ...
    (openzuma_home / ".env").write_text("DEEPSEEK_API_KEY=sk-ds-only\n")
    ...
    assert (openzuma_home / ".env").read_text().strip() == ""


def test_auth_add_clears_env_suppression_for_provider(tmp_path, monkeypatch):
    """Re-adding a credential via `openzuma auth add <provider>` clears any
    ...
    from types import SimpleNamespace
    from openzuma_cli.auth import is_source_suppressed
    from openzuma_cli.auth_commands import auth_add_command
...
def test_seed_from_env_respects_env_suppression(tmp_path, monkeypatch):
    ...
    via `openzuma auth remove`.  This is the gate that prevents shell-exported
...
# =============================================================================
# Unified credential-source stickiness — every source Openzuma reads from has a
...
def test_seed_from_singletons_respects_copilot_suppression(tmp_path, monkeypatch):
    ...
    import openzuma_cli.copilot_auth as ca
...
def test_seed_from_singletons_respects_qwen_suppression(tmp_path, monkeypatch):
    ...
    import openzuma_cli.auth as ha
...
def test_seed_from_singletons_respects_openzuma_pkce_suppression(tmp_path, monkeypatch):
    """anthropic openzuma_pkce must not re-seed from ~/.openzuma/.anthropic_oauth.json when suppressed."""
    openzuma_home = tmp_path / "openzuma"
    openzuma_home.mkdir(parents=True, exist_ok=True)
    monkeypatch.setenv("OPENZUMA_HOME", str(openzuma_home))

    import yaml
    (openzuma_home / "config.yaml").write_text(yaml.dump({"model": {"provider": "anthropic", "model": "claude"}}))
    (openzuma_home / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {},
        "suppressed_sources": {"anthropic": ["openzuma_pkce"]},
    }))

    # Stub the readers so only openzuma_pkce is "available"; claude_code returns None
    import agent.anthropic_adapter as aa
    monkeypatch.setattr(aa, "read_openzuma_oauth_credentials", lambda: {
    ...
    # openzuma_pkce suppressed, claude_code returns None → nothing should be seeded
    assert entries == []
    assert "openzuma_pkce" not in active
...
def test_seed_custom_pool_respects_config_suppression(tmp_path, monkeypatch):
    ...
    import yaml
    (openzuma_home / "config.yaml").write_text(yaml.dump({
        "model": {},
        "custom_providers": [
...
def test_auth_add_clears_all_suppressions_including_non_env(tmp_path, monkeypatch):
    """Re-adding a credential via `openzuma auth add <provider>` clears ALL
...
def test_auth_remove_codex_manual_device_code_suppresses_canonical(tmp_path, monkeypatch):
    """Removing a manual:device_code entry (from `openzuma auth add openai-codex`)
