...
Covers:
 - Normal path (tokens already in Openzuma auth store)
 - Claude Code fallback (tokens only in ~/.claude/.credentials.json)
 - Negative case (no credentials anywhere)

Note: auto-import from ~/.codex/auth.json was removed in #12360 — Openzuma
now owns its own openai-codex auth state, and users explicitly adopt
existing Codex CLI tokens via `openzuma auth openai-codex`. The old
...
@pytest.fixture()
def openzuma_auth_only_env(tmp_path, monkeypatch):
    """Tokens already in Openzuma auth store (no Codex CLI needed)."""
...
def test_normal_path_still_works(openzuma_auth_only_env):
    """openai-codex appears when tokens are already in Openzuma auth store."""
...
def claude_code_only_env(tmp_path, monkeypatch):
    ...
    ~/.claude/.credentials.json (Claude Code) — not in env vars or Openzuma
    auth store."""
    openzuma_home = tmp_path / ".openzuma"
    openzuma_home.mkdir()
    ...
    (openzuma_home / "auth.json").write_text(
