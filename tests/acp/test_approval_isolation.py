...
- 96vc: ACP didn't set OPENZUMA_EXEC_ASK, so `check_all_command_guards`
...
1. Setting OPENZUMA_EXEC_ASK inside _run_agent (wraps the agent call).
...
class TestAcpExecAskGate:
    """GHSA-96vc-wcxf-jjff: ACP's _run_agent must set OPENZUMA_INTERACTIVE so
    ...
    (OPENZUMA_EXEC_ASK takes the gateway-queue path which requires a
    ...
    def test_interactive_env_var_routes_to_callback(self, monkeypatch):
        """When OPENZUMA_INTERACTIVE is set and an approval callback is
        ...
        # Without OPENZUMA_INTERACTIVE: takes auto-approve path, callback NOT called
        result = check_all_command_guards(
            "rm -rf /tmp/test-exec-ask", "local", approval_callback=fake_cb,
        )
        assert result["approved"] is True
        assert called_with == [], (
            "without OPENZUMA_INTERACTIVE the non-interactive auto-approve "
            "path should fire without consulting the callback"
        )

        # With OPENZUMA_INTERACTIVE: callback IS called, approval flows through it
        ...
        assert called_with, (
            "with OPENZUMA_INTERACTIVE the approval path should consult the "
