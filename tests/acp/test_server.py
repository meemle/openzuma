"""Tests for acp_adapter.server — OpenzumaACPAgent ACP server."""
...
from acp_adapter.server import OpenzumaACPAgent, OPENZUMA_VERSION
...
def agent(mock_manager):
    """OpenzumaACPAgent backed by a mock session manager."""
    return OpenzumaACPAgent(session_manager=mock_manager)
...
class TestInitialize:
    ...
    async def test_initialize_returns_agent_info(self, agent):
        ...
        assert resp.agent_info.name == "openzuma-agent"
        assert resp.agent_info.version == OPENZUMA_VERSION
...
class TestSessionOps:
    ...
    async def test_new_session_returns_model_state(self):
        ...
        acp_agent = OpenzumaACPAgent(session_manager=manager)

        with patch(
            "openzuma_cli.models.curated_models_for_provider",
...
class TestSessionConfiguration:
    ...
    async def test_set_session_model_accepts_provider_prefixed_choice(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("openzuma_cli.config.load_config", lambda: {
...
class TestSlashCommands:
    ...
    def test_version(self, agent, mock_manager):
        ...
        assert OPENZUMA_VERSION in result
...
class TestRegisterSessionMcpServers:
    ...
    async def test_registers_stdio_servers(self, agent, mock_manager):
        ...
        state.agent.enabled_toolsets = ["openzuma-acp"]
