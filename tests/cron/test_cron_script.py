...
- Path resolution (absolute, relative to OPENZUMA_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp OPENZUMA_HOME."""
    openzuma_home = tmp_path / ".openzuma"
    openzuma_home.mkdir()
    (openzuma_home / "cron").mkdir()
    (openzuma_home / "cron" / "output").mkdir()
    (openzuma_home / "scripts").mkdir()
    monkeypatch.setenv("OPENZUMA_HOME", str(openzuma_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "OPENZUMA_DIR", openzuma_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", openzuma_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", openzuma_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", openzuma_home / "cron" / "output")
...
class TestCronjobToolScript:
    ...
    def test_create_with_script(self, cron_env, monkeypatch):
        monkeypatch.setenv("OPENZUMA_INTERACTIVE", "1")
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.openzuma/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("OPENZUMA_SESSION_PLATFORM") is None
        assert os.environ.get("OPENZUMA_SESSION_CHAT_ID") is None
        assert os.environ.get("OPENZUMA_SESSION_CHAT_NAME") is None
