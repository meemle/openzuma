"""
Tests for mcp_serve — Openzuma MCP server.
...
def _isolate_openzuma_home(tmp_path, monkeypatch):
    """Redirect OPENZUMA_HOME to a temp directory."""
    monkeypatch.setenv("OPENZUMA_HOME", str(tmp_path))
    try:
        import openzuma_constants
        monkeypatch.setattr(openzuma_constants, "get_openzuma_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking openzuma_state schema."""
