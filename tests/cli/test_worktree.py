...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("openzuma/openzuma-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned openzuma/* and pr-* branches."""

    def test_prunes_orphaned_openzuma_branch(self, git_repo):
        """openzuma/openzuma-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "openzuma/openzuma-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "openzuma/openzuma-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "openzuma/openzuma-deadbeef" in result.stdout
        ...
        assert "openzuma/openzuma-deadbeef" in orphaned
        ...
        assert "openzuma/openzuma-deadbeef" not in result.stdout
