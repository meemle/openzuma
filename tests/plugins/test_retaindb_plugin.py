...
def _isolate_env(tmp_path, monkeypatch):
    """Ensure OPENZUMA_HOME and RETAINDB vars are isolated."""
...
class TestRetainDBMemoryProvider:
    ...
    def _make_provider(self, tmp_path, monkeypatch, api_key="rdb-test-key"):
        ...
        (tmp_path / ".openzuma").mkdir(exist_ok=True)
    ...
    def test_initialize_creates_client_and_queue(self, tmp_path, monkeypatch):
        ...
        p.initialize("test-session", openzuma_home=str(tmp_path / ".openzuma"))
    ...
    def test_initialize_profile_project(self, tmp_path, monkeypatch):
        ...
        p.initialize("test-session", openzuma_home=profile_home)
        assert p._client.project == "openzuma-coder"
        p.shutdown()

    def test_initialize_seeds_soul_md(self, tmp_path, monkeypatch):
        p = self._make_provider(tmp_path, monkeypatch)
        soul_path = tmp_path / ".openzuma" / "SOUL.md"
...
class TestPrefetch:
    ...
    def _make_initialized_provider(self, tmp_path, monkeypatch):
        ...
        openzuma_home = tmp_path / ".openzuma"
        openzuma_home.mkdir(exist_ok=True)
        monkeypatch.setenv("OPENZUMA_HOME", str(openzuma_home))
        p = RetainDBMemoryProvider()
        p.initialize("test-session", openzuma_home=str(openzuma_home))
        return p
