...
class TestFirecrawlClientConfig:
    ...
    def test_nous_auth_token_respects_openzuma_home_override(self, tmp_path):
        """Auth lookup should read from OPENZUMA_HOME/auth.json, not ~/.openzuma/auth.json."""
        real_home = tmp_path / "real-home"
        (real_home / ".openzuma").mkdir(parents=True)
        ...
        with patch.dict(os.environ, {
            "HOME": str(real_home),
            "OPENZUMA_HOME": str(openzuma_home),
        }, clear=False):
            import tools.web_tools
...
class TestBackendSelection:
    ...
    ``openzuma tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
