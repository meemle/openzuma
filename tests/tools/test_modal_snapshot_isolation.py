...
def _restore_tool_modules():
    original_openzuma_home = os.environ.get("OPENZUMA_HOME")
    original_modules = {
        name: module
        for name, module in sys.modules.items()
        if name == "tools"
        or name.startswith("tools.")
        or name == "openzuma_cli"
        or name.startswith("openzuma_cli.")
        or name == "modal"
    ...
    try:
        ...
        if original_openzuma_home is None:
            os.environ.pop("OPENZUMA_HOME", None)
        else:
            os.environ["OPENZUMA_HOME"] = original_openzuma_home
        _reset_modules(("tools", "openzuma_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    openzuma_cli = types.ModuleType("openzuma_cli")
    openzuma_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["openzuma_cli"] = openzuma_cli
    openzuma_home = tmp_path / "openzuma-home"
    os.environ["OPENZUMA_HOME"] = str(openzuma_home)
    sys.modules["openzuma_cli.config"] = types.SimpleNamespace(
        get_openzuma_home=lambda: openzuma_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="openzuma-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": openzuma_home / "modal_snapshots.json",
