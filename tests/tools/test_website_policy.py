...
def test_check_website_access_uses_dynamic_openzuma_home(monkeypatch, tmp_path):
    ...
    # Invalidate the module-level cache so the new OPENZUMA_HOME is picked up.
    # A prior test may have cached a default policy (enabled=False) under the
    # old OPENZUMA_HOME set by the autouse _isolate_openzuma_home fixture.
...
def test_check_website_access_fails_open_on_malformed_config(tmp_path, monkeypatch):
    ...
    # Simulate default path by pointing OPENZUMA_HOME to tmp_path
