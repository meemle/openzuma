...
def _clean_state():
    ...
    saved = {}
    for k in ("OPENZUMA_INTERACTIVE", "OPENZUMA_GATEWAY_SESSION", "OPENZUMA_EXEC_ASK", "OPENZUMA_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["OPENZUMA_INTERACTIVE"] = "1"
        session_key = os.getenv("OPENZUMA_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No OPENZUMA_INTERACTIVE or OPENZUMA_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
