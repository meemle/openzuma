...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        openzuma_home.mkdir()
        (openzuma_home / "token.json").write_text("{}")

        with patch.dict(os.environ, {"OPENZUMA_HOME": str(openzuma_home)}):
            missing = register_credential_files([{"path": "token.json"}])

        assert missing == []
        mounts = get_credential_file_mounts()
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(openzuma_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.openzuma/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        openzuma_home = tmp_path / ".openzuma"
        openzuma_home.mkdir()
        (openzuma_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        openzuma_home = tmp_path / ".openzuma"
        openzuma_home.mkdir()
        (openzuma_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        openzuma_home = tmp_path / ".openzuma"
        openzuma_home.mkdir()
        (openzuma_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        openzuma_home = tmp_path / ".openzuma"
        skills_dir = openzuma_home / "skills"
        ...
        assert mounts[0]["container_path"] == "/root/.openzuma/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        openzuma_home = tmp_path / ".openzuma"
        (openzuma_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"OPENZUMA_HOME": str(openzuma_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.openzuma")

        assert mounts[0]["container_path"] == "/home/user/.openzuma/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.openzuma/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.openzuma/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape OPENZUMA_HOME."""
        monkeypatch.setenv("OPENZUMA_HOME", str(tmp_path / ".openzuma"))
        (tmp_path / ".openzuma").mkdir()

        # Create a sensitive file one level above openzuma_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside openzuma_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside OPENZUMA_HOME must still be registered."""
        openzuma_home = tmp_path / ".openzuma"
        openzuma_home.mkdir()
        monkeypatch.setenv("OPENZUMA_HOME", str(openzuma_home))
        (openzuma_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_openzuma_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of OPENZUMA_HOME must be allowed."""
        openzuma_home = tmp_path / ".openzuma"
        openzuma_home.mkdir()
        subdir = openzuma_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside OPENZUMA_HOME pointing outside must be rejected."""
        openzuma_home = tmp_path / ".openzuma"
        openzuma_home.mkdir()
        monkeypatch.setenv("OPENZUMA_HOME", str(openzuma_home))

        # Create a sensitive file outside openzuma_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside openzuma_home pointing outside
        symlink = openzuma_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes OPENZUMA_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, openzuma_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape OPENZUMA_HOME."""
        openzuma_home = tmp_path / ".openzuma"
        openzuma_home.mkdir()
        monkeypatch.setenv("OPENZUMA_HOME", str(openzuma_home))
        ...
        sensitive.write_text("{}")
        self._write_config(openzuma_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(openzuma_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside OPENZUMA_HOME via config must still mount."""
        openzuma_home = tmp_path / ".openzuma"
        openzuma_home.mkdir()
        monkeypatch.setenv("OPENZUMA_HOME", str(openzuma_home))

        (openzuma_home / "oauth.json").write_text("{}")
        self._write_config(openzuma_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        openzuma_home = tmp_path / ".openzuma"
        openzuma_home.mkdir()
        (openzuma_home / "cache" / "documents").mkdir(parents=True)
        (openzuma_home / "cache" / "audio").mkdir(parents=True)
        monkeypatch.setenv("OPENZUMA_HOME", str(openzuma_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.openzuma/cache/documents" in paths
        assert "/root/.openzuma/cache/audio" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.openzuma/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        openzuma_home = tmp_path / ".openzuma"
        openzuma_home.mkdir()
        # Use legacy dir name — get_openzuma_dir prefers old if it exists
        (openzuma_home / "document_cache").mkdir()
        (openzuma_home / "image_cache").mkdir()
        monkeypatch.setenv("OPENZUMA_HOME", str(openzuma_home))

        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(openzuma_home / "document_cache") in host_paths
        assert str(openzuma_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.openzuma/cache/documents" in container_paths
        assert "/root/.openzuma/cache/images" in container_paths
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        openzuma_home = tmp_path / ".openzuma"
        doc_dir = openzuma_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        openzuma_home = tmp_path / ".openzuma"
        ss_dir = openzuma_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.openzuma/cache/screenshots/session_abc/screen1.png"
