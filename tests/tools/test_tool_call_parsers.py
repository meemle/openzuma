...
class TestParserRegistry:
    ...
    def test_openzuma_parser_registered(self):
        parsers = list_parsers()
        assert "openzuma" in parsers
...
# ─── Openzuma parser tests ────────────────────────────────────────────────

class TestOpenzumaParser:
    @pytest.fixture
    def parser(self):
        return get_parser("openzuma")
...
class TestParseResultContract:
    ...
    @pytest.fixture(params=["openzuma"])  # Add more as needed
    ...
    def test_tool_calls_are_proper_objects(self, parser):
        ...
        # Use openzuma format since that's universal
