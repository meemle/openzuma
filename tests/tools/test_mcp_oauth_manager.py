...
def test_openzuma_provider_subclass_exists():
    """OpenzumaMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _OPENZUMA_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _OPENZUMA_PROVIDER_CLS is not None
    assert issubclass(_OPENZUMA_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_openzuma_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns OpenzumaMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _OPENZUMA_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("OPENZUMA_HOME", str(tmp_path))
    ...
    assert _OPENZUMA_PROVIDER_CLS is not None
    assert isinstance(provider, _OPENZUMA_PROVIDER_CLS)
    assert provider._openzuma_server_name == "srv"
