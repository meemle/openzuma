...
def _make_socket_dir(tmpdir, session_name, pid=None, owner_pid=None):
    ...
    Args:
        ...
        owner_pid: owning openzuma PID to write to <session>.owner_pid
...
class TestReapOrphanedBrowserSessions:
    ...
    def test_non_openzuma_dirs_are_ignored(self, fake_tmpdir):
...
class TestOwnerPidCrossProcess:
    ...
    The owner_pid file records which openzuma process owns a daemon so that
    concurrent openzuma processes don't reap each other's active browser
