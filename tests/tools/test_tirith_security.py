...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._openzuma_bin_dir", return_value="/nonexistent"), \
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_openzuma_bin(self):
        """After _INSTALL_FAILED, manual install in OPENZUMA_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        openzuma_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(openzuma_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(openzuma_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._openzuma_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == openzuma_bin
            assert _tirith_mod._resolved_path == openzuma_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# OPENZUMA_HOME isolation
# ---------------------------------------------------------------------------

class TestOpenzumaHomeIsolation:
    def test_openzuma_bin_dir_respects_openzuma_home(self):
        """_openzuma_bin_dir must use OPENZUMA_HOME, not hardcoded ~/.openzuma."""
        from tools.tirith_security import _openzuma_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"OPENZUMA_HOME": tmpdir}):
            result = _openzuma_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_openzuma_home(self):
        """_failure_marker_path must use OPENZUMA_HOME, not hardcoded ~/.openzuma."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"OPENZUMA_HOME": "/custom/openzuma"}):
            result = _failure_marker_path()
        assert result == "/custom/openzuma/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets OPENZUMA_HOME; verify it's active."""
        openzuma_home = os.getenv("OPENZUMA_HOME")
        assert openzuma_home is not None, "OPENZUMA_HOME should be set by conftest"
        assert "openzuma_test" in openzuma_home, "Should point to test temp dir"

    def test_get_openzuma_home_fallback(self):
        """Without OPENZUMA_HOME set, falls back to ~/.openzuma."""
        from tools.tirith_security import _get_openzuma_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove OPENZUMA_HOME entirely
            os.environ.pop("OPENZUMA_HOME", None)
            result = _get_openzuma_home()
        assert result == os.path.join(os.path.expanduser("~"), ".openzuma")
