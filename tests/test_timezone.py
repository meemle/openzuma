"""
Tests for timezone support (openzuma_time module + integration points).
...
import openzuma_time


def _reset_openzuma_time_cache():
    """Reset the openzuma_time module cache (replacement for removed reset_cache)."""
    openzuma_time._cached_tz = None
    openzuma_time._cached_tz_name = None
    openzuma_time._cache_resolved = False


# =========================================================================
# openzuma_time.now() — core helper
# =========================================================================

class TestOpenzumaTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        _reset_openzuma_time_cache()

    def teardown_method(self):
        _reset_openzuma_time_cache()
        os.environ.pop("OPENZUMA_TIMEZONE", None)

    def test_valid_timezone_applies(self):
        """With a valid IANA timezone, now() returns time in that zone."""
        os.environ["OPENZUMA_TIMEZONE"] = "Asia/Kolkata"
        result = openzuma_time.now()
    ...
    def test_utc_timezone(self):
        """UTC timezone works."""
        os.environ["OPENZUMA_TIMEZONE"] = "UTC"
        result = openzuma_time.now()
        assert result.utcoffset() == timedelta(0)

    def test_us_eastern(self):
        """US/Eastern timezone works (DST-aware zone)."""
        os.environ["OPENZUMA_TIMEZONE"] = "America/New_York"
        result = openzuma_time.now()
        assert result.tzinfo is not None
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        os.environ["OPENZUMA_TIMEZONE"] = "Mars/Olympus_Mons"
        with caplog.at_level(logging.WARNING, logger="openzuma_time"):
            result = openzuma_time.now()
    ...
    def test_cache_invalidation(self):
        ...
        os.environ["OPENZUMA_TIMEZONE"] = "UTC"
        _reset_openzuma_time_cache()
        r1 = openzuma_time.now()
        assert r1.utcoffset() == timedelta(0)

        os.environ["OPENZUMA_TIMEZONE"] = "Asia/Kolkata"
        _reset_openzuma_time_cache()
        r2 = openzuma_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["OPENZUMA_TIMEZONE"] = "Europe/London"
        tz = openzuma_time.get_timezone()
    ...
    def test_returns_none_for_invalid(self):
        os.environ["OPENZUMA_TIMEZONE"] = "Not/A/Timezone"
        tz = openzuma_time.get_timezone()
        assert tz is None
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When OPENZUMA_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) OPENZUMA_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("OPENZUMA_TIMEZONE=" + os.environ.get("OPENZUMA_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "OPENZUMA_TIMEZONE=NOT_SET" in result["output"], (
            "OPENZUMA_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When OPENZUMA_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("OPENZUMA_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=openzuma_tz) which shifted
        absolute time when system-local tz != Openzuma tz.  The fix interprets
    ...
    def test_ensure_aware_normalizes_aware_to_openzuma_tz(self):
        """Already-aware datetimes should be normalized to Openzuma tz."""
        from cron.jobs import _ensure_aware
        ...
        # Must be in Openzuma tz (Kolkata) but same absolute instant
    ...
    def test_ensure_aware_due_job_not_skipped_when_system_ahead(self, tmp_path, monkeypatch):
        """Reproduce the actual bug: system tz ahead of Openzuma tz caused
        overdue jobs to appear as not-yet-due.

        Scenario: system is Asia/Kolkata (UTC+5:30), Openzuma is UTC.
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when Openzuma tz
        ...
        # Use a Openzuma timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _openzuma_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["OPENZUMA_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_openzuma_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of Openzuma timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["OPENZUMA_TIMEZONE"] = "US/Eastern"
        _reset_openzuma_time_cache()
