...
def test_config_set_statusbar_survives_non_dict_display(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(server, "_openzuma_home", tmp_path)
...
def test_enable_gateway_prompts_sets_gateway_env(monkeypatch):
    monkeypatch.delenv("OPENZUMA_EXEC_ASK", raising=False)
    monkeypatch.delenv("OPENZUMA_GATEWAY_SESSION", raising=False)
    monkeypatch.delenv("OPENZUMA_INTERACTIVE", raising=False)

    server._enable_gateway_prompts()

    assert server.os.environ["OPENZUMA_GATEWAY_SESSION"] == "1"
    assert server.os.environ["OPENZUMA_EXEC_ASK"] == "1"
    assert server.os.environ["OPENZUMA_INTERACTIVE"] == "1"


def test_setup_status_reports_provider_config(monkeypatch):
    monkeypatch.setattr("openzuma_cli.main._has_any_provider_configured", lambda: False)
...
def test_config_set_model_global_persists(monkeypatch):
    ...
    monkeypatch.setattr("openzuma_cli.model_switch.switch_model", _switch_model)
    monkeypatch.setattr(server, "_restart_slash_worker", lambda session: None)
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
    monkeypatch.setattr("openzuma_cli.config.save_config", lambda cfg: saved.update(cfg))
...
def test_config_set_model_syncs_inference_provider_env(monkeypatch):
    """After an explicit provider switch, OPENZUMA_INFERENCE_PROVIDER must
    ...
    monkeypatch.setattr(
        "openzuma_cli.model_switch.switch_model", lambda **_kwargs: result
    )
    monkeypatch.setattr(server, "_restart_slash_worker", lambda session: None)
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
    ...
    assert os.environ["OPENZUMA_INFERENCE_PROVIDER"] == "anthropic"
...
def test_plugins_list_surfaces_loader_error(monkeypatch):
    with patch("openzuma_cli.plugins.get_plugin_manager", side_effect=Exception("boom")):
        resp = server.handle_request(
...
def test_complete_slash_surfaces_completer_error(monkeypatch):
    with patch(
        "openzuma_cli.commands.SlashCommandCompleter",
...
def test_get_db_degrades_cleanly_when_sessiondb_init_fails(monkeypatch):
    fake_mod = types.ModuleType("openzuma_state")
    ...
    monkeypatch.setitem(sys.modules, "openzuma_state", fake_mod)
...
# --------------------------------------------------------------------------
# model.options — curated-list parity with `openzuma model` and classic /model
# --------------------------------------------------------------------------
...
def test_model_options_does_not_overwrite_curated_models(monkeypatch):
    ...
    list as `openzuma model` and the classic CLI /model picker.
    ...
    with patch(
        "openzuma_cli.model_switch.list_authenticated_providers",
        ...
        # guarding against.
        with patch("openzuma_cli.models.provider_model_ids") as live_fetch:
