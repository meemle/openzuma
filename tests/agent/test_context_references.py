...
def sample_repo(tmp_path: Path) -> Path:
    ...
    repo.mkdir()
    _git(repo, "init")
    _git(repo, "config", "user.name", "Openzuma Tests")
...
@pytest.mark.asyncio
async def test_blocks_sensitive_home_and_openzuma_paths(tmp_path: Path, monkeypatch):
    from agent.context_references import preprocess_context_references_async

    monkeypatch.setenv("HOME", str(tmp_path))
    monkeypatch.setenv("OPENZUMA_HOME", str(tmp_path / ".openzuma"))

    openzuma_env = tmp_path / ".openzuma" / ".env"
    openzuma_env.parent.mkdir(parents=True)
    openzuma_env.write_text("API_KEY=super-secret\n", encoding="utf-8")
    ...
    result = await preprocess_context_references_async(
        "read @file:.openzuma/.env and @file:.ssh/id_rsa",
        cwd=tmp_path,
