...
class TestReasoningCommand:
    ...
    async def test_reasoning_command_reloads_current_state_from_config(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(gateway_run, "_openzuma_home", openzuma_home)
    ...
    def test_run_agent_reloads_reasoning_config_per_message(self, tmp_path, monkeypatch):
        openzuma_home = tmp_path / "openzuma"
        openzuma_home.mkdir()
        (openzuma_home / "config.yaml").write_text("agent:\n  reasoning_effort: low\n", encoding="utf-8")

        monkeypatch.setattr(gateway_run, "_openzuma_home", openzuma_home)
        monkeypatch.setattr(gateway_run, "_env_path", openzuma_home / ".env")
    ...
    def test_run_agent_homeassistant_uses_default_platform_toolset(self, tmp_path, monkeypatch):
        openzuma_home = tmp_path / "openzuma"
        openzuma_home.mkdir()
        (openzuma_home / "config.yaml").write_text("", encoding="utf-8")
