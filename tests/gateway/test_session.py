...
class TestBuildSessionContextPrompt:
    ...
    def test_local_delivery_path_uses_display_openzuma_home(self):
        config = GatewayConfig()
        source = SessionSource(
            platform=Platform.LOCAL, chat_id="cli",
            chat_name="CLI terminal", chat_type="dm",
        ...
        with patch("openzuma_constants.display_openzuma_home", return_value="~/.openzuma/profiles/coder"):
            prompt = build_session_context_prompt(ctx)

        assert "~/.openzuma/profiles/coder/cron/output/" in prompt
