...
See: https://github.com/NousResearch/openzuma-agent/issues/4426
"""
...
class TestGetSubprocessHome:
    """Unit tests for openzuma_constants.get_subprocess_home()."""

    def test_returns_none_when_openzuma_home_unset(self, monkeypatch):
    ...
    def test_returns_path_when_home_dir_exists(self, tmp_path, monkeypatch):
        openzuma_home = tmp_path / ".openzuma"
        openzuma_home.mkdir()
        profile_home = openzuma_home / "home"
        profile_home.mkdir()
        monkeypatch.setenv("OPENZUMA_HOME", str(openzuma_home))
        from openzuma_constants import get_subprocess_home
    ...
    def test_two_profiles_get_different_homes(self, tmp_path, monkeypatch):
        base = tmp_path / ".openzuma" / "profiles"
        ...
        monkeypatch.setenv("OPENZUMA_HOME", str(base / "alpha"))
        home_a = get_subprocess_home()

        monkeypatch.setenv("OPENZUMA_HOME", str(base / "beta"))
...
class TestMakeRunEnvHomeInjection:
    ...
    def test_injects_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["HOME"] == str(openzuma_home / "home")
    ...
    def test_no_injection_when_openzuma_home_unset(self, monkeypatch):
        monkeypatch.delenv("OPENZUMA_HOME", raising=False)
...
class TestProfileBootstrap:
    ...
    def test_profile_dirs_includes_home(self):
        from openzuma_cli.profiles import _PROFILE_DIRS
    ...
    def test_create_profile_bootstraps_home_dir(self, tmp_path, monkeypatch):
        ...
        from openzuma_cli.profiles import create_profile
