...
class TestToolsetConsistency:
    ...
    def test_openzuma_platforms_share_core_tools(self):
        """All openzuma-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord_server`` on
        openzuma-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["openzuma-cli", "openzuma-telegram", "openzuma-discord", "openzuma-whatsapp", "openzuma-slack", "openzuma-signal", "openzuma-homeassistant"]
