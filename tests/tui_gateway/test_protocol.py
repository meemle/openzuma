...
def server():
    with patch.dict("sys.modules", {
        "openzuma_constants": MagicMock(get_openzuma_home=MagicMock(return_value="/tmp/openzuma_test")),
        "openzuma_cli.env_loader": MagicMock(),
        "openzuma_cli.banner": MagicMock(),
        "openzuma_state": MagicMock(),
    }):
        import importlib
...
def test_config_load_missing(server, tmp_path):
    server._openzuma_home = tmp_path
...
def test_slash_exec_rejects_skill_commands(server):
    ...
    fake_skills = {"/openzuma-agent-dev": {"name": "openzuma-agent-dev", "description": "Dev workflow"}}

    with patch("agent.skill_commands.get_skill_commands", return_value=fake_skills):
        resp = server.handle_request({
            "id": "r1",
            "method": "slash.exec",
            "params": {"command": "openzuma-agent-dev", "session_id": sid},
        })
...
def test_command_dispatch_returns_skill_payload(server):
    ...
    with patch("agent.skill_commands.scan_skill_commands", return_value=fake_skills), \
        ...
        resp = server.handle_request({
            "id": "r2",
            "method": "command.dispatch",
            "params": {"name": "openzuma-agent-dev", "session_id": sid},
        })
    ...
    assert result["name"] == "openzuma-agent-dev"
