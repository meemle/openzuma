...
Auto-tracks and cleans up ephemeral files created during Openzuma Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$OPENZUMA_HOME` and `/tmp/openzuma-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `OPENZUMA_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `OPENZUMA_HOME` or `/tmp/openzuma-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$OPENZUMA_HOME/disk-cleanup/` is itself excluded
- `$OPENZUMA_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
