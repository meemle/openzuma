"""disk_cleanup — ephemeral file cleanup for Openzuma Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under OPENZUMA_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly OPENZUMA_HOME and /tmp/openzuma-*
Never touches: ~/.openzuma/logs/ or any system directory.
"""
...
try:
    ...
    def get_openzuma_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("OPENZUMA_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".openzuma").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$OPENZUMA_HOME/logs/``."""
    return get_openzuma_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$OPENZUMA_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under OPENZUMA_HOME or ``/tmp/openzuma-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    openzuma_home = get_openzuma_home()
    try:
        path.resolve().relative_to(openzuma_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/openzuma-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("openzuma-"):
        return True
    return False
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside OPENZUMA_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under OPENZUMA_HOME (but leave OPENZUMA_HOME itself and
    ...
    _PROTECTED_TOP_LEVEL = {
        ...
        "openzuma-agent", "backups", "profiles", ".worktrees",
    }
    empty_removed = 0
    try:
        for dirpath in sorted(openzuma_home.rglob("*"), reverse=True):
            if not dirpath.is_dir() or dirpath == openzuma_home:
                continue
            try:
                rel_parts = dirpath.relative_to(openzuma_home).parts
            except ValueError:
                continue
...
def guess_category(path: Path) -> Optional[str]:
    ...
    openzuma_home = get_openzuma_home()
    try:
        rel = path.resolve().relative_to(openzuma_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "openzuma-agent",
        }:
            return None
        ...
        # Path isn't under OPENZUMA_HOME (e.g. /tmp/openzuma-*) — fall through.
        pass
