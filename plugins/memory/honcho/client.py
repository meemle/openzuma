...
Resolution order for config file:
  1. $OPENZUMA_HOME/honcho.json  (instance-local, enables isolated Openzuma instances)
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active Openzuma profile.

    Resolution order:
      1. OPENZUMA_HONCHO_HOST env var (explicit override)
      2. Active profile name via profiles system -> ``openzuma.<profile>``
      3. Fallback: ``"openzuma"`` (default profile)
    """
    explicit = os.environ.get("OPENZUMA_HONCHO_HOST", "").strip()
    if explicit:
        return explicit
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      1. $OPENZUMA_HOME/honcho.json      (profile-local, if it exists)
      2. ~/.openzuma/honcho.json          (default profile — shared host blocks live here)
      3. ~/.honcho/config.json          (global, cross-app interop)

    Returns the global path if none exist (for first-time setup writes).
    """
    local_path = get_openzuma_home() / "honcho.json"
    if local_path.exists():
        return local_path

    # Default profile's config — host blocks accumulate here via setup/clone
    default_path = Path.home() / ".openzuma" / "honcho.json"
...
class HonchoClientConfig:
    ...
    # Max chars of dialectic result to inject into Openzuma system prompt
    ...
    # True when Honcho was explicitly configured for this host (hosts.openzuma
    ...
    def from_global_config(
        ...
        Resolution: $OPENZUMA_HOME/honcho.json -> ~/.honcho/config.json -> env vars.
        When host is None, derives it from the active Openzuma profile.
        """
        resolved_host = host or resolve_active_host()
        ...
        # A hosts.openzuma block or explicit enabled flag means the user
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. Openzuma session title (from /title command)
          3. Gateway session key (stable per-chat identifier from gateway platforms)
          4. per-session strategy — Openzuma session_id ({timestamp}_{hex})
        ...
        # per-session: inherit Openzuma session_id (new Honcho session each run)
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError(
            ...
            "then run 'openzuma honcho setup' or set HONCHO_API_KEY. "
    ...
    if not resolved_base_url or resolved_timeout is None:
        try:
            ...
            openzuma_cfg = load_config()
            honcho_cfg = openzuma_cfg.get("honcho", {})
