...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/openzuma>
...
```bash
openzuma honcho setup    # full interactive wizard (cloud or local)
openzuma memory setup    # generic picker, also works
```

Or manually:
```bash
openzuma config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.openzuma/.env
```
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$OPENZUMA_HOME/honcho.json` | Profile-local (isolated Openzuma instances) |
| 2 | `~/.openzuma/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Openzuma profile: `openzuma` (default) or `openzuma.<profile>`.
...
| 4 | `per-session` strategy | Openzuma session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`openzuma-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`openzuma`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-openzuma-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening openzuma in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Openzuma session ID (timestamp + hex). Every `openzuma` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Openzuma profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`openzuma`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Openzuma profile: `openzuma` (default) or `openzuma.<profile>` (e.g. `openzuma -p coder` → host key `openzuma.coder`).
...
| `OPENZUMA_HONCHO_HOST` | Host key override |

## CLI Commands

| Command | Description |
|---------|-------------|
| `openzuma honcho setup` | Full interactive setup wizard |
| `openzuma honcho status` | Show resolved config for active profile |
| `openzuma honcho enable` / `disable` | Toggle Honcho for active profile |
| `openzuma honcho mode <mode>` | Change recall or observation mode |
| `openzuma honcho peer --user <name>` | Update user peer name |
| `openzuma honcho peer --ai <name>` | Update AI peer name |
| `openzuma honcho tokens --context <N>` | Set context token budget |
| `openzuma honcho tokens --dialectic <N>` | Set dialectic max chars |
| `openzuma honcho map <name>` | Map current directory to a session name |
| `openzuma honcho sync` | Create host blocks for all Openzuma profiles |
