...
Config file: `$OPENZUMA_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `openzuma` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `openzuma-{identity}` → `openzuma-coder`). |
...
Use `{identity}` in the `container_tag` to scope memories per Openzuma profile:

```json
{
  "container_tag": "openzuma-{identity}"
}
```

For a profile named `coder`, this resolves to `openzuma-coder`. The default profile resolves to `openzuma-default`. Without `{identity}`, all profiles share the same container.
