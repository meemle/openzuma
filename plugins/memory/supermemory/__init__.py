...
_DEFAULT_CONTAINER_TAG = "openzuma"
...
def _load_supermemory_config(openzuma_home: str) -> dict:
    config = _default_config()
    config_path = Path(openzuma_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, openzuma_home: str) -> None:
    config_path = Path(openzuma_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `openzuma memory setup`.
        # All other options are documented for $OPENZUMA_HOME/supermemory.json
    ...
    def save_config(self, values, openzuma_home):
        ...
        _save_supermemory_config(sanitized, openzuma_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from openzuma_constants import get_openzuma_home
        self._openzuma_home = kwargs.get("openzuma_home") or str(get_openzuma_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._openzuma_home)
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        metadata = {"source": "openzuma", "type": "conversation_turn"}
        ...
        def _run():
            try:
                self._client.add_memory(
                    content.strip(),
                    metadata={"source": "openzuma_memory", "target": target, "type": "explicit_memory"},
