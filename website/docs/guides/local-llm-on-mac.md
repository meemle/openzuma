...
Both expose an OpenAI-compatible `/v1/chat/completions` endpoint. Openzuma works with either one — just point it at `http://localhost:8080` or `http://localhost:8000`.
...
Openzuma automatically detects local endpoints (localhost, LAN IPs) and relaxes its streaming timeouts. No configuration needed for most setups.
...
OPENZUMA_STREAM_READ_TIMEOUT=1800
```

| Timeout | Default | Local auto-adjustment | Env var override |
|---------|---------|----------------------|------------------|
| Stream read (socket-level) | 120s | Raised to 1800s | `OPENZUMA_STREAM_READ_TIMEOUT` |
| Stale stream detection | 180s | Disabled entirely | `OPENZUMA_STREAM_STALE_TIMEOUT` |
| API call (non-streaming) | 1800s | No change needed | `OPENZUMA_API_TIMEOUT` |
