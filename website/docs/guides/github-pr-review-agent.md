...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  Openzuma Agent  ──▶  GitHub API  ──▶  Review     │
...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to Openzuma instantly when PRs are opened or updated.
:::
...
- **Openzuma Agent installed** — see the [Installation guide](/docs/getting-started/installation)
- **Gateway running** for cron jobs:
  ```bash
  openzuma gateway install   # Install as a service
  # or
...
Use `deliver: "local"` to save reviews to `~/.openzuma/cron/output/`. Great for testing before wiring up notifications.
:::
...
Make sure Openzuma can access GitHub. Start a chat:
...
Still in the chat, ask Openzuma to review a real PR:
...
A skill gives Openzuma consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.

```bash
mkdir -p ~/.openzuma/skills/code-review
```

Create `~/.openzuma/skills/code-review/SKILL.md`:
...
Verify it loaded — start `openzuma` and you should see `code-review` in the skills list at startup.

---

## Step 4: Teach It Your Conventions

This is what makes the reviewer actually useful. Start a session and teach Openzuma your team's standards:
...
2. Teach Openzuma your conventions via memory (Step 4)
3. The more context it has about your stack, the better the reviews

### Cron job doesn't run
```bash
openzuma gateway status    # Is the gateway running?
openzuma cron list         # Is the job enabled?
```
...
- **[Build a Plugin](/docs/guides/build-a-openzuma-plugin)** — wrap the review logic into a shareable plugin
