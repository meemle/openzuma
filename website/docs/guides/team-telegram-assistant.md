...
This tutorial walks you through setting up a Telegram bot powered by Openzuma Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Openzuma Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/docs/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.openzuma/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Openzuma itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Openzuma Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_openzuma_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Openzuma Agent. DM me for help with code, research, debugging, and more.
   ```
...
Add these lines to `~/.openzuma/.env`:
...
```
[Gateway] Starting Openzuma Gateway...
...
openzuma gateway stop
openzuma gateway status

# View live logs
journalctl --user -u openzuma-gateway -f
...
openzuma gateway start
openzuma gateway stop
tail -f ~/.openzuma/logs/gateway.log
```

:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `openzuma gateway install` to update the plist.
:::
...
```bash
openzuma gateway stop && openzuma gateway start
```
...
openzuma pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.openzuma/.env`:
...
Control how much detail the bot shows when using tools. In `~/.openzuma/config.yaml`:
...
Customize how the bot communicates by editing `~/.openzuma/SOUL.md`:

For a full guide, see [Use SOUL.md with Openzuma](/docs/guides/use-soul-with-openzuma).
...
```bash
# From the CLI
openzuma cron list          # View all scheduled jobs
openzuma cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u openzuma-gateway` (Linux) or `~/.openzuma/logs/gateway.log` (macOS) |
| Cron job output | `~/.openzuma/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.openzuma/cron/jobs.json` |
| Pairing data | `~/.openzuma/pairing/` |
| Session history | `~/.openzuma/sessions/` |
