...
2. **Openzuma spins up** a fresh agent session with your prompt
...
You can still follow this tutorial using `deliver: "local"`. Briefings will be saved to `~/.openzuma/cron/output/` and you can read them anytime.
:::
...
Openzuma will search the web, read through results, and produce something like:
...
---
3 stories • Sources searched: 8 • Generated by Openzuma Agent
```
...
Just tell Openzuma what you want:
...
Openzuma will create the cron job for you using the unified `cronjob` tool.
...
For faster briefings, tell Openzuma to delegate each topic to a sub-agent:
...
Openzuma will use `cronjob(action="list")` to find it and `cronjob(action="remove")` to delete it.
