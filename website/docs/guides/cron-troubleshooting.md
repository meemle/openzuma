...
description: "Diagnose and fix common Openzuma cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`openzuma gateway` or `openzuma serve`). For one-off debugging, you can manually trigger a tick with `openzuma cron tick`.
...
```bash
date
openzuma cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.openzuma/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.openzuma/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.openzuma/.env` |
...
| `local` | Write access to `~/.openzuma/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `openzuma cron list` for updated `last_error` field (if available).
...
```bash
openzuma skills list
```

Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `openzuma skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-daily-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `openzuma skills list`.
...
2. `~/.openzuma/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `openzuma cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the Openzuma config directory). Verify:
```bash
ls ~/.openzuma/scripts/your-script.py   # Must exist
openzuma cron edit <job_id> --script ~/.openzuma/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `openzuma skills install <skill-name>`.
...
```bash
ps aux | grep openzuma
# Kill duplicate processes, keep only one
```

### Check 4: Permissions on jobs.json

Jobs are stored in `~/.openzuma/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.openzuma/cron/jobs.json
chmod 600 ~/.openzuma/cron/jobs.json   # Your user should own it
```
...
```bash
openzuma cron list                    # Show all jobs, states, next_run times
openzuma cron run <job_id>            # Schedule for next tick (for testing)
openzuma cron edit <job_id>           # Fix configuration issues
openzuma logs                         # View recent Openzuma logs
openzuma skills list                  # Verify installed skills
```
