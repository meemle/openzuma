...
description: "Complete guide to migrating your OpenClaw / Clawdbot setup to Openzuma Agent — what gets migrated, how config maps, and what to check after."
---

# Migrate from OpenClaw

`openzuma claw migrate` imports your OpenClaw (or legacy Clawdbot/Moldbot) setup into Openzuma. This guide covers exactly what gets migrated, the config key mappings, and what to verify after migration.

## Quick start

```bash
# Preview then migrate (always shows a preview first, then asks to confirm)
openzuma claw migrate

# Preview only, no changes
openzuma claw migrate --dry-run

# Full migration including API keys, skip confirmation
openzuma claw migrate --preset full --yes
```
...
| What | OpenClaw source | Openzuma destination | Notes |
|------|----------------|-------------------|-------|
| Persona | `workspace/SOUL.md` | `~/.openzuma/SOUL.md` | Direct copy |
| Workspace instructions | `workspace/AGENTS.md` | `AGENTS.md` in `--workspace-target` | Requires `--workspace-target` flag |
| Long-term memory | `workspace/MEMORY.md` | `~/.openzuma/memories/MEMORY.md` | Parsed into entries, merged with existing, deduped. Uses `§` delimiter. |
| User profile | `workspace/USER.md` | `~/.openzuma/memories/USER.md` | Same entry-merge logic as memory. |
| Daily memory files | `workspace/memory/*.md` | `~/.openzuma/memories/MEMORY.md` | All daily files merged into main memory. |
...
| Source | OpenClaw location | Openzuma destination |
|--------|------------------|-------------------|
| Workspace skills | `workspace/skills/` | `~/.openzuma/skills/openclaw-imports/` |
| Managed/shared skills | `~/.openclaw/skills/` | `~/.openzuma/skills/openclaw-imports/` |
| Personal cross-project | `~/.agents/skills/` | `~/.openzuma/skills/openclaw-imports/` |
| Project-level shared | `workspace/.agents/skills/` | `~/.openzuma/skills/openclaw-imports/` |

Skill conflicts are handled by `--skill-conflict`: `skip` leaves the existing Openzuma skill, `overwrite` replaces it, `rename` creates a `-imported` copy.

### Model and provider configuration

| What | OpenClaw config path | Openzuma destination | Notes |
|------|---------------------|-------------------|-------|
...
| Provider API keys | `models.providers.*.apiKey` | `~/.openzuma/.env` | Requires `--migrate-secrets`. See [API key resolution](#api-key-resolution) below. |

### Agent behavior

| What | OpenClaw config path | Openzuma config path | Mapping |
|------|---------------------|-------------------|---------|
...
| OpenClaw config path | Openzuma config path | Notes |
|---------------------|-------------------|-------|
...
| OpenClaw field | Openzuma field | Notes |
|----------------|-------------|-------|
...
| TTS assets | `~/.openzuma/tts/` (file copy) |

### Messaging platforms

| Platform | OpenClaw config path | Openzuma `.env` variable | Notes |
|----------|---------------------|----------------------|-------|
...
| What | OpenClaw path | Openzuma path | Notes |
|------|-------------|-------------|-------|
...
| Gateway auth token | `gateway.auth.token` | `.env` → `OPENZUMA_GATEWAY_TOKEN` | Requires `--migrate-secrets` |
| Working directory | `agents.defaults.workspace` | `.env` → `MESSAGING_CWD` | |

### Archived (no direct Openzuma equivalent)

These are saved to `~/.openzuma/migration/openclaw/<timestamp>/archive/` for manual review:

| What | Archive file | How to recreate in Openzuma |
|------|-------------|--------------------------|
| `IDENTITY.md` | `archive/workspace/IDENTITY.md` | Merge into `SOUL.md` |
| `TOOLS.md` | `archive/workspace/TOOLS.md` | Openzuma has built-in tool instructions |
...
| Cron jobs | `archive/cron-config.json` | Recreate with `openzuma cron create` |
| Plugins | `archive/plugins-config.json` | See [plugins guide](/docs/user-guide/features/hooks) |
| Hooks/webhooks | `archive/hooks-config.json` | Use `openzuma webhook` or gateway hooks |
| Memory backend | `archive/memory-backend-config.json` | Configure via `openzuma honcho` |
| Skills registry | `archive/skills-registry-config.json` | Use `openzuma skills config` |
...
| Multi-agent list | `archive/agents-list.json` | Use Openzuma profiles |
...
The migration resolves all three formats. For env templates and SecretRef objects with `source: "env"`, it looks up the value in `~/.openclaw/.env` and the `openclaw.json` env sub-object. SecretRef objects with `source: "file"` or `source: "exec"` can't be resolved automatically — the migration warns about these, and those values must be added to Openzuma manually via `openzuma config set`.

## After migration

1. **Check the migration report** — printed on completion with counts of migrated, skipped, and conflicting items.

2. **Review archived files** — anything in `~/.openzuma/migration/openclaw/<timestamp>/archive/` needs manual attention.

3. **Start a new session** — imported skills and memory entries take effect in new sessions, not the current one.

4. **Verify API keys** — run `openzuma status` to check provider authentication.

5. **Test messaging** — if you migrated platform tokens, restart the gateway: `systemctl --user restart openzuma-gateway`

6. **Check session policies** — verify `openzuma config get session_reset` matches your expectations.

7. **Re-pair WhatsApp** — WhatsApp uses QR code pairing (Baileys), not token migration. Run `openzuma whatsapp` to pair.

8. **Archive cleanup** — after confirming everything works, run `openzuma claw cleanup` to rename leftover OpenClaw directories to `.pre-migration/` (prevents state confusion).
...
Keys might be stored in several places depending on your OpenClaw version: inline in `openclaw.json` under `models.providers.*.apiKey`, in `~/.openclaw/.env`, in the `openclaw.json` `"env"` sub-object, or in `agents/main/agent/auth-profiles.json`. The migration checks all four. If keys use `source: "file"` or `source: "exec"` SecretRefs, they can't be resolved automatically — add them via `openzuma config set`.

### Skills not appearing after migration

Imported skills land in `~/.openzuma/skills/openclaw-imports/`. Start a new session for them to take effect, or run `/skills` to verify they're loaded.

### TTS voice not migrated

OpenClaw stores TTS settings in two places: `messages.tts.providers.*` and the top-level `talk` config. The migration checks both. If your voice ID was set via the OpenClaw UI (stored in a different path), you may need to set it manually: `openzuma config set tts.elevenlabs.voice_id YOUR_VOICE_ID`.
