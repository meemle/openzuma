---
sidebar_position: 6
title: "Use MCP with Openzuma"
description: "A practical guide to connecting MCP servers to Openzuma Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Openzuma

This guide shows how to actually use MCP with Openzuma Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Openzuma tool
- you want Openzuma to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Openzuma to internal APIs, databases, or company systems without modifying Openzuma core

Do not use MCP when:
- a built-in Openzuma tool already solves the job well
...
- Openzuma remains the agent
- MCP servers contribute tools
- Openzuma discovers those tools at startup or reload time
...
If you installed Openzuma with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Openzuma banner/status should show MCP integration when configured
- ask Openzuma what tools it has available
...
There are two categories of MCP-exposed functionality in Openzuma:
...
2. Openzuma-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Openzuma will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Openzuma to reason over a bounded workspace.
...
Now Openzuma can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Openzuma core.
...
Because Openzuma now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
