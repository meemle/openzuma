...
Copy-paste recipes for common automation patterns. Each template uses Openzuma's built-in [cron scheduler](/docs/user-guide/features/cron) for time-based triggers and [webhook platform](/docs/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`openzuma webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `openzuma webhook subscribe`) |
...
```bash
openzuma cron create "0 2 * * *" \
  "You are a project manager triaging the NousResearch/openzuma-agent GitHub repo.

1. Run: gh issue list --repo NousResearch/openzuma-agent --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
openzuma webhook subscribe github-pr-review \
  --events "pull_request" \
...
Then in GitHub: **Settings → Webhooks → Add webhook** → Payload URL: `http://your-server:8744/webhooks/github-pr-review`, Content type: `application/json`, Secret: `github-webhook-secret`, Events: **Pull requests**.
...
```bash
openzuma cron create "0 9 * * 1" \
  "Scan the NousResearch/openzuma-agent repo for documentation drift.

1. Run: gh pr list --repo NousResearch/openzuma-agent --state merged --json number,title,files,mergedAt --limit 30
2. Filter to PRs merged in the last 7 days
3. For each merged PR, check if it modified:
   - Tool schemas (tools/*.py) — may need docs/reference/tools-reference.md update
   - CLI commands (openzuma_cli/commands.py, openzuma_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (openzuma_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
openzuma cron create "0 6 * * *" \
  "Run a dependency security audit on the openzuma-agent project.

1. cd ~/.openzuma/openzuma-agent && source .venv/bin/activate
...
```bash
openzuma webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
curl -X POST http://your-server:8744/webhooks/deploy-verify \
...
```bash
openzuma webhook subscribe alert-triage \
...
```python title="~/.openzuma/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Openzuma-Monitor/1.0"})
...
```bash
openzuma cron create "every 30m" \
  ...
  --script ~/.openzuma/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
openzuma cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Openzuma Agent development." \
...
```bash
openzuma webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
openzuma webhook subscribe stripe-payments \
...
```bash
openzuma cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the openzuma-agent codebase.
