...
openzuma cron create "0 2 * * *" \
  "You are a project manager triaging the meemle/openzuma GitHub repo.

1. Run: gh issue list --repo meemle/openzuma --state open --json number,title,labels,author,createdAt --limit 30
...
openzuma cron create "0 9 * * 1" \
  "Scan the meemle/openzuma repo for documentation drift.

1. Run: gh pr list --repo meemle/openzuma --state merged --json number,title,files,mergedAt --limit 30
