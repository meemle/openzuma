...
sidebar_position: 1
title: "Quickstart"
description: "Your first conversation with Openzuma Agent — from install to chatting in under 5 minutes"
---

# Quickstart

This guide gets you from zero to a working Openzuma setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Who this is for

- Brand new and want the shortest path to a working setup
- Switching providers and don't want to lose time to config mistakes
- Setting up Openzuma for a team, bot, or always-on workflow
...
|---|---|---|
| I just want Openzuma working on my machine | `openzuma setup` | Run a real chat and verify it responds |
| I already know my provider | `openzuma model` | Save the config, then start chatting |
| I want a bot or always-on setup | `openzuma gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `openzuma model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `openzuma model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if Openzuma cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.

---

## 1. Install Openzuma Agent
...
The single most important setup step. Use `openzuma model` to walk through the choice interactively:
...
Openzuma Agent requires a model with at least **64,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 64K (e.g. `--ctx-size 65536` for llama.cpp or `-c 65536` for Ollama).
:::

:::tip
You can switch providers at any time with `openzuma model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

Openzuma separates secrets from normal config:

- **Secrets and tokens** → `~/.openzuma/.env`
- **Non-secret settings** → `~/.openzuma/config.yaml`

The easiest way to set values correctly is through the CLI:

```bash
openzuma config set model anthropic/claude-opus-4.6
openzuma config set terminal.backend docker
openzuma config set OPENROUTER_API_KEY sk-or-...
```
...
```bash
openzuma            # classic CLI
openzuma --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
Openzuma ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `openzuma` vs `openzuma --tui`.
:::
...
- Openzuma replies without error
...
```bash
openzuma --continue    # Resume the most recent session
openzuma -c            # Short form
```
...
```bash
openzuma gateway setup    # Interactive platform configuration
```
...
- `openzuma tools` — tune tool access per platform
- `openzuma skills` — browse and install reusable workflows
...
```bash
openzuma config set terminal.backend docker    # Docker isolation
openzuma config set terminal.backend ssh       # Remote server
```

### Voice mode

```bash
pip install "openzuma-agent[voice]"
...
```bash
openzuma skills search kubernetes
...
|---|---|---|
| Openzuma opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `openzuma model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `openzuma gateway setup` and check `openzuma gateway status` |
| `openzuma --continue` can't find old session | Switched profiles or session never saved | Check `openzuma sessions list` and confirm you're in the right profile |
...
| `openzuma doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
2. `openzuma model`
3. `openzuma setup`
4. `openzuma sessions list`
5. `openzuma --continue`
6. `openzuma gateway status`
...
| `openzuma tools` | Configure which tools are enabled per platform |
| `openzuma setup` | Full setup wizard (configures everything at once) |
| `openzuma doctor` | Diagnose issues |
| `openzuma update` | Update to latest version |
| `openzuma gateway` | Start the messaging gateway |
| `openzuma --continue` | Resume last session |
