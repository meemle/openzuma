...
description: "Run Openzuma Agent directly on an Android phone with Termux"
---

# Openzuma on Android with Termux

This is the tested path for running Openzuma Agent directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Openzuma from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

Openzuma now ships a Termux-aware installer path:
...
- links `openzuma` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `openzuma` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/openzuma" "$PREFIX/bin/openzuma"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `openzuma` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.openzuma/.env`.
...
### `openzuma doctor` says ripgrep or Node is missing
