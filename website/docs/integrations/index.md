...
Openzuma Agent connects to external systems for AI inference, tool servers, IDE workflows, programmatic access, and more. These integrations extend what Openzuma can do and where it can run.

## AI Providers & Routing

Openzuma supports multiple AI inference providers out of the box. Use `openzuma model` to configure interactively, or set them in `config.yaml`.

- **[AI Providers](/docs/user-guide/features/provider-routing)** — OpenRouter, Anthropic, OpenAI, Google, and any OpenAI-compatible endpoint. Openzuma auto-detects capabilities like vision, streaming, and tool use per provider.
...
- **[MCP Servers](/docs/user-guide/features/mcp)** — Connect Openzuma to external tool servers via Model Context Protocol. Access tools from GitHub, databases, file systems, browser stacks, internal APIs, and more without writing native Openzuma tools. Supports both stdio and SSE transports, per-server tool filtering, and capability-aware resource/prompt registration.

## Web Search Backends

The `web_search` and `web_extract` tools support four backend providers, configured via `config.yaml` or `openzuma tools`:
...
Openzuma includes full browser automation with multiple backend options for navigating websites, filling forms, and extracting information:
...
- **[IDE Integration (ACP)](/docs/user-guide/features/acp)** — Use Openzuma Agent inside ACP-compatible editors such as VS Code, Zed, and JetBrains. Openzuma runs as an ACP server, rendering chat messages, tool activity, file diffs, and terminal commands inside your editor.

## Programmatic Access

- **[API Server](/docs/user-guide/features/api-server)** — Expose Openzuma as an OpenAI-compatible HTTP endpoint. Any frontend that speaks the OpenAI format — Open WebUI, LobeChat, LibreChat, NextChat, ChatBox — can connect and use Openzuma as a backend with its full toolset.
...
Openzuma runs as a gateway bot on 15+ messaging platforms, all configured through the same `gateway` subsystem:
...
- **[Plugin System](/docs/user-guide/features/plugins)** — Extend Openzuma with custom tools, lifecycle hooks, and CLI commands without modifying core code. Plugins are discovered from `~/.openzuma/plugins/`, project-local `.openzuma/plugins/`, and pip-installed entry points.
- **[Build a Plugin](/docs/guides/build-a-openzuma-plugin)** — Step-by-step guide for creating Openzuma plugins with tools, hooks, and CLI commands.
