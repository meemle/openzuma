...
The messaging gateway is the long-running process that connects Openzuma to 14+ external messaging platforms through a unified architecture.
...
1. `resolve_command()` from `openzuma_cli/commands.py` maps input to canonical name (handles aliases, prefix matching)
...
|--------|-----------------|
| `~/.openzuma/.env` | API keys, bot tokens, platform credentials |
| `~/.openzuma/config.yaml` | Model settings, tool configuration, display options |
...
Hooks are discovered from `gateway/builtin_hooks/` (always active) and `~/.openzuma/hooks/` (user-installed). Each hook is a directory with a `HOOK.yaml` manifest and `handler.py`.
...
- `openzuma gateway start` / `openzuma gateway stop` — manual control
- `systemctl` (Linux) or `launchctl` (macOS) — service management
- PID file at `~/.openzuma/gateway.pid` — profile-scoped process tracking

**Profile-scoped vs global**: `start_gateway()` uses profile-scoped PID files. `openzuma gateway stop` stops only the current profile's gateway. `openzuma gateway stop --all` uses global `ps aux` scanning to kill all gateway processes (used during updates).
