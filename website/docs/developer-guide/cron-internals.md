...
description: "How Openzuma stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `openzuma_cli/cron.py` | CLI `openzuma cron` subcommands |
...
Jobs are stored in `~/.openzuma/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `openzuma cron` commands are run or during active CLI sessions.
...
```python
# ~/.openzuma/scripts/check_competitors.py
...
2. **Environment variable** — `OPENZUMA_CRON_SCRIPT_TIMEOUT`
...
| Local file | `local` | Save to `~/.openzuma/cron/output/` |
...
The `openzuma cron` CLI provides direct job management:

```bash
openzuma cron list                    # Show all jobs
openzuma cron create                  # Interactive job creation (alias: add)
openzuma cron edit <job_id>           # Edit job configuration
openzuma cron pause <job_id>          # Pause a running job
openzuma cron resume <job_id>         # Resume a paused job
openzuma cron run <job_id>            # Trigger immediate execution
openzuma cron remove <job_id>         # Delete a job
```
