...
description: "Building RL training environments, running evaluation benchmarks, and generating SFT data with the Openzuma-Agent Atropos integration"
---

# Environments, Benchmarks & Data Generation

Openzuma Agent includes a full environment framework that connects its tool-calling capabilities to the [Atropos](https://github.com/NousResearch/atropos) RL training framework. This enables three workflows:
...
The Python environment framework documented here lives under the repo's `environments/` directory and is the implementation-level API for Openzuma/Atropos integration. This is separate from the user-facing `rl_*` tools, which operate as an orchestration surface for remote RL training workflows.
:::
...
classDiagram
    ...
    BaseEnv <|-- OpenzumaAgentBaseEnv
    OpenzumaAgentBaseEnv <|-- TerminalTestEnv
    OpenzumaAgentBaseEnv <|-- OpenzumaSweEnv
    OpenzumaAgentBaseEnv <|-- TerminalBench2EvalEnv
...
### OpenzumaAgentBaseEnv

The openzuma-agent layer (`environments/openzuma_base_env.py`). Adds:
- **Terminal backend configuration** — sets `TERMINAL_ENV` for sandboxed execution (local, Docker, Modal, Daytona, SSH, Singularity)
- **Tool resolution** — `_resolve_tools_for_group()` calls openzuma-agent's `get_tool_definitions()` to get the right tool schemas based on enabled/disabled toolsets
- **Agent loop integration** — `collect_trajectory()` runs `OpenzumaAgentLoop` and scores the result
...
Your environment inherits from `OpenzumaAgentBaseEnv` and implements five methods:
...
`OpenzumaAgentLoop` (`environments/agent_loop.py`) is the reusable multi-turn agent engine. It runs the same tool-calling pattern as openzuma-agent's main loop:
...
| **Generic** | `call_tool(name, args)` — escape hatch for any openzuma-agent tool |
...
parser = get_parser("openzuma")  # or "mistral", "llama3_json", "qwen", "deepseek_v3", etc.
content, tool_calls = parser.parse(raw_model_output)
```

Available parsers: `openzuma`, `mistral`, `llama3_json`, `qwen`, `qwen3_coder`, `deepseek_v3`, `deepseek_v3_1`, `kimi_k2`, `longcat`, `glm45`, `glm47`.
...
pip install "openzuma-agent[yc-bench]"
...
```bash
python environments/openzuma_swe_env/openzuma_swe_env.py serve \
...
- Set `tool_call_parser` in config to match your model's format (e.g., `"openzuma"`, `"qwen"`, `"mistral"`)
...
class MyEnvConfig(OpenzumaAgentEnvConfig):
    my_custom_field: str = "default_value"

class MyEnv(OpenzumaAgentBaseEnv):
    name = "my-env"
...
### OpenzumaAgentEnvConfig Fields

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `enabled_toolsets` | `List[str]` | `None` (all) | Which openzuma toolsets to enable |
...
| `tool_call_parser` | `str` | `"openzuma"` | Parser for Phase 2 raw output |
...
env:
  ...
  tokenizer_name: "NousResearch/Openzuma-3-Llama-3.1-8B"
  use_wandb: true
...
- [Modal](https://modal.com) account and CLI: `pip install "openzuma-agent[modal]"`
- `MODAL_TOKEN_ID` and `MODAL_TOKEN_SECRET` environment variables

### For YC-Bench

- `pip install "openzuma-agent[yc-bench]"` (installs the yc-bench CLI + SQLAlchemy)
...
```
environments/
├── openzuma_base_env.py          # Abstract base class (OpenzumaAgentBaseEnv)
├── agent_loop.py               # Multi-turn agent engine (OpenzumaAgentLoop)
├── tool_context.py             # Per-rollout tool access for reward functions
├── patches.py                  # Async-safety patches for Modal backend
│
├── tool_call_parsers/          # Phase 2 client-side parsers
│   ├── openzuma_parser.py        # Openzuma/ChatML <tool_call> format
...
├── openzuma_swe_env/             # SWE-bench training environment
│
