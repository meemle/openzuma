...
Openzuma tools are self-registering functions grouped into toolsets and executed through a central registry/dispatch system.
...
2. **Plugin tools** — `openzuma_cli.plugins.discover_plugins()` loads user/project/pip plugins that may register additional tools.
...
Toolsets are named bundles of tools. Openzuma resolves them through:

- explicit enabled/disabled toolset lists
- platform presets (`openzuma-cli`, `openzuma-telegram`, etc.)
- dynamic MCP toolsets
- curated special-purpose sets like `openzuma-acp`
