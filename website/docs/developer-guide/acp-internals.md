...
The ACP adapter wraps Openzuma' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
openzuma acp / openzuma-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> load ~/.openzuma/.env
  -> configure stderr logging
  -> construct OpenzumaACPAgent
  -> acp.run_agent(agent)
```
...
- `allow_once` -> Openzuma `once`
- `allow_always` -> Openzuma `always`
- reject options -> Openzuma `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Openzuma tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["openzuma-acp"])
...
Instead it reuses Openzuma' runtime resolver:

- `acp_adapter/auth.py`
- `openzuma_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Openzuma provider/credentials.
...
- `toolsets.py` — `openzuma-acp` toolset definition
- `openzuma_cli/main.py` — `openzuma acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `openzuma-acp` script
