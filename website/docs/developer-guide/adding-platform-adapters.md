...
This guide covers adding a new messaging platform to the Openzuma gateway. A platform adapter connects Openzuma to an external messaging service (Telegram, Discord, WeCom, etc.) so users can interact with the agent through that service.
...
1. **`openzuma_cli/config.py`** — Add all `NEWPLAT_*` vars to `_EXTRA_ENV_KEYS`
2. **`openzuma_cli/gateway.py`** — Add entry to `_PLATFORMS` list with key, label, emoji, token_var, setup_instructions, and vars
3. **`openzuma_cli/platforms.py`** — Add `PlatformInfo` entry with label and default_toolset (used by `skills_config` and `tools_config` TUIs)
4. **`openzuma_cli/setup.py`** — Add `_setup_newplat()` function (can delegate to `gateway.py`) and add tuple to the messaging platforms list
5. **`openzuma_cli/status.py`** — Add platform detection entry: `"NewPlat": ("NEWPLAT_TOKEN", "NEWPLAT_HOME_CHANNEL")`
6. **`openzuma_cli/dump.py`** — Add `"newplat": "NEWPLAT_TOKEN"` to platform detection dict
...
1. **`toolsets.py`** — Add `"openzuma-newplat"` toolset definition with `_OPENZUMA_CORE_TOOLS`
2. **`toolsets.py`** — Add `"openzuma-newplat"` to the `"openzuma-gateway"` includes list
...
| `website/docs/reference/toolsets-reference.md` | openzuma-newplat toolset |
