...
description: "How Openzuma builds the system prompt, preserves cache stability, and injects ephemeral layers"
---

# Prompt Assembly

Openzuma deliberately separates:
...
1. agent identity — `SOUL.md` from `OPENZUMA_HOME` when available, otherwise falls back to `DEFAULT_AGENT_IDENTITY` in `prompt_builder.py`
...
```
# Layer 1: Agent Identity (from ~/.openzuma/SOUL.md)
You are Openzuma, an AI assistant created by Nous Research.
...
`SOUL.md` lives at `~/.openzuma/SOUL.md` and serves as the agent's identity — the very first section of the system prompt. The loading logic in `prompt_builder.py` works as follows:
...
def build_context_files_prompt(cwd=None, skip_soul=False):
    ...
    project_context = (
        _load_openzuma_md(cwd_path)       # 1. .openzuma.md / OPENZUMA.md (walks to git root)
    ...
    # SOUL.md from OPENZUMA_HOME (independent of project context)
    if not skip_soul:
...
|----------|-------|-------------|-------|
| 1 | `.openzuma.md`, `OPENZUMA.md` | CWD up to git root | Openzuma-native project config |
...
- **YAML frontmatter stripped** — `.openzuma.md` frontmatter is removed (reserved for future config overrides)
...
1. `.openzuma.md` / `OPENZUMA.md` (walks to git root)
