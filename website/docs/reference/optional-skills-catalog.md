...
description: "Official optional skills shipped with openzuma-agent — install via openzuma skills install official/<category>/<skill>"
---

# Optional Skills Catalog

Official optional skills ship with the openzuma-agent repository under `optional-skills/` but are **not active by default**. Install them explicitly:
...
```bash
openzuma skills install official/blockchain/solana
openzuma skills install official/mlops/flash-attention
```
...
```bash
openzuma skills uninstall <skill-name>
```
...
| **honcho** | Configure and use Honcho memory with Openzuma — cross-session user modeling, multi-profile peer isolation, observation config, and dialectic reasoning. |
...
| Skill | Description |
|-------|-------------|
| **blender-mcp** | Control Blender directly from Openzuma via socket connection to the blender-mcp addon. Create 3D objects, materials, animations, and run arbitrary Blender Python (bpy) code. |
...
| Skill | Description |
|-------|-------------|
| **openclaw-migration** | Migrate a user's OpenClaw customization footprint into Openzuma Agent. Imports memories, SOUL.md, command allowlists, user skills, and selected workspace assets. |
...
| **openzuma-atropos-environments** | Build, test, and debug Openzuma Agent RL environments for Atropos training. Covers the OpenzumaAgentBaseEnv interface, reward functions, agent loop integration, and evaluation. |
...
| **telephony** | Give Openzuma phone capabilities — provision a Twilio number, send/receive SMS/MMS, make calls, and place AI-driven outbound calls through Bland.ai or Vapi. |
