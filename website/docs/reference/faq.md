...
description: "Frequently asked questions and solutions to common issues with Openzuma Agent"
---
...
### What LLM providers work with Openzuma?

Openzuma Agent works with any OpenAI-compatible API. Supported providers include:
...
Set your provider with `openzuma model` or by editing `~/.openzuma/.env`. See the [Environment Variables](./environment-variables.md) reference for all provider keys.

### Does it work on Windows?

**Not natively.** Openzuma Agent requires a Unix-like environment. On Windows, install [WSL2](https://learn.microsoft.com/en-us/windows/wsl/install) and run Openzuma from inside it. The standard install command works perfectly in WSL2:
...
Yes — Openzuma now has a tested Termux install path for Android phones.
...
API calls go **only to the LLM provider you configure** (e.g., OpenRouter, your local Ollama instance). Openzuma Agent does not collect telemetry, usage data, or analytics. Your conversations, memory, and skills are stored locally in `~/.openzuma/`.

### Can I use it offline / with local models?

Yes. Run `openzuma model`, select **Custom endpoint**, and enter your server's URL:
...
Openzuma persists the endpoint, provider, and base URL in `config.yaml` so it survives restarts. If your local server has exactly one model loaded, `/model custom` auto-detects it. You can also set `provider: custom` in config.yaml — it's a first-class provider, not an alias for anything else.
...
:::tip Ollama users
If you set a custom `num_ctx` in Ollama (e.g., `ollama run --num_ctx 16384`), make sure to set the matching context length in Openzuma — Ollama's `/api/show` reports the model's *maximum* context, not the effective `num_ctx` you configured.
:::

:::tip Timeouts with local models
Openzuma auto-detects local endpoints and relaxes streaming timeouts (read timeout raised from 120s to 1800s, stale stream detection disabled). If you still hit timeouts on very large contexts, set `OPENZUMA_STREAM_READ_TIMEOUT=1800` in your `.env`. See the [Local LLM guide](../guides/local-llm-on-mac.md#timeouts) for details.
:::

### How much does it cost?

Openzuma Agent itself is **free and open-source** (MIT license). You pay only for the LLM API usage from your chosen provider. Local models are completely free to run.

### Can multiple people use one instance?

Yes. The [messaging gateway](../user-guide/messaging/index.md) lets multiple users interact with the same Openzuma Agent instance via Telegram, Discord, Slack, WhatsApp, or Home Assistant. Access is controlled through allowlists (specific user IDs) and DM pairing (first user to message claims access).
...
Yes. Import the `AIAgent` class and use Openzuma programmatically:
...
#### `openzuma: command not found` after installation
...
**Cause:** Openzuma requires Python 3.11 or newer.
...
**Cause:** Openzuma builds a per-session environment snapshot by running `bash -l` once at startup. A bash login shell reads `/etc/profile`, `~/.bash_profile`, and `~/.profile`, but **does not source `~/.bashrc`** — so tools that install themselves there (`nvm`, `asdf`, `pyenv`, `cargo`, custom `PATH` exports) stay invisible to the snapshot. This most commonly happens when Openzuma runs under systemd or in a minimal shell where nothing has pre-loaded the interactive shell profile.

**Solution:** Openzuma auto-sources `~/.bashrc` by default. If that's not enough — e.g. you're a zsh user whose PATH lives in `~/.zshrc`, or you init `nvm` from a standalone file — list the extra files to source in `~/.openzuma/config.yaml`:
...
sudo rm /usr/local/bin/openzuma
...
**Solution:** Exit your session and use `openzuma model` from your terminal to add new providers:

```bash
# Exit the Openzuma chat session first (Ctrl+C or /quit)
...
After adding a new provider via `openzuma model`, start a new chat session — `/model` will now show all your configured providers.

:::tip Quick reference
| Want to... | Use |
|-----------|-----|
| Add a new provider | `openzuma model` (from terminal) |
| Enter/change API keys | `openzuma model` (from terminal) |
...
openzuma config show

# Re-configure your provider
openzuma model

# Or set directly
openzuma config set OPENROUTER_API_KEY sk-or-v1-xxxxxxxxxxxx
```

:::warning
Make sure the key matches the provider. An OpenAI key won't work with OpenRouter and vice versa. Check `~/.openzuma/.env` for conflicting entries.
:::
...
# Set a valid model
openzuma config set OPENZUMA_MODEL anthropic/claude-opus-4.7

# Or specify per-session
openzuma chat --model openrouter/meta-llama/llama-3.1-70b-instruct
```
...
- Using `openzuma chat --provider <alternative>` to route to a different backend

#### Context length exceeded

**Cause:** The conversation has grown too long for the model's context window, or Openzuma detected the wrong context length for your model.
...
openzuma chat --model openrouter/google/gemini-3-flash-preview
```

If this happens on the first long conversation, Openzuma may have the wrong context length for your model. Check what it detected:
...
**Cause:** Openzuma detected a potentially destructive command (e.g., `rm -rf`, `DROP TABLE`). This is a safety feature.
...
:::tip
This is working as intended — Openzuma never silently runs destructive commands. The approval prompt shows you exactly what will execute.
:::
...
- Or switch to the terminal interface for administrative tasks: `openzuma chat`
...
# Start the gateway
openzuma gateway start

# Check logs for errors
cat ~/.openzuma/logs/gateway.log | tail -50
```
...
**Solution:**
- Verify your bot token is valid with `openzuma gateway setup`
- Check gateway logs: `cat ~/.openzuma/logs/gateway.log | tail -50`
...
Configure in `~/.openzuma/config.yaml` under your gateway's settings. See the [Messaging docs](../user-guide/messaging/index.md).
...
pip install "openzuma-agent[telegram]"   # or [discord], [slack], [whatsapp]
...
#### WSL: Gateway keeps disconnecting or `openzuma gateway start` fails
...
openzuma gateway run

# Option 2: Persistent via tmux (survives terminal close)
tmux new -s openzuma 'openzuma gateway run'
# Reattach later: tmux attach -t openzuma

# Option 3: Background via nohup
nohup openzuma gateway run > ~/.openzuma/logs/gateway.log 2>&1 &
```
...
1. Create a task that runs `wsl -d Ubuntu -- bash -lc 'openzuma gateway run'`
...
**Solution:** The gateway captures your shell PATH when you run `openzuma gateway install`. If you installed tools after setting up the gateway, re-run the install to capture the updated PATH:

```bash
openzuma gateway install    # Re-snapshots your current PATH
openzuma gateway start      # Detects the updated plist and reloads
```

You can verify the plist has the correct PATH:
```bash
/usr/libexec/PlistBuddy -c "Print :EnvironmentVariables:PATH" \
  ~/Library/LaunchAgents/ai.openzuma.gateway.plist
```
...
**Solution:**
- Try a faster/smaller model: `openzuma chat --model openrouter/meta-llama/llama-3.1-8b-instruct`
- Reduce active toolsets: `openzuma chat -t "terminal"`
...
cd ~/.openzuma/openzuma-agent && uv pip install -e ".[mcp]"
...
Verify your `~/.openzuma/config.yaml` MCP configuration:
```yaml
mcp_servers:
  filesystem:
    command: "npx"
...
openzuma config show | grep -A 12 mcp_servers

# Restart Openzuma or reload MCP after config changes
openzuma chat
```

See also:
- [MCP (Model Context Protocol)](/docs/user-guide/features/mcp)
- [Use MCP with Openzuma](/docs/guides/use-mcp-with-openzuma)
...
:::warning
If an MCP server crashes mid-request, Openzuma will report a timeout. Check the server's own logs (not just Openzuma logs) to diagnose the root cause.
:::
...
No. Each profile has its own memory store, session database, and skills directory. They are completely isolated. If you want to start a new profile with existing memories and sessions, use `openzuma profile create newname --clone-all` to copy everything from the current profile.

### What happens when I run `openzuma update`?

`openzuma update` pulls the latest code and reinstalls dependencies **once** (not per-profile). It then syncs updated skills to all profiles automatically. You only need to run `openzuma update` once — it covers every profile on the machine.
...
openzuma profile export work ./work-backup.tar.gz

# Copy the file to the target machine, then:
openzuma profile import ./work-backup.tar.gz work
```
...
There is no hard limit. Each profile is just a directory under `~/.openzuma/profiles/`. The practical limit depends on your disk space and how many concurrent gateways your system can handle (each gateway is a lightweight Python process). Running dozens of profiles is fine; each idle profile uses no resources.
...
**Solution: Delegation config.** Openzuma can route subagents to a different model automatically. Set this in `~/.openzuma/config.yaml`:
...
Now when you tell Openzuma "write me a Twitter thread about X" and it spawns a `delegate_task` subagent, that subagent runs on Gemini instead of your main model. Your primary conversation stays on GPT-5.4.
...
**Scenario:** In OpenClaw, you had multiple independent agents bound to specific WhatsApp chats — one for a family shopping list group, another for your private chat. Can Openzuma do this?

**Current limitation:** Openzuma profiles each require their own WhatsApp number/session. You cannot bind multiple profiles to different chats on the same WhatsApp number — the WhatsApp bridge (Baileys) uses one authenticated session per number.
...
**Scenario:** You see gateway exec logs, Openzuma reasoning, and tool call details in Telegram instead of just the final output.
...
**Scenario:** Telegram has a 100 slash command limit, and your skills are pushing past it. You want to disable skills you don't need on Telegram, but `openzuma skills config` settings don't seem to take effect.

**Solution:** Use `openzuma skills config` to disable skills per-platform. This writes to `config.yaml`:
...
After changing this, **restart the gateway** (`openzuma gateway restart` or kill and relaunch). The Telegram bot command menu rebuilds on startup.
...
**Current behavior:** Openzuma creates sessions keyed by user ID on most platforms, so each person gets their own conversation context. This is by design for privacy and context isolation.
...
### Exporting Openzuma to another machine
...
1. Install Openzuma Agent on the new machine:
   ```bash
   curl -fsSL https://raw.githubusercontent.com/NousResearch/openzuma-agent/main/scripts/install.sh | bash
   ```

2. Copy your entire `~/.openzuma/` directory **except** the `openzuma-agent` subdirectory (that's the code repo — the new install has its own):
   ```bash
   # On the source machine
   rsync -av --exclude='openzuma-agent' ~/.openzuma/ newmachine:~/.openzuma/
   ```

   Or use profile export/import:
   ```bash
   # On source machine
   openzuma profile export default ./openzuma-backup.tar.gz

   # On target machine
   openzuma profile import ./openzuma-backup.tar.gz default
   ```

3. On the new machine, run `openzuma setup` to verify API keys and provider config are working. Re-authenticate any messaging platforms (especially WhatsApp, which uses QR pairing).

The `~/.openzuma/` directory contains everything: `config.yaml`, `.env`, `SOUL.md`, `memories/`, `skills/`, `state.db` (sessions), `cron/`, and any custom plugins. The code itself lives in `~/.openzuma/openzuma-agent/` and is installed fresh.

### Permission denied when reloading shell after install

**Scenario:** After running the Openzuma installer, `source ~/.zshrc` gives a permission denied error.

**Cause:** This usually happens when `~/.zshrc` (or `~/.bashrc`) has incorrect file permissions, or when the installer couldn't write to it cleanly. It's not a Openzuma-specific issue — it's a shell config permissions problem.
...
openzuma config show | head -20

# Re-run model selection
openzuma model

# Or test with a known-good model
openzuma chat -q "hello" --model anthropic/claude-opus-4.7
```
...
1. **Search existing issues:** [GitHub Issues](https://github.com/NousResearch/openzuma-agent/issues)
2. **Ask the community:** [Nous Research Discord](https://discord.gg/nousresearch)
3. **File a bug report:** Include your OS, Python version (`python3 --version`), Openzuma version (`openzuma --version`), and the full error message
