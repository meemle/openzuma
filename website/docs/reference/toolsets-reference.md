...
description: "Reference for Openzuma core, composite, platform, and dynamic toolsets"
---
...
- **Platform** — A complete tool configuration for a specific deployment context (e.g., `openzuma-cli` is the default for interactive CLI sessions)
...
```bash
openzuma chat --toolsets web,file,terminal
openzuma chat --toolsets debugging        # composite — expands to file + terminal + web
openzuma chat --toolsets all              # everything
```
...
```yaml
toolsets:
  - openzuma-cli          # default for CLI
  # - openzuma-telegram   # override for Telegram gateway
```

### Interactive management

```bash
openzuma tools                            # curses UI to enable/disable per platform
```
...
| `code_execution` | `execute_code` | Run Python scripts that call Openzuma tools programmatically. |
...
| `feishu_drive` | `feishu_drive_add_comment`, `feishu_drive_list_comments`, `feishu_drive_list_comment_replies`, `feishu_drive_reply_comment` | Feishu/Lark drive comment operations. Scoped to the comment agent; not exposed on `openzuma-cli` or other messaging toolsets. |
...
Platform toolsets define the complete tool configuration for a deployment target. Most messaging platforms use the same set as `openzuma-cli`:

| Toolset | Differences from `openzuma-cli` |
|---------|-------------------------------|
| `openzuma-cli` | Full toolset — all 36 core tools including `clarify`. The default for interactive CLI sessions. |
| `openzuma-acp` | Drops `clarify`, `cronjob`, `image_generate`, `send_message`, `text_to_speech`, homeassistant tools. Focused on coding tasks in IDE context. |
| `openzuma-api-server` | Drops `clarify`, `send_message`, and `text_to_speech`. Adds everything else — suitable for programmatic access where user interaction isn't possible. |
| `openzuma-telegram` | Same as `openzuma-cli`. |
| `openzuma-discord` | Same as `openzuma-cli`. |
| `openzuma-slack` | Same as `openzuma-cli`. |
| `openzuma-whatsapp` | Same as `openzuma-cli`. |
| `openzuma-signal` | Same as `openzuma-cli`. |
| `openzuma-matrix` | Same as `openzuma-cli`. |
| `openzuma-mattermost` | Same as `openzuma-cli`. |
| `openzuma-email` | Same as `openzuma-cli`. |
| `openzuma-sms` | Same as `openzuma-cli`. |
...
| `openzuma-homeassistant` | Same as `openzuma-cli` plus the `homeassistant` toolset always on. |
| `openzuma-webhook` | Same as `openzuma-cli`. |
| `openzuma-gateway` | Internal gateway orchestrator toolset — union of the broadest possible tool set when the gateway needs to accept any message source. |
...
## Relationship to `openzuma tools`

The `openzuma tools` command provides a curses-based UI for toggling individual tools on or off per platform. This operates at the tool level (finer than toolsets) and persists to `config.yaml`. Disabled tools are filtered out even if their toolset is enabled.
