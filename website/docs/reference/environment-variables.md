...
description: "Complete reference of all environment variables used by Openzuma Agent"
---

# Environment Variables Reference

All variables go in `~/.openzuma/.env`. You can also set them with `openzuma config set VAR value`.
...
| `OPENZUMA_COPILOT_ACP_COMMAND` | Override Copilot ACP CLI binary path (default: `copilot`) |
| `COPILOT_CLI_PATH` | Alias for `OPENZUMA_COPILOT_ACP_COMMAND` |
| `OPENZUMA_COPILOT_ACP_ARGS` | Override Copilot ACP arguments (default: `--acp --stdio`) |
...
| `MINIMAX_BASE_URL` | Override MiniMax base URL (default: `https://api.minimax.io/anthropic` — Openzuma uses MiniMax's Anthropic Messages-compatible endpoint) |
...
| `OPENZUMA_GEMINI_CLIENT_ID` | OAuth client ID for `google-gemini-cli` PKCE login (optional; defaults to Google's public gemini-cli client) |
| `OPENZUMA_GEMINI_CLIENT_SECRET` | OAuth client secret for `google-gemini-cli` (optional) |
| `OPENZUMA_GEMINI_PROJECT_ID` | GCP project ID for paid Gemini tiers (free tier auto-provisions) |
...
| `OPENZUMA_QWEN_BASE_URL` | Qwen Portal base URL override (default: `https://portal.qwen.ai/v1`) |
...
| `OPENZUMA_MODEL` | Override model name at process level (used by cron scheduler; prefer `config.yaml` for normal use) |
| `VOICE_TOOLS_OPENAI_KEY` | Preferred OpenAI key for OpenAI speech-to-text and text-to-speech providers |
| `OPENZUMA_LOCAL_STT_COMMAND` | Optional local speech-to-text command template. Supports `{input_path}`, `{output_dir}`, `{language}`, and `{model}` placeholders |
| `OPENZUMA_LOCAL_STT_LANGUAGE` | Default language passed to `OPENZUMA_LOCAL_STT_COMMAND` or auto-detected local `whisper` CLI fallback (default: `en`) |
| `OPENZUMA_HOME` | Override Openzuma config directory (default: `~/.openzuma`). Also scopes the gateway PID file and systemd service name, so multiple installations can run concurrently |

## Provider Auth (OAuth)

For native Anthropic auth, Openzuma prefers Claude Code's own credential files when they exist because those credentials can refresh automatically. Environment variables such as `ANTHROPIC_TOKEN` remain useful as manual overrides, but they are no longer the preferred path for Claude Pro/Max login.

| Variable | Description |
|----------|-------------|
| `OPENZUMA_INFERENCE_PROVIDER` | Override provider selection: `auto`, `openrouter`, `nous`, `openai-codex`, `copilot`, `copilot-acp`, `anthropic`, `huggingface`, `zai`, `kimi-coding`, `kimi-coding-cn`, `minimax`, `minimax-cn`, `kilocode`, `xiaomi`, `arcee`, `alibaba`, `deepseek`, `nvidia`, `ollama-cloud`, `xai` (alias `grok`), `google-gemini-cli`, `qwen-oauth`, `bedrock`, `opencode-zen`, `opencode-go`, `ai-gateway` (default: `auto`) |
| `OPENZUMA_PORTAL_BASE_URL` | Override Nous Portal URL (for development/testing) |
| `NOUS_INFERENCE_BASE_URL` | Override Nous inference API URL |
| `OPENZUMA_NOUS_MIN_KEY_TTL_SECONDS` | Min agent key TTL before re-mint (default: 1800 = 30min) |
| `OPENZUMA_NOUS_TIMEOUT_SECONDS` | HTTP timeout for Nous credential / token flows |
| `OPENZUMA_DUMP_REQUESTS` | Dump API request payloads to log files (`true`/`false`) |
| `OPENZUMA_PREFILL_MESSAGES_FILE` | Path to a JSON file of ephemeral prefill messages injected at API-call time |
| `OPENZUMA_TIMEZONE` | IANA timezone override (for example `America/New_York`) |
...
These variables configure the [Tool Gateway](/docs/user-guide/features/tool-gateway) for paid Nous subscribers or self-hosted gateway deployments. Most users don't need to set these — the gateway is configured automatically via `openzuma model` or `openzuma tools`.
...
For cloud sandbox backends, persistence is filesystem-oriented. `TERMINAL_LIFETIME_SECONDS` controls when Openzuma cleans up an idle terminal session, and later resumes may recreate the sandbox rather than keep the same live processes running.
...
| `TERMINAL_SANDBOX_DIR` | Host directory for workspaces and overlays (default: `~/.openzuma/sandboxes/`) |
...
| `WECOM_CALLBACK_PORT` | Callback server port (default: `8745`) |
...
| `BLUEBUBBLES_WEBHOOK_PORT` | Webhook listener port (default: `8745`) |
...
| `MATRIX_USER_ID` | Matrix user ID (e.g. `@openzuma:matrix.org`) — required for password login, optional with access token |
...
| `MATRIX_DEVICE_ID` | Stable Matrix device ID for E2EE persistence across restarts (e.g. `OPENZUMA_BOT`). Without this, E2EE keys rotate every startup and historic-room decrypt breaks. |
...
| `WEBHOOK_PORT` | HTTP server port for receiving webhooks (default: `8744`) |
...
| `API_SERVER_PORT` | Port for the API server (default: `8742`) |
...
| `API_SERVER_MODEL_NAME` | Model name advertised on `/v1/models`. Defaults to the profile name (or `openzuma-agent` for the default profile). Useful for multi-user setups where frontends like Open WebUI need distinct model names per connection. |
| `GATEWAY_PROXY_URL` | URL of a remote Openzuma API server to forward messages to ([proxy mode](/docs/user-guide/messaging/matrix#proxy-mode-e2ee-on-macos)). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Also configurable via `gateway.proxy_url` in `config.yaml`. |
...
| Variable | Description |
|----------|-------------|
| `OPENZUMA_MAX_ITERATIONS` | Max tool-calling iterations per conversation (default: 90) |
| `OPENZUMA_TOOL_PROGRESS` | Deprecated compatibility variable for tool progress display. Prefer `display.tool_progress` in `config.yaml`. |
| `OPENZUMA_TOOL_PROGRESS_MODE` | Deprecated compatibility variable for tool progress mode. Prefer `display.tool_progress` in `config.yaml`. |
| `OPENZUMA_HUMAN_DELAY_MODE` | Response pacing: `off`/`natural`/`custom` |
| `OPENZUMA_HUMAN_DELAY_MIN_MS` | Custom delay range minimum (ms) |
| `OPENZUMA_HUMAN_DELAY_MAX_MS` | Custom delay range maximum (ms) |
| `OPENZUMA_QUIET` | Suppress non-essential output (`true`/`false`) |
| `OPENZUMA_API_TIMEOUT` | LLM API call timeout in seconds (default: `1800`) |
| `OPENZUMA_API_CALL_STALE_TIMEOUT` | Non-streaming stale-call timeout in seconds (default: `300`). Auto-disabled for local providers when left unset. Also configurable via `providers.<id>.stale_timeout_seconds` or `providers.<id>.models.<model>.stale_timeout_seconds` in `config.yaml`. |
| `OPENZUMA_STREAM_READ_TIMEOUT` | Streaming socket read timeout in seconds (default: `120`). Auto-increased to `OPENZUMA_API_TIMEOUT` for local providers. Increase if local LLMs time out during long code generation. |
| `OPENZUMA_STREAM_STALE_TIMEOUT` | Stale stream detection timeout in seconds (default: `180`). Auto-disabled for local providers. Triggers connection kill if no chunks arrive within this window. |
| `OPENZUMA_EXEC_ASK` | Enable execution approval prompts in gateway mode (`true`/`false`) |
| `OPENZUMA_ENABLE_PROJECT_PLUGINS` | Enable auto-discovery of repo-local plugins from `./.openzuma/plugins/` (`true`/`false`, default: `false`) |
| `OPENZUMA_BACKGROUND_NOTIFICATIONS` | Background process notification mode in gateway: `all` (default), `result`, `error`, `off` |
| `OPENZUMA_EPHEMERAL_SYSTEM_PROMPT` | Ephemeral system prompt injected at API-call time (never persisted to sessions) |
...
| Variable | Description |
|----------|-------------|
| `OPENZUMA_TUI` | Launch the [TUI](../user-guide/tui.md) instead of the classic CLI when set to `1`. Equivalent to passing `--tui`. |
| `OPENZUMA_TUI_DIR` | Path to a prebuilt `ui-tui/` directory (must contain `dist/entry.js` and populated `node_modules`). Used by distros and Nix to skip the first-launch `npm install`. |

## Cron Scheduler

| Variable | Description |
|----------|-------------|
| `OPENZUMA_CRON_TIMEOUT` | Inactivity timeout for cron job agent runs in seconds (default: `600`). The agent can run indefinitely while actively calling tools or receiving stream tokens — this only triggers when idle. Set to `0` for unlimited. |
| `OPENZUMA_CRON_SCRIPT_TIMEOUT` | Timeout for pre-run scripts attached to cron jobs in seconds (default: `120`). Override for scripts that need longer execution (e.g., randomized delays for anti-bot timing). Also configurable via `cron.script_timeout_seconds` in `config.yaml`. |
...
For task-specific direct endpoints, Openzuma uses the task's configured API key or `OPENAI_API_KEY`. It does not reuse `OPENROUTER_API_KEY` for those custom endpoints.
...
These go in `~/.openzuma/config.yaml` under the `provider_routing` section:
...
:::tip
Use `openzuma config set` to set environment variables — it automatically saves them to the right file (`.env` for secrets, `config.yaml` for everything else).
:::
