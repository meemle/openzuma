...
description: "Catalog of bundled skills that ship with Openzuma Agent"
---

# Bundled Skills Catalog

Openzuma ships with a large built-in skill library copied into `~/.openzuma/skills/` on install. This page catalogs the bundled skills that live in the repository under `skills/`.
...
| `openzuma-agent` | Complete guide to using and extending Openzuma Agent — CLI usage, setup, configuration, spawning additional agents, gateway platforms, skills, voice, tools, profiles, and a concise contributor reference. Load this skill when helping users configure Openzuma, troubleshoot issues, s… | `autonomous-ai-agents/openzuma-agent` |
...
Internal dogfooding and QA skills used to test Openzuma Agent itself.
...
| Skill | Description | Path |
|-------|-------------|------|
| `native-mcp` | Built-in MCP (Model Context Protocol) client that connects to external MCP servers, discovers their tools, and registers them as native Openzuma Agent tools. Supports stdio and HTTP transports with automatic reconnection, security filtering, and zero-config tool injection. | `mcp/native-mcp` |
...
| Skill | Description | Path |
|-------|-------------|------|
| `google-workspace` | Gmail, Calendar, Drive, Contacts, Sheets, and Docs integration for Openzuma. Uses Openzuma-managed OAuth2 setup, prefers the Google Workspace CLI (`gws`) when available for broader API coverage, and falls back to the Python client libraries otherwise. | `productivity/google-workspace` |
...
| Skill | Description | Path |
|-------|-------------|------|
| `godmode` | Jailbreak API-served LLMs using G0DM0D3 techniques — Parseltongue input obfuscation (33 techniques), GODMODE CLASSIC system prompt templates, ULTRAPLINIAN multi-model racing, encoding escalation, and Openzuma-native prefill/system prompt integration. Use when a user wants to byp… | `red-teaming/godmode` |
...
| Skill | Description | Path |
|-------|-------------|------|
| `plan` | Plan mode for Openzuma — inspect context, write a markdown plan into the active workspace's `.openzuma/plans/` directory, and do not execute the work. | `software-development/plan` |
...
```bash
openzuma skills install official/<category>/<skill>
```
...
| Skill | Description | Path |
|-------|-------------|------|
| `blender-mcp` | Control Blender directly from Openzuma via socket connection to the blender-mcp addon. Create 3D objects, materials, animations, and run arbitrary Blender Python (bpy) code. | `creative/blender-mcp` |
...
| Skill | Description | Path |
|-------|-------------|------|
| `agentmail` | Give the agent its own dedicated email inbox via AgentMail. Send, receive, and manage email autonomously using agent-owned email addresses (e.g. openzuma-agent@agentmail.to). | `email/agentmail` |
...
| Skill | Description | Path |
|-------|-------------|------|
| `openclaw-migration` | Migrate a user's OpenClaw customization footprint into Openzuma Agent. Imports Openzuma-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports what could not be migrated and why. | `migration/openclaw-migration` |

## productivity

| Skill | Description | Path |
|-------|-------------|------|
| `telephony` | Give Openzuma phone capabilities — provision and persist a Twilio number, send and receive SMS/MMS, make direct calls, and place AI-driven outbound calls through Bland.ai or Vapi. | `productivity/telephony` |
