...
sidebar_position: 2
title: "TUI"
description: "Launch the modern terminal UI for Openzuma — mouse-friendly, rich overlays, and non-blocking input."
---

# TUI

The TUI is the modern front-end for Openzuma — a terminal UI backed by the same Python runtime as the [Classic CLI](cli.md). Same agent, same sessions, same slash commands; a cleaner, more responsive surface for interacting with them.

It's the recommended way to run Openzuma interactively.
...
openzuma --tui -c
openzuma --tui --continue

# Resume a specific session by ID or title
openzuma --tui -r 20260409_000000_aa11bb
openzuma --tui --resume "my t0p session"
...
```bash
export OPENZUMA_TUI=1
openzuma          # now uses the TUI
openzuma chat     # same
```
...
- **Instant first frame** — the banner paints before the app finishes loading, so the terminal never feels frozen while Openzuma is starting.
...
- **Node.js** ≥ 20 — the TUI runs as a subprocess launched from the Python CLI. `openzuma doctor` verifies this.
- **TTY** — like the classic CLI, piping stdin or running in non-interactive environments falls back to single-query mode.

On first launch Openzuma installs the TUI's Node dependencies into `ui-tui/node_modules` (one-time, a few seconds). Subsequent launches are fast. If you pull a new Openzuma version, the TUI bundle is rebuilt automatically when sources are newer than the dist.

### External prebuild

Distributions that ship a prebuilt bundle (Nix, system packages) can point Openzuma at it:

```bash
export OPENZUMA_TUI_DIR=/path/to/prebuilt/ui-tui
openzuma --tui
```
...
The TUI respects all standard Openzuma config: `~/.openzuma/config.yaml`, profiles, personalities, skins, quick commands, credential pools, memory providers, tool/skill enablement. No TUI-specific config file exists.
...
Sessions are shared between the TUI and the classic CLI — both write to the same `~/.openzuma/state.db`. You can start a session in one, resume in the other. The session picker surfaces sessions from both sources, with a source tag.

See [Sessions](sessions.md) for lifecycle, search, compression, and export.

## Reverting to the classic CLI

Launching `openzuma` (without `--tui`) stays on the classic CLI. To make a machine prefer the TUI, set `OPENZUMA_TUI=1` in your shell profile. To go back, unset it.

If the TUI fails to launch (no Node, missing bundle, TTY issue), Openzuma prints a diagnostic and falls back — rather than leaving you stuck.
