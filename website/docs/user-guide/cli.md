...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Openzuma Agent terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

Openzuma Agent's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.

:::tip
Openzuma also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `openzuma --tui` — see the [TUI](tui.md) guide.
:::
...
openzuma chat --model "anthropic/claude-sonnet-4"

# With a specific provider
openzuma chat --provider nous        # Use Nous Portal
openzuma chat --provider openrouter  # Force OpenRouter
...
openzuma chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
openzuma --continue             # Resume the most recent CLI session (-c)
openzuma --resume <session_id>  # Resume a specific session by ID (-r)

# Verbose mode (debug output)
openzuma chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
openzuma -w                         # Interactive mode in worktree
openzuma -w -q "Fix issue #123"     # Single query in worktree
```

## Interface Layout

<img className="docs-terminal-figure" src="/img/docs/cli-layout.svg" alt="Stylized preview of the Openzuma CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Openzuma CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
When resuming a previous session (`openzuma -c` or `openzuma --resume <id>`), a "Previous Conversation" panel appears between the banner and the input prompt, showing a compact recap of the conversation history. See [Sessions — Conversation Recap on Resume](sessions.md#conversation-recap-on-resume) for details and configuration.
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Openzuma to background (Unix only). Run `fg` in the shell to resume. |
| `Tab` | Accept auto-suggestion (ghost text) or autocomplete slash commands |

## Slash Commands

Type `/` to see the autocomplete dropdown. Openzuma supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Openzuma replies |
...
```bash
openzuma -s openzuma-agent-dev,github-auth
openzuma chat -s github-pr-workflow -s github-auth
```

Openzuma loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.openzuma/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.openzuma/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain Openzuma..."
  # Add your own!
```
...
On Unix systems, press **`Ctrl+Z`** to suspend Openzuma to the background — just like any terminal process. The shell prints a confirmation:

```
Openzuma Agent has been suspended. Run `fg` to bring Openzuma Agent back.
```
...
Resume this session with:
  openzuma --resume 20260225_143052_a1b2c3
...
```bash
openzuma --continue                          # Resume the most recent CLI session
openzuma -c                                  # Short form
openzuma -c "my project"                     # Resume a named session (latest in lineage)
openzuma --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
openzuma --resume "refactoring auth"         # Resume by title
openzuma -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `openzuma sessions rename <id> <title>` from the command line. Use `openzuma sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in Openzuma's SQLite state database under `~/.openzuma/state.db`. The database keeps:
...
Openzuma immediately confirms the task and gives you back the prompt:
