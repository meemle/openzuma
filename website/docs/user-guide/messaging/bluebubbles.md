...
Connect Openzuma to Apple iMessage via [BlueBubbles](https://bluebubbles.app/) — a free, open-source macOS server that bridges iMessage to any device.
...
- Network connectivity between Openzuma and the BlueBubbles server
...
Or set environment variables directly in `~/.openzuma/.env`:
...
When someone messages your iMessage, Openzuma automatically sends them a pairing code. Approve it with:
```bash
openzuma pairing approve bluebubbles <CODE>
```
Use `openzuma pairing list` to see pending codes and approved users.

**Pre-authorize specific users** (in `~/.openzuma/.env`):
```bash
BLUEBUBBLES_ALLOWED_USERS=user@icloud.com,+15551234567
```

**Open access** (in `~/.openzuma/.env`):
```bash
...
Openzuma will connect to your BlueBubbles server, register a webhook, and start listening for iMessage messages.

## How It Works

```
iMessage → Messages.app → BlueBubbles Server → Webhook → Openzuma
Openzuma → BlueBubbles REST API → Messages.app → iMessage
```

- **Inbound:** BlueBubbles sends webhook events to a local listener when new messages arrive. No polling — instant delivery.
- **Outbound:** Openzuma sends messages via the BlueBubbles REST API.
...
| `BLUEBUBBLES_WEBHOOK_PORT` | No | `8745` | Webhook listener port |
...
### Chat Addressing
You can address chats by email or phone number — Openzuma resolves them to BlueBubbles chat GUIDs automatically. No need to use raw GUID format.
...
- Check `openzuma logs gateway` for webhook errors (or `openzuma logs -f` to follow in real-time)
