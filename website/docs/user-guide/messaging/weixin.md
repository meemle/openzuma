...
sidebar_position: 15
title: "Weixin (WeChat)"
description: "Connect Openzuma Agent to personal WeChat accounts via the iLink Bot API"
---

# Weixin (WeChat)

Connect Openzuma to [WeChat](https://weixin.qq.com/) (微信), Tencent's personal messaging platform. The adapter uses Tencent's **iLink Bot API** for personal WeChat accounts — this is distinct from WeCom (Enterprise WeChat). Messages are delivered via long-polling, so no public endpoint or webhook is required.
...
- Terminal QR rendering is included when Openzuma is installed with the `messaging` extra
...
5. Save the account credentials automatically to `~/.openzuma/weixin/accounts/`
...
After initial QR login, set at minimum the account ID in `~/.openzuma/.env`:
...
- **QR code login** — scan-to-connect setup via `openzuma gateway setup`
...
- Tokens are saved per account+peer to `~/.openzuma/weixin/accounts/<account_id>.context-tokens.json`
...
| `Weixin startup failed: WEIXIN_TOKEN is required` | Run `openzuma gateway setup` to complete QR login, or set `WEIXIN_TOKEN` manually |
| `Weixin startup failed: WEIXIN_ACCOUNT_ID is required` | Set `WEIXIN_ACCOUNT_ID` in your `.env` or run `openzuma gateway setup` |
| `Another local Openzuma gateway is already using this Weixin token` | Stop the other gateway instance first — only one poller per token is allowed |
| Session expired (`errcode=-14`) | Your login session has expired. Re-run `openzuma gateway setup` to scan a new QR code |
...
| Terminal QR code doesn't render | Reinstall with the messaging extra: `pip install openzuma-agent[messaging]`. Alternatively, open the URL printed above the QR |
