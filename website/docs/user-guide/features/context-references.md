...
Type `@` followed by a reference to inject content directly into your message. Openzuma expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Openzuma env: `$OPENZUMA_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$OPENZUMA_HOME/skills/.hub/`
