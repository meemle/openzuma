...
Schedule tasks to run automatically with natural language or cron expressions. Openzuma exposes cron management through a single `cronjob` tool with action-style operations instead of separate schedule/list/remove tools.
...
:::warning
Cron-run sessions cannot recursively create more cron jobs. Openzuma disables cron management tools inside cron executions to prevent runaway scheduling loops.
:::
...
```bash
openzuma cron create "every 2h" "Check server status"
openzuma cron create "every 1h" "Summarize new feed items" --skill blogwatcher
openzuma cron create "every 1h" "Use both skills and combine the result" \
  --skill blogwatcher \
  --skill maps \
  --name "Skill combo"
```
...
Ask Openzuma normally:

```text
Every morning at 9am, check Hacker News for AI news and send me a summary on Telegram.
```

Openzuma will use the unified `cronjob` tool internally.
...
```bash
openzuma cron edit <job_id> --schedule "every 4h"
openzuma cron edit <job_id> --prompt "Use the revised task"
openzuma cron edit <job_id> --skill blogwatcher --skill maps
openzuma cron edit <job_id> --add-skill maps
openzuma cron edit <job_id> --remove-skill blogwatcher
openzuma cron edit <job_id> --clear-skills
```
...
```bash
openzuma cron list
openzuma cron pause <job_id>
openzuma cron resume <job_id>
openzuma cron run <job_id>
openzuma cron remove <job_id>
openzuma cron status
openzuma cron tick
```
...
sudo openzuma gateway install --system   # Linux: boot-time system service for servers
openzuma gateway             # Or run in foreground
...
On each tick Openzuma:

1. loads jobs from `~/.openzuma/cron/jobs.json`
...
A file lock at `~/.openzuma/cron/.tick.lock` prevents overlapping scheduler ticks from double-running the same job batch.
...
| `"local"` | Save to local files only (`~/.openzuma/cron/output/`) | Default on CLI |
...
If the agent's final response starts with `[SILENT]`, delivery is suppressed entirely. The output is still saved locally for audit (in `~/.openzuma/cron/output/`), but no message is sent to the delivery target.
...
Or set the `OPENZUMA_CRON_SCRIPT_TIMEOUT` environment variable. The resolution order is: env var → config.yaml → 120s default.
...
The agent's final response is automatically delivered — you do **not** need to include `send_message` in the cron prompt for that same destination. If a cron run calls `send_message` to the exact target the scheduler will already deliver to, Openzuma skips that duplicate send and tells the model to put the user-facing content in the final response instead. Use `send_message` only for additional or different targets.
...
Jobs are stored in `~/.openzuma/cron/jobs.json`. Output from job runs is saved to `~/.openzuma/cron/output/{job_id}/{timestamp}.md`.
