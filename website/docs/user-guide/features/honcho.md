...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Openzuma's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Openzuma instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
openzuma memory setup    # select "honcho" from the provider list
```
...
Honcho is configured in `~/.honcho/config.json` (global) or `$OPENZUMA_HOME/honcho.json` (profile-local). The setup wizard handles this for you.
...
- `per-session` — each `openzuma` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Openzuma syncs them back at session init.
...
```bash
openzuma honcho status          # Connection status, config, and key settings
openzuma honcho setup           # Interactive setup wizard
openzuma honcho strategy        # Show or set session strategy
openzuma honcho peer            # Update peer names for multi-agent setups
openzuma honcho mode            # Show or set recall mode
openzuma honcho tokens          # Show or set context token budget
openzuma honcho identity        # Show Honcho peer identity
openzuma honcho sync            # Sync host blocks for all profiles
openzuma honcho enable          # Enable Honcho
openzuma honcho disable         # Disable Honcho
```

## Migrating from `openzuma honcho`

If you previously used the standalone `openzuma honcho setup`:
...
No re-login or re-setup needed. Run `openzuma memory setup` and select "honcho" — the wizard detects your existing config.
