...
Openzuma has three hook systems that run custom code at key lifecycle points:

| System | Registered via | Runs in | Use case |
|--------|---------------|---------|----------|
| **[Gateway hooks](#gateway-event-hooks)** | `HOOK.yaml` + `handler.py` in `~/.openzuma/hooks/` | Gateway only | Logging, alerts, webhooks |
...
| **[Shell hooks](#shell-hooks)** | `hooks:` block in `~/.openzuma/config.yaml` pointing at shell scripts | CLI + Gateway | Drop-in scripts for blocking, auto-formatting, context injection |
...
Each hook is a directory under `~/.openzuma/hooks/` containing two files:
...
LOG_FILE = Path.home() / ".openzuma" / "hooks" / "my-hook" / "activity.log"
...
The gateway ships with a built-in `boot-md` hook that looks for `~/.openzuma/BOOT.md` on every startup. If the file exists, the agent runs its instructions in a background session. No installation needed — just create the file.

**Create `~/.openzuma/BOOT.md`:**

```markdown
# Startup Checklist

1. Check if any cron jobs failed overnight — run `openzuma cron list`
...
```yaml
# ~/.openzuma/hooks/long-task-alert/HOOK.yaml
...
```python
# ~/.openzuma/hooks/long-task-alert/handler.py
import os
import httpx
...
```yaml
# ~/.openzuma/hooks/command-logger/HOOK.yaml
name: command-logger
...
```python
# ~/.openzuma/hooks/command-logger/handler.py
import json
from datetime import datetime
from pathlib import Path

LOG = Path.home() / ".openzuma" / "logs" / "command_usage.jsonl"
...
```yaml
# ~/.openzuma/hooks/session-webhook/HOOK.yaml
...
```python
# ~/.openzuma/hooks/session-webhook/handler.py
import httpx

WEBHOOK_URL = "https://your-service.example.com/openzuma-events"
...
1. On gateway startup, `HookRegistry.discover_and_load()` scans `~/.openzuma/hooks/`
...
**Where context is injected:** Always the **user message**, never the system prompt. This preserves the prompt cache — the system prompt stays identical across turns, so cached tokens are reused. The system prompt is Openzuma's territory (model guidance, tool enforcement, personality, skills). Plugins contribute context alongside the user's input.
...
See the **[Build a Plugin guide](/docs/guides/build-a-openzuma-plugin)** for the full walkthrough including tool schemas, handlers, and advanced hook patterns.
...
Declare shell-script hooks in your `cli-config.yaml` and Openzuma will run them as subprocesses whenever the corresponding plugin-hook event fires — in both CLI and gateway sessions. No Python plugin authoring required.
...
Shell hooks are registered by calling `agent.shell_hooks.register_from_config(cfg)` at both CLI startup (`openzuma_cli/main.py`) and gateway startup (`gateway/run.py`). They compose naturally with Python plugin hooks — both flow through the same dispatcher.

### Comparison at a glance

| Dimension | Shell hooks | [Plugin hooks](#plugin-hooks) | [Gateway hooks](#gateway-event-hooks) |
|-----------|-------------|-------------------------------|---------------------------------------|
| Declared in | `hooks:` block in `~/.openzuma/config.yaml` | `register()` in a `plugin.yaml` plugin | `HOOK.yaml` + `handler.py` directory |
| Lives under | `~/.openzuma/agent-hooks/` (by convention) | `~/.openzuma/plugins/<name>/` | `~/.openzuma/hooks/<name>/` |
...
Each time the event fires, Openzuma spawns a subprocess for every matching hook (matcher permitting), pipes a JSON payload to **stdin**, and reads **stdout** back as JSON.
...
{"action":   "block", "message": "Forbidden: rm -rf"}   // Openzuma-canonical
...
hooks:
  post_tool_call:
    - matcher: "write_file|patch"
      command: "~/.openzuma/agent-hooks/auto-format.sh"
```

```bash
#!/usr/bin/env bash
# ~/.openzuma/agent-hooks/auto-format.sh
payload="$(cat -)"
...
hooks:
  pre_tool_call:
    - matcher: "terminal"
      command: "~/.openzuma/agent-hooks/block-rm-rf.sh"
      timeout: 5
```

```bash
#!/usr/bin/env bash
# ~/.openzuma/agent-hooks/block-rm-rf.sh
payload="$(cat -)"
...
hooks:
  pre_llm_call:
    - command: "~/.openzuma/agent-hooks/inject-cwd-context.sh"
```

```bash
#!/usr/bin/env bash
# ~/.openzuma/agent-hooks/inject-cwd-context.sh
...
Claude Code's `UserPromptSubmit` event is intentionally not a separate Openzuma event — `pre_llm_call` fires at the same place and already supports context injection. Use it here.
...
hooks:
  subagent_stop:
    - command: "~/.openzuma/agent-hooks/log-orchestration.sh"
```

```bash
#!/usr/bin/env bash
# ~/.openzuma/agent-hooks/log-orchestration.sh
log=~/.openzuma/logs/orchestration.log
...
Each unique `(event, command)` pair prompts the user for approval the first time Openzuma sees it, then persists the decision to `~/.openzuma/shell-hooks-allowlist.json`. Subsequent runs (CLI or gateway) skip the prompt.

Three escape hatches bypass the interactive prompt — any one is sufficient:

1. `--accept-hooks` flag on the CLI (e.g. `openzuma --accept-hooks chat`)
2. `OPENZUMA_ACCEPT_HOOKS=1` environment variable
...
**Script edits are silently trusted.** The allowlist keys on the exact command string, not the script's hash, so editing the script on disk does not invalidate consent. `openzuma hooks doctor` flags mtime drift so you can spot edits and decide whether to re-approve.

### The `openzuma hooks` CLI

| Command | What it does |
|---------|--------------|
| `openzuma hooks list` | Dump configured hooks with matcher, timeout, and consent status |
| `openzuma hooks test <event> [--for-tool X] [--payload-file F]` | Fire every matching hook against a synthetic payload and print the parsed response |
| `openzuma hooks revoke <command>` | Remove every allowlist entry matching `<command>` (takes effect on next restart) |
| `openzuma hooks doctor` | For every configured hook: check exec bit, allowlist status, mtime drift, JSON output validity, and rough execution time |
...
- Keep scripts inside `~/.openzuma/agent-hooks/` so the path is easy to audit.
- Re-run `openzuma hooks doctor` after you pull a shared config to spot newly-added hooks before they register.
