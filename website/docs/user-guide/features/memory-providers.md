...
Openzuma Agent ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
openzuma memory setup      # interactive picker + configuration
openzuma memory status     # check what's active
openzuma memory off        # disable external provider
```

You can also select the active memory provider via `openzuma plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.openzuma/config.yaml`:
...
When a memory provider is active, Openzuma automatically:
...
**Setup Wizard:**
```bash
openzuma honcho setup        # (legacy command) 
# or
openzuma memory setup        # select "honcho"
```

**Config:** `$OPENZUMA_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$OPENZUMA_HOME/honcho.json` > `~/.openzuma/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/openzuma-ai/openzuma-agent/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/openzuma).
...
:::tip Migrating from `openzuma honcho`
If you previously used `openzuma honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per Openzuma profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All Openzuma profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per Openzuma profile. Host key `openzuma` → default; `openzuma.<profile>` for others. |
...
```bash
openzuma profile create coder --clone
```

`--clone` creates a `openzuma.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.

### Existing profiles, backfill Honcho peers

```bash
openzuma honcho sync
```

Scans every Openzuma profile, creates host blocks for any profile without one, inherits settings from the default `openzuma` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
See the [config reference](https://github.com/openzuma-ai/openzuma-agent/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/openzuma).
...
openzuma memory setup    # select "openviking"
# Or manually:
openzuma config set memory.provider openviking
echo "OPENVIKING_ENDPOINT=http://localhost:1933" >> ~/.openzuma/.env
```
...
**Setup:**
```bash
openzuma memory setup    # select "mem0"
# Or manually:
openzuma config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.openzuma/.env
```

**Config:** `$OPENZUMA_HOME/mem0.json`

| Key | Default | Description |
|-----|---------|-------------|
| `user_id` | `openzuma-user` | User identifier |
| `agent_id` | `openzuma` | Agent identifier |
...
**Setup:**
```bash
openzuma memory setup    # select "hindsight"
# Or manually:
openzuma config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.openzuma/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p openzuma ui start`

**Config:** `$OPENZUMA_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `openzuma` | Memory bank identifier |
| `recall_budget` | `mid` | Recall thoroughness: `low` / `mid` / `high` |
| `memory_mode` | `hybrid` | `hybrid` (context + tools), `context` (auto-inject only), `tools` (tools only) |
| `auto_retain` | `true` | Automatically retain conversation turns |
| `auto_recall` | `true` | Automatically recall memories before each turn |
| `retain_async` | `true` | Process retain asynchronously on the server |
| `retain_context` | `conversation between Openzuma Agent and the User` | Context label for retained memories |
...
See [plugin README](https://github.com/NousResearch/openzuma-agent/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Setup:**
```bash
openzuma memory setup    # select "holographic"
# Or manually:
openzuma config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.openzuma-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$OPENZUMA_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
openzuma memory setup    # select "retaindb"
# Or manually:
openzuma config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.openzuma/.env
```
...
# Then configure Openzuma
openzuma memory setup    # select "byterover"
# Or manually:
openzuma config set memory.provider byterover
```

**Key features:**
- Automatic pre-compression extraction (saves insights before context compression discards them)
- Knowledge tree stored at `$OPENZUMA_HOME/byterover/` (profile-scoped)
...
**Setup:**
```bash
openzuma memory setup    # select "supermemory"
# Or manually:
openzuma config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.openzuma/.env
```

**Config:** `$OPENZUMA_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `openzuma` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `openzuma-{identity}` → `openzuma-coder`) to isolate memories per Openzuma profile
...
- **Local storage providers** (Holographic, ByteRover) use `$OPENZUMA_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$OPENZUMA_HOME/` so each profile has its own credentials
