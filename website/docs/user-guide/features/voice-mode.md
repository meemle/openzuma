...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with Openzuma Agent — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

Openzuma Agent supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with Openzuma](/docs/guides/use-voice-mode-with-openzuma).

## Prerequisites

Before using voice features, make sure you have:

1. **Openzuma Agent installed** — `pip install openzuma-agent` (see [Installation](/docs/getting-started/installation))
2. **An LLM provider configured** — run `openzuma model` or set your preferred provider credentials in `~/.openzuma/.env`
3. **A working base setup** — run `openzuma` to verify the agent responds to text before enabling voice

:::tip
The `~/.openzuma/` directory and default `config.yaml` are created automatically the first time you run `openzuma`. You only need to create `~/.openzuma/.env` manually for API keys.
:::
...
pip install "openzuma-agent[messaging]"

# Premium TTS (ElevenLabs)
pip install "openzuma-agent[tts-premium]"

# Local TTS (NeuTTS, optional)
python -m pip install -U neutts[all]

# Everything at once
pip install "openzuma-agent[all]"
```
...
```bash
openzuma                # Start the interactive CLI
```
...
1. Start the CLI with `openzuma` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.openzuma/config.yaml` (default: `ctrl+b`).
:::
...
```bash
openzuma gateway        # Start the gateway (connects to configured platforms)
openzuma gateway setup  # Interactive setup wizard for first-time configuration
```
...
**Server channels:** The bot only responds when you @mention it (e.g. `@openzumabyt4 hello`). Make sure you select the **bot user** from the mention popup, not the role with the same name.

:::tip
To disable the mention requirement in server channels, add to `~/.openzuma/.env`:
```bash
...
```bash
openzuma gateway        # Start with existing configuration
```
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.openzuma/.env`
...
- Review gateway logs: `tail -f ~/.openzuma/logs/gateway.log`
