...
Openzuma Agent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.openzuma/state.db`) — structured session metadata with FTS5 full-text search
2. **JSONL transcripts** (`~/.openzuma/sessions/`) — raw conversation transcripts including tool calls (gateway)
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`openzuma` or `openzuma chat`) |
...
openzuma -c "my project"

# If there are lineage variants (my project, my project #2, my project #3),
# this automatically resumes the most recent one
openzuma -c "my project"   # → resumes "my project #3"
```
...
openzuma --resume 20250305_091523_a1b2c3d4
openzuma -r 20250305_091523_a1b2c3d4

# Resume by title
openzuma --resume "refactoring auth"

# Or with the chat subcommand
openzuma chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `openzuma sessions list`.

### Conversation Recap on Resume

When you resume a session, Openzuma displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src="/img/docs/session-recap.svg" alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Openzuma session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.openzuma/config.yaml`:
...
Openzuma automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `openzuma sessions list` or `openzuma sessions browse`.
...
```bash
openzuma sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Openzuma creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`openzuma -c "my project"`), it automatically picks the most recent session in the lineage.
...
Openzuma provides a full set of session management commands via `openzuma sessions`:
...
# Filter by platform
openzuma sessions list --source telegram

# Show more sessions
openzuma sessions list --limit 50
```
...
openzuma sessions export backup.jsonl

# Export sessions from a specific platform
openzuma sessions export telegram-history.jsonl --source telegram

# Export a single session
openzuma sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4
```
...
openzuma sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
openzuma sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
openzuma sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
openzuma sessions prune

# Custom age threshold
openzuma sessions prune --older-than 30

# Only prune sessions from a specific platform
openzuma sessions prune --source telegram --older-than 60

# Skip confirmation
openzuma sessions prune --older-than 30 --yes
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`openzuma insights`](/docs/reference/cli-commands#openzuma-insights).
...
When Openzuma cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Openzuma uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Openzuma in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.openzuma/state.db` | All session metadata + messages with FTS5 |
| Gateway transcripts | `~/.openzuma/sessions/` | JSONL transcripts per session + sessions.json index |
| Gateway index | `~/.openzuma/sessions/sessions.json` | Maps session keys to active session IDs |
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every Openzuma process in the same `OPENZUMA_HOME`

Default is **off** — session history is valuable for `session_search` recall, and silently deleting it could surprise users. Enable in `~/.openzuma/config.yaml`:
...
openzuma sessions delete <session_id>
