...
title: "Git Worktrees"
description: "Run multiple Openzuma agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Openzuma Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Openzuma so each session has a clean, isolated working directory.

## Why Use Worktrees with Openzuma?

Openzuma treats the **current working directory** as the project root:

- CLI: the directory where you run `openzuma` or `openzuma chat`
...
git worktree add ../repo-feature feature/openzuma-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/openzuma-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Openzuma there:

```bash
cd ../repo-feature

# Start Openzuma in the worktree
openzuma
```
...
git worktree add ../repo-experiment-a feature/openzuma-a
git worktree add ../repo-experiment-b feature/openzuma-b
```
...
Each Openzuma process:

- Works on its own branch (`feature/openzuma-a` vs `feature/openzuma-b`).
...
- Openzuma checkpoint data under `~/.openzuma/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Openzuma experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/openzuma-checkpoints-docs`, `feature/openzuma-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Openzuma from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `openzuma -w` (Automatic Worktree Mode)

Openzuma has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `openzuma/openzuma-<hash>`).
...
```bash
openzuma -w -q "Fix issue #123"
```

For parallel agents, open multiple terminals and run `openzuma -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Openzuma session its own clean checkout.
