...
sidebar_position: 2
title: "Configuration"
description: "Configure Openzuma Agent — config.yaml, providers, models, API keys, and more"
---

# Configuration

All settings are stored in the `~/.openzuma/` directory for easy access.
...
```bash
openzuma config              # View current configuration
openzuma config edit         # Open config.yaml in your editor
openzuma config set KEY VAL  # Set a specific value
openzuma config check        # Check for missing options (after updates)
openzuma config migrate      # Interactively add missing options

# Examples:
openzuma config set model anthropic/claude-opus-4
openzuma config set terminal.backend docker
openzuma config set OPENROUTER_API_KEY sk-or-...  # Saves to .env
```

:::tip
The `openzuma config set` command automatically routes values to the right file — API keys are saved to `.env`, everything else to `config.yaml`.
:::
...
1. **CLI arguments** — e.g., `openzuma chat --model anthropic/claude-sonnet-4` (per-invocation override)
2. **`~/.openzuma/config.yaml`** — the primary config file for all non-secret settings
3. **`~/.openzuma/.env`** — fallback for env vars; **required** for secrets (API keys, tokens, passwords)
...
You can set `providers.<id>.request_timeout_seconds` for a provider-wide request timeout, plus `providers.<id>.models.<model>.timeout_seconds` for a model-specific override. Applies to the primary turn client on every transport (OpenAI-wire, native Anthropic, Anthropic-compatible), the fallback chain, rebuilds after credential rotation, and (for OpenAI-wire) the per-request timeout kwarg — so the configured value wins over the legacy `OPENZUMA_API_TIMEOUT` env var.

You can also set `providers.<id>.stale_timeout_seconds` for the non-streaming stale-call detector, plus `providers.<id>.models.<model>.stale_timeout_seconds` for a model-specific override. This wins over the legacy `OPENZUMA_API_CALL_STALE_TIMEOUT` env var.

Leaving these unset keeps the legacy defaults (`OPENZUMA_API_TIMEOUT=1800`s, `OPENZUMA_API_CALL_STALE_TIMEOUT=300`s, native Anthropic 900s). Not currently wired for AWS Bedrock (both `bedrock_converse` and AnthropicBedrock SDK paths use boto3 with its own timeout configuration). See the commented example in [`cli-config.yaml.example`](https://github.com/NousResearch/openzuma-agent/blob/main/cli-config.yaml.example).

## Terminal Backend Configuration

Openzuma supports six terminal backends. Each determines where the agent's shell commands actually execute — your local machine, a Docker container, a remote server via SSH, a Modal cloud sandbox, a Daytona workspace, or a Singularity/Apptainer container.
...
For cloud sandboxes such as Modal and Daytona, `container_persistent: true` means Openzuma will try to preserve filesystem state across sandbox recreation. It does not promise that the same live sandbox, PID space, or background processes will still be running later.
...
:::warning
The agent has the same filesystem access as your user account. Use `openzuma tools` to disable tools you don't want, or switch to Docker for sandboxing.
:::
...
**Requirements:** Docker Desktop or Docker Engine installed and running. Openzuma probes `$PATH` plus common macOS install locations (`/usr/local/bin/docker`, `/opt/homebrew/bin/docker`, Docker Desktop app bundle).
...
**Credential forwarding:** Env vars listed in `docker_forward_env` are resolved from your shell environment first, then `~/.openzuma/.env`. Skills can also declare `required_environment_variables` which are merged automatically.
...
**Persistence:** When enabled, the sandbox filesystem is snapshotted on cleanup and restored on next session. Snapshots are tracked in `~/.openzuma/modal_snapshots.json`. This preserves filesystem state, not live processes, PID space, or background jobs.

**Credential files:** Automatically mounted from `~/.openzuma/` (OAuth tokens, etc.) and synced before each command.
...
**Persistence:** When enabled, sandboxes are stopped (not deleted) on cleanup and resumed on next session. Sandbox names follow the pattern `openzuma-{task_id}`.
...
**Scratch directory:** Resolved in order: `TERMINAL_SCRATCH_DIR` → `TERMINAL_SANDBOX_DIR/singularity` → `/scratch/$USER/openzuma-agent` (HPC convention) → `~/.openzuma/sandboxes/singularity`.
...
- **Docker** — Run `docker version` to verify Docker is working. If it fails, fix Docker or `openzuma config set terminal.backend local`.
- **SSH** — Both `TERMINAL_SSH_HOST` and `TERMINAL_SSH_USER` must be set. Openzuma logs a clear error if either is missing.
- **Modal** — Needs `MODAL_TOKEN_ID` env var or `~/.modal.toml`. Run `openzuma doctor` to check.
...
terminal:
  ...
  docker_volumes:
    ...
    - "/home/user/.openzuma/cache/documents:/output" # Gateway-visible exports
```
...
`/home/user/.openzuma/cache/documents:/output`.

- Write files inside Docker to `/output/...`
- Emit the **host path** in `MEDIA:`, for example:
  `MEDIA:/home/user/.openzuma/cache/documents/report.txt`
...
Openzuma resolves each listed variable from your current shell first, then falls back to `~/.openzuma/.env` if it was saved with `openzuma config set`.
...
Docker sandboxes stay isolated by default. Openzuma does **not** pass your current host working directory into the container unless you explicitly opt in.
...
When enabled:
- if you launch Openzuma from `~/projects/my-app`, that host directory is bind-mounted to `/workspace`
...
- `true` gives the sandbox direct access to the directory you launched Openzuma from
...
```bash
openzuma config set terminal.persistent_shell false
```
...
- `openzuma config migrate` scans all enabled skills, finds unconfigured settings, and offers to prompt you
- `openzuma config show` displays all skill settings under "Skill Settings" with the skill they belong to
- When a skill loads, its resolved config values are injected into the skill context automatically

**Setting values manually:**

```bash
openzuma config set skills.config.myplugin.path ~/myplugin-data
```
...
```yaml
worktree: true    # Always create a worktree (same as openzuma -w)
...
Openzuma automatically compresses long conversations to stay within your model's context window. The compression summarizer is a separate LLM call — you can point it at any provider or endpoint.
...
Plugin engines are **never auto-activated** — you must explicitly set `context.engine` to the plugin name. Available engines can be browsed and selected via `openzuma plugins` → Provider Plugins → Context Engine.
...
Openzuma has separate timeout layers for streaming, plus a stale detector for non-streaming calls. The stale detectors auto-adjust for local providers only when you leave them at their implicit defaults.

| Timeout | Default | Local providers | Config / env |
|---------|---------|----------------|--------------|
| Socket read timeout | 120s | Auto-raised to 1800s | `OPENZUMA_STREAM_READ_TIMEOUT` |
| Stale stream detection | 180s | Auto-disabled | `OPENZUMA_STREAM_STALE_TIMEOUT` |
| Stale non-stream detection | 300s | Auto-disabled when left implicit | `providers.<id>.stale_timeout_seconds` or `OPENZUMA_API_CALL_STALE_TIMEOUT` |
| API call (non-streaming) | 1800s | Unchanged | `providers.<id>.request_timeout_seconds` / `timeout_seconds` or `OPENZUMA_API_TIMEOUT` |

The **socket read timeout** controls how long httpx waits for the next chunk of data from the provider. Local LLMs can take minutes for prefill on large contexts before producing the first token, so Openzuma raises this to 30 minutes when it detects a local endpoint. If you explicitly set `OPENZUMA_STREAM_READ_TIMEOUT`, that value is always used regardless of endpoint detection.
...
The **stale non-stream detection** kills non-streaming calls that produce no response for too long. By default Openzuma disables this on local endpoints to avoid false positives during long prefills. If you explicitly set `providers.<id>.stale_timeout_seconds`, `providers.<id>.models.<model>.stale_timeout_seconds`, or `OPENZUMA_API_CALL_STALE_TIMEOUT`, that explicit value is honored even on local endpoints.
...
Openzuma uses lightweight "auxiliary" models for side tasks like image analysis, web page summarization, and browser screenshot analysis. By default, these use **Gemini Flash** via auto-detection — you don't need to configure anything.

### The universal config pattern

Every model slot in Openzuma — auxiliary tasks, compression, fallback — uses the same three knobs:
...
When `base_url` is set, Openzuma ignores the provider and calls that endpoint directly (using `api_key` or `OPENAI_API_KEY` for auth). When only `provider` is set, Openzuma uses that provider's built-in auth and base URL.
...
If you use a reasoning-heavy model for `auxiliary.session_search`, Openzuma now gives you two built-in controls:

- `auxiliary.session_search.max_concurrency`: limits how many matched sessions Openzuma summarizes at once
...
Use `extra_body` only when your provider documents OpenAI-compatible request-body fields you want Openzuma to pass through for that task. Openzuma forwards the object as-is.

:::warning
`extra_body` is only effective when your provider actually supports the field you send. If the provider does not expose a native OpenAI-compatible reasoning-off flag, Openzuma cannot synthesize one on its behalf.
:::
...
Or via environment variable (in `~/.openzuma/.env`):
...
| `"nous"` | Force Nous Portal | `openzuma auth` |
| `"codex"` | Force Codex OAuth (ChatGPT account). Supports vision (gpt-5.3-codex). | `openzuma model` → Codex |
| `"main"` | Use your active custom/main endpoint. This can come from `OPENAI_BASE_URL` + `OPENAI_API_KEY` or from a custom endpoint saved via `openzuma model` / `config.yaml`. Works with OpenAI, local models, or any OpenAI-compatible API. **Auxiliary tasks only — not valid for `model.provider`.** | Custom endpoint credentials + base URL |
...
`base_url` takes precedence over `provider`, so this is the most explicit way to route an auxiliary task to a specific endpoint. For direct endpoint overrides, Openzuma uses the configured `api_key` or falls back to `OPENAI_API_KEY`; it does not reuse `OPENROUTER_API_KEY` for that custom endpoint.
...
`provider: "main"` uses whatever provider Openzuma uses for normal chat — whether that's a named custom provider (e.g. `beans`), a built-in provider like `openrouter`, or a legacy `OPENAI_BASE_URL` endpoint.
...
:::tip
Run `openzuma config` to see your current auxiliary model settings. Overrides only show up when they differ from the defaults.
:::
...
`interim_assistant_messages` is gateway-only. When enabled, Openzuma sends completed mid-turn assistant updates as separate chat messages. This is independent from `tool_progress` and does not require gateway streaming.
...
If the requested provider is unavailable, Openzuma falls back automatically in this order: `local` → `groq` → `openai`.
...
:::note
Streaming is disabled by default. Enable it in `~/.openzuma/config.yaml` to try the streaming UX.
:::
...
- `false` reverts to the old shared-room behavior. That can be useful if you explicitly want Openzuma to treat a channel like one collaborative conversation, but it also means users share context, token costs, and interrupt state.
- Direct messages are unaffected. Openzuma still keys DMs by chat/DM ID as usual.
...
Control what Openzuma does when an unknown user sends a direct message:
...
- `pair` is the default. Openzuma denies access, but replies with a one-time pairing code in DMs.
...
quick_commands:
  status:
    type: exec
    command: systemctl status openzuma-agent
  disk:
    type: exec
    command: df -h /
  update:
    type: exec
    command: cd ~/.openzuma/openzuma-agent && git pull && pip install -e .
  gpu:
    type: exec
...
- **`strict`** — scripts run in a temp staging directory with `sys.executable` (Openzuma's own python). Maximum reproducibility, but project deps and relative paths won't resolve.
...
The `web_search`, `web_extract`, and `web_crawl` tools support four backend providers. Configure the backend in `config.yaml` or via `openzuma tools`:
...
**Exa:** Set `EXA_API_KEY` in `~/.openzuma/.env`. Supports `category` filtering (`company`, `research paper`, `news`, `people`, `personal site`, `pdf`) and domain/date filters.
...
browser:
  ...
  record_sessions: false         # Auto-record browser sessions as WebM videos to ~/.openzuma/browser_recordings/
  camofox:
...
security:
  website_blocklist:
    ...
    shared_files:                # Load additional rules from external files
      - "/etc/openzuma/blocked-sites.txt"
```
...
Control how Openzuma handles potentially dangerous commands:
...
| `off` | Skip all approval checks. Equivalent to `OPENZUMA_YOLO_MODE=true`. **Use with caution.** |
...
```yaml
checkpoints:
  enabled: true                  # Enable automatic checkpoints (also: openzuma --checkpoints)
...
**Direct endpoint override:** If you want the obvious custom-endpoint path, set `delegation.base_url`, `delegation.api_key`, and `delegation.model`. That sends subagents directly to that OpenAI-compatible endpoint and takes precedence over `delegation.provider`. If `delegation.api_key` is omitted, Openzuma falls back to `OPENAI_API_KEY` only.
...
Openzuma uses two different context scopes:

| File | Purpose | Scope |
|------|---------|-------|
| `SOUL.md` | **Primary agent identity** — defines who the agent is (slot #1 in the system prompt) | `~/.openzuma/SOUL.md` or `$OPENZUMA_HOME/SOUL.md` |
| `.openzuma.md` / `OPENZUMA.md` | Project-specific instructions (highest priority) | Walks to git root |
...
- If SOUL.md is missing, empty, or cannot be loaded, Openzuma falls back to a built-in default identity.
- **Project context files use a priority system** — only ONE type is loaded (first match wins): `.openzuma.md` → `AGENTS.md` → `CLAUDE.md` → `.cursorrules`. SOUL.md is always loaded independently.
- **AGENTS.md** is hierarchical: if subdirectories also have AGENTS.md, all are combined.
- Openzuma automatically seeds a default `SOUL.md` if one does not already exist.
...
| Context | Default |
|---------|---------|
| **CLI (`openzuma`)** | Current directory where you run the command |
| **Messaging gateway** | Home directory `~` (override with `MESSAGING_CWD`) |
| **Docker / Singularity / Modal / SSH** | User's home directory inside the container or remote machine |

Override the working directory:
```bash
# In ~/.openzuma/.env or ~/.openzuma/config.yaml:
