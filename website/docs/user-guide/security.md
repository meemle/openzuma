...
Openzuma Agent is designed with a defense-in-depth security model. This page covers every security boundary — from command approval to container isolation to user authorization on messaging platforms.
...
Before executing any command, Openzuma checks it against a curated list of dangerous patterns. If a match is found, the user must explicitly approve it.

### Approval Modes

The approval system supports three modes, configured via `approvals.mode` in `~/.openzuma/config.yaml`:
...
1. **CLI flag**: Start a session with `openzuma --yolo` or `openzuma chat --yolo`
2. **Slash command**: Type `/yolo` during a session to toggle it on/off
3. **Environment variable**: Set `OPENZUMA_YOLO_MODE=1`
...
YOLO mode is available in both CLI and gateway sessions. Internally, it sets the `OPENZUMA_YOLO_MODE` environment variable which is checked before every command execution.
...
Configure the timeout in `~/.openzuma/config.yaml`:
...
| `tee` to `/etc/`, `~/.ssh/`, `~/.openzuma/.env` | Overwrite sensitive file via tee |
| `>` / `>>` to `/etc/`, `~/.ssh/`, `~/.openzuma/.env` | Overwrite sensitive file via redirection |
...
| `pkill`/`killall` openzuma/gateway | Self-termination prevention |
...
The `OPENZUMA_EXEC_ASK=1` environment variable is automatically set when running the gateway.

### Permanent Allowlist

Commands approved with "always" are saved to `~/.openzuma/config.yaml`:
...
:::tip
Use `openzuma config edit` to review or remove patterns from your permanent allowlist.
:::

## User Authorization (Gateway)

When running the messaging gateway, Openzuma controls who can interact with the bot through a layered authorization system.
...
Set allowed user IDs as comma-separated values in `~/.openzuma/.env`:
...
For more flexible authorization, Openzuma includes a code-based pairing system. Instead of requiring user IDs upfront, unknown users receive a one-time pairing code that the bot owner approves via the CLI.

**How it works:**

1. An unknown user sends a DM to the bot
2. The bot replies with an 8-character pairing code
3. The bot owner runs `openzuma pairing approve <platform> <code>` on the CLI
4. The user is permanently approved for that platform

Control how unauthorized direct messages are handled in `~/.openzuma/config.yaml`:
...
openzuma pairing list

# Approve a pairing code
openzuma pairing approve telegram ABC12DEF

# Revoke a user's access
openzuma pairing revoke telegram 123456789

# Clear all pending codes
openzuma pairing clear-pending
```

**Storage:** Pairing data is stored in `~/.openzuma/pairing/` with per-platform JSON files:
...
When using the `docker` terminal backend, Openzuma applies strict security hardening to every container.
...
Container resources are configurable in `~/.openzuma/config.yaml`:
...
- **Persistent mode** (`container_persistent: true`): Bind-mounts `/workspace` and `/root` from `~/.openzuma/sandboxes/docker/<task_id>/`
...
Some skills need **files** (not just env vars) in the sandbox — for example, Google Workspace stores OAuth tokens as `google_token.json` under the active profile's `OPENZUMA_HOME`. Skills declare these in frontmatter:
...
When loaded, Openzuma checks if these files exist in the active profile's `OPENZUMA_HOME` and registers them for mounting:
...
Paths are relative to `~/.openzuma/`. Files are mounted to `/root/.openzuma/` inside the container.
...
| **terminal** (local) | Blocks explicit Openzuma infrastructure vars (provider keys, gateway tokens, tool API keys) | ✅ Passthrough vars bypass the blocklist |
...
- Openzuma infrastructure secrets (provider API keys, gateway tokens) should never be added to `env_passthrough` — they have dedicated mechanisms
...
Openzuma integrates [tirith](https://github.com/sheeki03/tirith) for content-level command scanning before execution. Tirith detects threats that pattern matching alone misses:
...
4. **Store secrets securely** — keep API keys in `~/.openzuma/.env` with proper file permissions
...
9. **Monitor logs** — check `~/.openzuma/logs/` for unauthorized access attempts
10. **Keep updated** — run `openzuma update` regularly for security patches

### Securing API Keys

```bash
# Set proper permissions on the .env file
chmod 600 ~/.openzuma/.env
...
terminal:
  ...
  ssh_user: "openzuma"
  ssh_key: "~/.ssh/openzuma_agent_key"
```
