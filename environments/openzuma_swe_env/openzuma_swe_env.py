"""
OpenzumaSweEnv -- SWE-Bench Style Environment with Modal Sandboxes
...
Usage:
    ...
    vllm serve YourModel --tool-parser openzuma
    run-api
    python environments/openzuma_swe_env.py serve \\
...
class OpenzumaSweEnv(OpenzumaAgentBaseEnv):
    ...
    name = "openzuma-swe"
    env_config_cls = OpenzumaSweEnvConfig

    @classmethod
    def config_init(cls) -> Tuple[OpenzumaSweEnvConfig, List[APIServerConfig]]:
        """
        ...
        """
        env_config = OpenzumaSweEnvConfig(
            ...
            # Atropos settings
            group_size=4,
            tokenizer_name="NousResearch/DeepOpenzuma-3-Llama-3-3B-Preview",
            tool_call_parser="openzuma",
            steps_per_eval=50,
            total_steps=500,
            use_wandb=True,
            wandb_name="openzuma-swe",
        )

        server_configs = [
            APIServerConfig(
                base_url="http://localhost:8000/v1",
                model_name="NousResearch/DeepOpenzuma-3-Llama-3-3B-Preview",
