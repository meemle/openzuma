"""
OpenzumaAgentBaseEnv -- Abstract Base Environment for Openzuma-Agent + Atropos

Provides the Atropos integration plumbing that all openzuma-agent environments share:
- Two-mode operation (OpenAI server for Phase 1, VLLM ManagedServer for Phase 2)
- Per-group toolset/distribution resolution
- Agent loop orchestration via OpenzumaAgentLoop
...
# Ensure the openzuma-agent repo root is on sys.path so that imports like
...
# Load API keys from openzuma-agent/.env so all environments can access them
...
from environments.agent_loop import AgentResult, OpenzumaAgentLoop
...
# Import openzuma-agent toolset infrastructure
...
class OpenzumaAgentEnvConfig(BaseEnvConfig):
    """
    Configuration for openzuma-agent Atropos environments.
    ...
    enabled_toolsets: Optional[List[str]] = Field(
        default=None,
        description="Explicit list of openzuma toolsets to enable (e.g., ['terminal', 'file', 'web']). "
    ...
    tool_call_parser: str = Field(
        ...
        "Options: openzuma, mistral, llama3_json, qwen, deepseek_v3, etc.",
    )
    ...
    default_result_size_chars: int = Field(
        ...
        "to sandbox. Results exceeding this are written to /tmp/openzuma-results/ "
...
class OpenzumaAgentBaseEnv(BaseEnv):
    """
    Abstract base environment for openzuma-agent Atropos integration.
    ...
    name: Optional[str] = "openzuma-agent"
    env_config_cls = OpenzumaAgentEnvConfig

    def __init__(
        self,
        config: OpenzumaAgentEnvConfig,
        ...
        # Set terminal environment variables so openzuma tools pick them up.
    ...
    async def compute_reward(
        ...
        - ctx: ToolContext -- call ANY openzuma-agent tool (terminal, file, web,
