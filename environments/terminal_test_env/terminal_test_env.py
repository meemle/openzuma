...
Training tasks (3):
    1. Create ~/greeting.txt with "Hello from Openzuma Agent"
...
TRAIN_TASKS = [
    {
        "prompt": "Create a file at ~/greeting.txt containing exactly the text: Hello from Openzuma Agent",
        "verify_path": "~/greeting.txt",
        "expected_content": "Hello from Openzuma Agent",
    },
    {
...
class TerminalTestEnv(OpenzumaAgentBaseEnv):
    ...
    def config_init(cls) -> Tuple[TerminalTestEnvConfig, List[APIServerConfig]]:
        ...
        Claude for inference. API keys loaded from ~/openzuma-agent/.env.
        """
