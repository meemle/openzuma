...
The evaluate flow:
    ...
    2. evaluate()  -- Iterates over all runs sequentially through:
        a. rollout_and_score_eval()  -- Per-run agent loop
            ...
            - Runs OpenzumaAgentLoop with terminal tool only
...
Requires: pip install openzuma-agent[yc-bench]
"""
...
class YCBenchEvalConfig(OpenzumaAgentEnvConfig):
    """
    Configuration for the YC-Bench evaluation environment.

    Extends OpenzumaAgentEnvConfig with YC-Bench-specific settings for
...
class YCBenchEvalEnv(OpenzumaAgentBaseEnv):
    """
    ...
    a competing built-in agent loop). The OpenzumaAgentLoop then drives the
    ...
    async def rollout_and_score_eval(self, eval_item: Dict[str, Any]) -> Dict:
        ...
        3. Runs OpenzumaAgentLoop with terminal tool
        ...
        try:
            ...
            # LiteLLM), which would compete with our OpenzumaAgentLoop.
            ...
            # ----------------------------------------------------------
            # Step 2: Run the OpenzumaAgentLoop
            # ----------------------------------------------------------
