...
## 5. Openzuma Configuration

### Config location

`$OPENZUMA_HOME/config.yaml` (defaults to `~/.openzuma/config.yaml` when `OPENZUMA_HOME` is unset)
...
Restart the Openzuma session for changes to take effect. The MCP connection is
