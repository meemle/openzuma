...
OPENZUMA_HOME_DIR="${OPENZUMA_HOME:-$HOME/.openzuma}"
OPENZUMA_CFG="${OPENZUMA_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Openzuma config has twozero_td MCP entry ──
if [[ ! -f "$OPENZUMA_CFG" ]]; then
    echo -e " ${FAIL} Openzuma config not found at ${OPENZUMA_CFG}"
    manual_steps+=("Create ${OPENZUMA_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$OPENZUMA_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Openzuma config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Openzuma config..."
    python3 -c "
import yaml, sys, copy

cfg_path = '$OPENZUMA_CFG'
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${OPENZUMA_CFG} manually"); }
    manual_steps+=("Restart Openzuma session to pick up config change")
fi
