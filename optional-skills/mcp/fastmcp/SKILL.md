...
- smoke-test a server with the FastMCP CLI before wiring it into Openzuma or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Openzuma. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.openzuma/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.openzuma/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Openzuma integration, either:

- configure the server in `~/.openzuma/config.yaml` using the `native-mcp` skill, or
...
### Openzuma cannot see the deployed server

The server-building part may be correct while the Openzuma config is not. Load the `native-mcp` skill and configure the server in `~/.openzuma/config.yaml`, then restart Openzuma.
