...
version: 1.0.0
author: arceus77-7, enhanced by Openzuma Agent
license: MIT
metadata:
  openzuma:
...
- `tmux` available for stable authenticated sessions during Openzuma terminal calls (desktop app flow only)
...
### Service Account (recommended for Openzuma)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.openzuma/.env` (the skill will prompt for this on first load).
...
## Openzuma Execution Pattern (desktop app flow)

Openzuma terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/openzuma-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/openzuma-op.sock"
