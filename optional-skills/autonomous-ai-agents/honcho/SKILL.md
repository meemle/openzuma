---
name: honcho
description: Configure and use Honcho memory with Openzuma -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.
version: 2.0.0
author: Openzuma Agent
license: MIT
metadata:
  openzuma:
...
# Honcho Memory for Openzuma

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Openzuma profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/openzuma#running-honcho-locally-with-openzuma

### Verify

```bash
openzuma honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Openzuma profile's AI peer
...
Honcho models conversations as interactions between **peers**. Openzuma creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Openzuma instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Openzuma run |
| `global` | Single session across all directories |

Manual override: `openzuma honcho map my-project-name`
...
Each Openzuma profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block openzuma.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `openzuma.coder` host block in `honcho.json`
...
```bash
openzuma honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Openzuma when Honcho memory is active.
...
Config file: `$OPENZUMA_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `openzuma honcho setup`. Ensure `memory.provider: honcho` is in `~/.openzuma/config.yaml`.

### Memory not persisting across sessions
Check `openzuma honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `openzuma profile create <name> --clone`. For existing profiles: `openzuma honcho sync`.
...
| Command | Description |
|---------|-------------|
| `openzuma honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `openzuma honcho status` | Show resolved config, connection test, peer info for active profile |
| `openzuma honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `openzuma honcho disable` | Disable Honcho for the active profile |
| `openzuma honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `openzuma honcho peers` | Show peer identities across all profiles |
| `openzuma honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `openzuma honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `openzuma honcho sessions` | List known directory-to-session-name mappings |
| `openzuma honcho map <name>` | Map current working directory to a Honcho session name |
| `openzuma honcho identity` | Seed AI peer identity or show both peer representations |
| `openzuma honcho sync` | Create host blocks for all Openzuma profiles that don't have one yet |
| `openzuma honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Openzuma + Honcho |
| `openzuma memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `openzuma memory status` | Show active memory provider and config |
| `openzuma memory off` | Disable external memory provider |
