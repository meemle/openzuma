---
name: openzuma-atropos-environments
description: Build, test, and debug Openzuma Agent RL environments for Atropos training. Covers the OpenzumaAgentBaseEnv interface, reward functions, agent loop integration, evaluation with tools, wandb logging, and the three CLI modes (serve/process/evaluate). Use when creating, reviewing, or fixing RL environments in the openzuma-agent repo.
version: 1.1.0
...
Guide for building RL environments in the openzuma-agent repo that integrate with the Atropos training framework.

## Architecture Overview

```
Atropos BaseEnv (atroposlib/envs/base.py)
    └── OpenzumaAgentBaseEnv (environments/openzuma_base_env.py)
...
Openzuma environments are special because they run a **multi-turn agent loop with tool calling** — not just single-turn completions. The base env handles the loop; you implement the task and scoring.

## File Locations

| File | Purpose |
|------|---------|
| `environments/openzuma_base_env.py` | Base class with agent loop + tool resolution |
| `environments/agent_loop.py` | `OpenzumaAgentLoop` + `AgentResult` dataclass |
| `environments/tool_context.py` | `ToolContext` for reward verification |
| `environments/tool_call_parsers.py` | Phase 2 tool call parsers (openzuma, mistral, etc.) |
...
The whole point of openzuma-agent environments is agentic evaluation:
...
2. **evaluate() must use OpenzumaAgentLoop, not chat_completion** — Single-turn chat_completion has no tools. The whole point of openzuma-agent benchmarks is agentic evaluation with tool use.
