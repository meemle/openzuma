...
description: Migrate a user's OpenClaw customization footprint into Openzuma Agent. Imports Openzuma-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.
version: 1.0.0
author: Openzuma Agent (Nous Research)
license: MIT
metadata:
  openzuma:
    tags: [Migration, OpenClaw, Openzuma, Memory, Persona, Import]
    related_skills: [openzuma-agent]
---

# OpenClaw -> Openzuma Migration

Use this skill when a user wants to move their OpenClaw setup into Openzuma Agent with minimal manual cleanup.
...
```bash
openzuma claw migrate              # Full interactive migration
openzuma claw migrate --dry-run    # Preview what would be migrated
openzuma claw migrate --preset user-data   # Migrate without secrets
openzuma claw migrate --overwrite  # Overwrite existing conflicts
openzuma claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `openzuma setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_openzuma.py` to:

- import `SOUL.md` into the Openzuma home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Openzuma memory entries
- merge OpenClaw command approval patterns into Openzuma `command_allowlist`
- migrate Openzuma-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS` and `MESSAGING_CWD`
- copy OpenClaw skills into `~/.openzuma/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Openzuma workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.openzuma/tts/`
- archive non-secret docs that do not have a direct Openzuma destination
...
- `scripts/openclaw_to_openzuma.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.openzuma/skills/migration/openclaw-migration/scripts/openclaw_to_openzuma.py`

Do not guess a shorter path like `~/.openzuma/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.openzuma/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Openzuma-compatible secrets, currently:
...
Openzuma CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Openzuma. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Openzuma workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Openzuma skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.openzuma/skills/migration/openclaw-migration/scripts/openclaw_to_openzuma.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.openzuma/skills/migration/openclaw-migration/scripts/openclaw_to_openzuma.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.openzuma/skills/migration/openclaw-migration/scripts/openclaw_to_openzuma.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.openzuma/skills/migration/openclaw-migration/scripts/openclaw_to_openzuma.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.openzuma/skills/migration/openclaw-migration/scripts/openclaw_to_openzuma.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.openzuma/skills/migration/openclaw-migration/scripts/openclaw_to_openzuma.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Openzuma unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Openzuma targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Openzuma destination. Unsupported auth blobs must still be reported as skipped.
...
- Openzuma persona state imported
- Openzuma memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.openzuma/skills/openclaw-imports/`
