...
VERSION_FILE = REPO_ROOT / "openzuma_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "openzuma@nousresearch.com": "NousResearch",
    "openzuma@noushq.ai": "benbarclay",
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "openzuma@nousresearch.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/NousResearch/openzuma-agent",
    ...
    lines.append(f"# Openzuma Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Openzuma Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Openzuma Agent Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  Openzuma Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Openzuma Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Openzuma Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Openzuma Agent v{new_version} ({calver_date})' "
