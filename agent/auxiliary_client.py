...
Resolution order for text tasks (auto mode):
  ...
  2. Nous Portal (~/.openzuma/auth.json active provider)
...
_AI_GATEWAY_HEADERS = {
    "HTTP-Referer": "https://openzuma-agent.nousresearch.com",
    "X-Title": "Openzuma Agent",
    "User-Agent": f"OpenzumaAgent/{_OPENZUMA_VERSION}",
}

# Nous Portal extra_body for product attribution.
# Callers should pass this as extra_body in chat.completions.create()
# when the auxiliary client is backed by Nous Portal.
NOUS_EXTRA_BODY = {"tags": ["product=openzuma-agent"]}
...
_AUTH_JSON_PATH = get_openzuma_home() / "auth.json"
...
def _codex_cloudflare_headers(access_token: str) -> Dict[str, str]:
    ...
    """
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (Openzuma Agent)",
...
def _read_nous_auth() -> Optional[dict]:
    """Read and validate ~/.openzuma/auth.json for an active Nous provider.
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from Openzuma auth store.
    ...
    try:
        from openzuma_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from openzuma_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if provider_id == "anthropic":
            ...
            try:
                from openzuma_cli.auth import is_provider_explicitly_configured
...
def _try_nous(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    model = _NOUS_MODEL
    try:
        from openzuma_cli.models import get_nous_recommended_aux_model
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `openzuma setup` or `openzuma model` and enter a valid http(s) base URL."
        ) from exc
...
def _resolve_auto(main_runtime: Optional[Dict[str, Any]] = None) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    #    scenario where a user switches providers via `openzuma model` but the
    #    old OPENAI_BASE_URL lingers in ~/.openzuma/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "Run: openzuma model to reconfigure, or remove "
                "OPENAI_BASE_URL from ~/.openzuma/.env",
...
def resolve_provider_client(
    ...
    if provider == "nous":
        ...
        if client is None:
            logger.warning("resolve_provider_client: nous requested "
                           "but Nous Portal not configured (run: openzuma auth)")
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model) if async_mode
                else (client, final_model))
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: openzuma model)")
                return None, None
    ...
    try:
        from openzuma_cli.runtime_provider import _get_named_custom_provider
    ...
    try:
        ...
        logger.debug("openzuma_cli.auth not available for provider %s", provider)
        return None, None
...
def _build_call_kwargs(
    ...
    if provider == "nous" or auxiliary_is_nous:
        merged_extra.setdefault("tags", []).extend(["product=openzuma-agent"])
    if merged_extra:
