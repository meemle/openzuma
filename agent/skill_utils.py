...
from openzuma_constants import get_config_path, get_skills_dir
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``OPENZUMA_PLATFORM`` or
            ``OPENZUMA_SESSION_PLATFORM`` env vars.  Falls back to the
    ...
    resolved_platform = (
        platform
        or os.getenv("OPENZUMA_PLATFORM")
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.openzuma/skills/`` are silently skipped.
    """
    config_path = get_config_path()
    if not config_path.exists():
        return []
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.openzuma/skills/`` first, then external.
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    openzuma = metadata.get("openzuma") or {}
    if not isinstance(openzuma, dict):
        openzuma = {}
    return {
        "fallback_for_toolsets": openzuma.get("fallback_for_toolsets", []),
        "requires_toolsets": openzuma.get("requires_toolsets", []),
        "fallback_for_tools": openzuma.get("fallback_for_tools", []),
        "requires_tools": openzuma.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    openzuma = metadata.get("openzuma")
    if not isinstance(openzuma, dict):
        return []
    raw = openzuma.get("config")
    if not raw:
        return []
