...
class CredentialPool:
        ...
        try:
            if self.provider == "anthropic":
                ...
                refreshed = refresh_anthropic_oauth_pure(
                    entry.refresh_token,
                    use_json=entry.source.endswith("openzuma_pkce"),
                )
                updated = replace(
                    entry,
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            ...
                            use_json=synced.source.endswith("openzuma_pkce"),
                        )
                        updated = replace(
                            synced,
                            access_token=refreshed["access_token"],
    ...
    def _available_entries(self, *, clear_expired: bool = False, refresh: bool = False) -> List[PooledCredential]:
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other Openzuma profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `openzuma auth remove <provider> <N>` is stable across all source types.
    try:
        from openzuma_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
        def _is_suppressed(_p, _s):  # type: ignore[misc]
            return False

    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, Openzuma PKCE)
        ...
        from agent.anthropic_adapter import read_claude_code_credentials, read_openzuma_oauth_credentials

        for source_name, creds in (
            ("openzuma_pkce", read_openzuma_oauth_credentials()),
        ...
        if state and not _is_suppressed(provider, "device_code"):
            ...
            # `openzuma auth add nous --label <name>`).  Fall back to the
        ...
        # Openzuma auth store or env vars, so resolve them here.
        # Use refresh_if_expiring=False to avoid network calls during
        # pool loading / provider discovery.
        try:
            from openzuma_cli.auth import resolve_qwen_runtime_credentials
        ...
        # Respect user suppression — `openzuma auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the Openzuma auth store.  Without this gate the removal is instantly
        ...
        # Openzuma owns its own Codex auth state — we do NOT auto-import from
        ...
        # via `openzuma auth openai-codex`.
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    changed = False
    active_sources: Set[str] = set()
    # Honour user suppression — `openzuma auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.openzuma/.env.
    ...
    try:
        from openzuma_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
...
def _prune_stale_seeded_entries(entries: List[PooledCredential], active_sources: Set[str]) -> bool:
    retained = [
        ...
        or not (
            ...
            or entry.source in {"claude_code", "openzuma_pkce"}
        )
    ]
