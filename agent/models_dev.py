...
Data resolution order (like TypeScript OpenCode):
  ...
  2. Disk cache (~/.openzuma/models_dev_cache.json)
...
# ---------------------------------------------------------------------------
# Provider ID mapping: Openzuma ↔ models.dev
# ---------------------------------------------------------------------------

# Openzuma provider names → models.dev provider IDs
...
# Reverse mapping: models.dev → Openzuma (built lazily)
...
def _get_cache_path() -> Path:
    ...
    return get_openzuma_home() / "models_dev_cache.json"
...
def _get_provider_models(provider: str) -> Optional[Dict[str, Any]]:
    """Resolve a Openzuma provider ID to its models dict from models.dev.
...
# Gemma models whose TPM quotas are too small for normal Openzuma agent traffic.
...
def get_provider_info(provider_id: str) -> Optional[ProviderInfo]:
    ...
    Accepts either a Openzuma provider ID (e.g. "kilocode") or a models.dev
    ID (e.g. "kilo").  Returns None if the provider is not in the catalog.
    """
    # Resolve Openzuma ID → models.dev ID
...
def get_model_info(
    ...
    Accepts Openzuma or models.dev provider ID.  Tries exact match then
