...
as ``kind: backend``) or ``~/.openzuma/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class ImageGenProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``openzuma tools``. Defaults to ``name.title()``."""
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        """Return catalog entries for ``openzuma tools`` model picker.
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        """Return provider metadata for the ``openzuma tools`` picker.
...
def _images_cache_dir() -> Path:
    """Return ``$OPENZUMA_HOME/cache/images/``, creating parents as needed."""
    from openzuma_constants import get_openzuma_home

    path = get_openzuma_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$OPENZUMA_HOME/cache/images/``.
