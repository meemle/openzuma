...
it is automatically downloaded from GitHub releases to $OPENZUMA_HOME/bin/tirith.
...
def _get_openzuma_home() -> str:
    """Return the Openzuma home directory, respecting OPENZUMA_HOME env var."""
    return str(get_openzuma_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_openzuma_home(), ".tirith-install-failed")
...
def _openzuma_bin_dir() -> str:
    """Return $OPENZUMA_HOME/bin, creating it if needed."""
    d = os.path.join(_get_openzuma_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $OPENZUMA_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_openzuma_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $OPENZUMA_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $OPENZUMA_HOME/bin/tirith
    ...
    openzuma_bin = os.path.join(_openzuma_bin_dir(), "tirith")
    if os.path.isfile(openzuma_bin) and os.access(openzuma_bin, os.X_OK):
        _resolved_path = openzuma_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return openzuma_bin
