...
def _browser_candidate_path_dirs() -> list[str]:
    ...
    openzuma_node_bin = str(openzuma_home / "node" / "bin")
    return [openzuma_node_bin, *list(_discover_homebrew_node_dirs()), *_SANE_PATH_DIRS]
...
def _get_command_timeout() -> int:
    ...
    try:
        from openzuma_cli.config import read_raw_config
        cfg = read_raw_config()
...
def _socket_safe_tmpdir() -> str:
    ...
    append ``agent-browser-openzuma_…`` the resulting socket path exceeds the
...
def _emergency_cleanup_all_sessions():
    ...
    crashed openzuma processes — this way every clean openzuma exit sweeps
    ...
    # Sweep orphans from other crashed openzuma processes.  Safe even if we
    # never used the browser — uses owner_pid liveness to avoid reaping
    # daemons owned by other live openzuma processes.
    try:
...
def _write_owner_pid(socket_dir: str, session_name: str) -> None:
    """Record the current openzuma PID as the owner of a browser socket dir.

    Written atomically to ``<socket_dir>/<session_name>.owner_pid`` so the
    orphan reaper can distinguish daemons owned by a live openzuma process
...
def _reap_orphaned_browser_sessions():
    ...
    daemons whose owning openzuma process is no longer alive.

    Ownership detection priority:
      1. ``<session>.owner_pid`` file (written by current code) — if the
         referenced openzuma PID is alive, leave the daemon alone regardless
         of whether it's in *this* process's ``_active_sessions``.  This is
         cross-process safe: two concurrent openzuma instances won't reap each
         other's daemons.
    ...
    socket_dirs += glob.glob(os.path.join(tmpdir, "agent-browser-openzuma_*"))
    ...
    for socket_dir in socket_dirs:
        ...
        if owner_alive is True:
            # Owner is alive — this session belongs to a live openzuma process.
            continue
...
def _find_agent_browser() -> str:
    ...
    Checks in order: current PATH, Homebrew/common bin dirs, Openzuma-managed
    ...
    # Build an extended search PATH including Openzuma-managed Node, macOS
...
def _run_browser_command(
    ...
    try:
        ...
        # Record this openzuma PID as the session owner (cross-process safe
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        recordings_dir = openzuma_home / "browser_recordings"
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> str:
    ...
    from openzuma_constants import get_openzuma_dir
    screenshots_dir = get_openzuma_dir("cache/screenshots", "browser_screenshots")
