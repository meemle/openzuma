...
def _is_openzuma_provider_credential(name: str) -> bool:
    """True if ``name`` is a Openzuma-managed provider credential (API key,
    token, or similar) per ``_OPENZUMA_PROVIDER_ENV_BLOCKLIST``.
    ...
    Non-Openzuma API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
    ...
    try:
        ...
        return False
    return name in _OPENZUMA_PROVIDER_ENV_BLOCKLIST
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are Openzuma-managed provider credentials (from
    ``_OPENZUMA_PROVIDER_ENV_BLOCKLIST``) are rejected here to preserve
    the ``execute_code`` sandbox's credential-scrubbing guarantee per
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a Openzuma-managed
    ...
    Non-Openzuma third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    for name in var_names:
        ...
        if not name:
            continue
        if _is_openzuma_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register Openzuma provider "
                "credential %r (blocked by _OPENZUMA_PROVIDER_ENV_BLOCKLIST). "
