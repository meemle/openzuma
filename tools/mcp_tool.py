...
discovers their tools, and registers them into the openzuma-agent tool registry
so the agent can call them like any built-in tool.

Configuration is read from ~/.openzuma/config.yaml under the ``mcp_servers`` key.
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            openzuma_home = os.path.expanduser(
                os.getenv(
                    "OPENZUMA_HOME", os.path.join(os.path.expanduser("~"), ".openzuma")
                )
            )
            candidates = [
                os.path.join(openzuma_home, "node", "bin", resolved_command),
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"Run `openzuma mcp login {server_name}` (or delete the tokens "
            f"file under ~/.openzuma/mcp-tokens/ and restart). Do NOT retry "
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the Openzuma config file.
    ...
    ``os.environ`` (which includes ``~/.openzuma/.env`` loaded at startup).
    """
    try:
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves Openzuma's historical behavior of converting hyphens to
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the Openzuma registry schema format.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``openzuma tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the Openzuma registry.
