...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("OPENZUMA_SESSION_KEY", default)

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $OPENZUMA_HOME.
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_OPENZUMA_ENV_PATH = (
    r'(?:~\/\.openzuma/|'
    r'(?:\$home|\$\{home\})/\.openzuma/|'
    r'(?:\$openzuma_home|\$\{openzuma_home\})/)'
    r'\.env\b'
)
_SENSITIVE_WRITE_TARGET = (
    r'(?:/etc/|/dev/sd|'
    rf'{_SSH_SENSITIVE_PATH}|'
    rf'{_OPENZUMA_ENV_PATH})'
)
...
DANGEROUS_PATTERNS = [
    ...
    (r'\bopenzuma\s+gateway\s+(stop|restart)\b', "stop/restart openzuma gateway (kills running agents)"),
    (r'\bopenzuma\s+update\b', "openzuma update (restarts gateway, kills running agents)"),
    # Gateway protection: never start gateway outside systemd management
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart openzuma-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart openzuma-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(openzuma|gateway|cli\.py)\b', "kill openzuma/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill openzuma` but not
    # `kill -9 $(pgrep -f openzuma)` because the substitution is opaque
...
def check_dangerous_command(command: str, env_type: str,
    ...
    if os.getenv("OPENZUMA_YOLO_MODE") or is_current_session_yolo_enabled():
        return {"approved": True, "message": None}
    ...
    is_cli = os.getenv("OPENZUMA_INTERACTIVE")
    is_gateway = os.getenv("OPENZUMA_GATEWAY_SESSION")

    if not is_cli and not is_gateway:
        # Cron sessions: respect cron_mode config
        if os.getenv("OPENZUMA_CRON_SESSION"):
    ...
    if is_gateway or os.getenv("OPENZUMA_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    if os.getenv("OPENZUMA_YOLO_MODE") or is_current_session_yolo_enabled() or approval_mode == "off":
        return {"approved": True, "message": None}

    is_cli = os.getenv("OPENZUMA_INTERACTIVE")
    is_gateway = os.getenv("OPENZUMA_GATEWAY_SESSION")
    is_ask = os.getenv("OPENZUMA_EXEC_ASK")
