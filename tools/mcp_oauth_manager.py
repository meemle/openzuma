...
`mcp_tool.py`, and `openzuma_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# OpenzumaMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_openzuma_provider_class() -> Optional[type]:
    ...
    class OpenzumaMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(self, *args: Any, server_name: str = "", **kwargs: Any):
            ...
            self._openzuma_server_name = server_name
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`OpenzumaTokenStorage` persisting an absolute
            ...
            if (
                ...
                try:
                    ...
                    logger.debug(
                        ...
                        "failed (non-fatal): %s",
                        self._openzuma_server_name, exc,
                    )
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._openzuma_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        logger.debug(
                            ...
                            "token_endpoint=%s",
                            self._openzuma_server_name, asm.token_endpoint,
                        )
                        break
    ...
    return OpenzumaMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_OPENZUMA_PROVIDER_CLS: Optional[type] = _make_openzuma_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`OpenzumaMCPOAuthProvider` directly using the helpers
        ...
        Returns None if the MCP SDK's OAuth support is unavailable.
        """
        if _OPENZUMA_PROVIDER_CLS is None:
            logger.warning(
        ...
        storage = OpenzumaTokenStorage(server_name)
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``openzuma mcp remove <name>`` and (indirectly) by
        ``openzuma mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
