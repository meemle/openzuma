...
if tinker_atropos_dir.exists():
    ...
    os.environ['OPENZUMA_QUIET'] = '1'  # Disable temp subdirectory creation
    print(f"📂 Terminal working directory: {tinker_atropos_dir}")
else:
    # Fall back to openzuma-agent directory if submodule not found
...
from openzuma_constants import get_openzuma_home, OPENROUTER_BASE_URL

DEFAULT_MODEL = "anthropic/claude-opus-4.5"
DEFAULT_BASE_URL = OPENROUTER_BASE_URL


def load_openzuma_config() -> dict:
    """
...
def main(
    ...
    Args:
        ...
        model: Model to use for the agent (reads from ~/.openzuma/config.yaml if not provided)
    ...
    """
    # Load config from ~/.openzuma/config.yaml
    config = load_openzuma_config()
    ...
    if check_server:
        ...
        if ok:
            ...
            if missing:
                ...
                print("   Add them to ~/.openzuma/.env")
            else:
