...
describe('DEFAULT_THEME', () => {
  it('has brand defaults', () => {
    expect(DEFAULT_THEME.brand.name).toBe('Openzuma Agent')
...
describe('detectLightMode', () => {
  ...
  it('honors OPENZUMA_TUI_LIGHT on/off', () => {
    expect(detectLightMode({ OPENZUMA_TUI_LIGHT: '1' })).toBe(true)
    expect(detectLightMode({ OPENZUMA_TUI_LIGHT: 'true' })).toBe(true)
    expect(detectLightMode({ OPENZUMA_TUI_LIGHT: 'on' })).toBe(true)
    expect(detectLightMode({ OPENZUMA_TUI_LIGHT: '0' })).toBe(false)
    expect(detectLightMode({ OPENZUMA_TUI_LIGHT: 'off' })).toBe(false)
  })
  ...
  it('lets OPENZUMA_TUI_LIGHT=0 override a light COLORFGBG', () => {
    expect(detectLightMode({ COLORFGBG: '0;15', OPENZUMA_TUI_LIGHT: '0' })).toBe(false)
  })
})
