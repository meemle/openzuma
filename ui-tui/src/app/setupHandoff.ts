import type { RunExternalProcess } from '@openzuma/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`openzuma ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching openzuma: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`openzuma ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
