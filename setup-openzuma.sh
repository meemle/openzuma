...
# 5. Symlinks the 'openzuma' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Openzuma Agent Setup${NC}"
echo ""
...
# ============================================================================
# PATH setup — symlink openzuma into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up openzuma command..."

OPENZUMA_BIN="$SCRIPT_DIR/venv/bin/openzuma"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$OPENZUMA_BIN" "$COMMAND_LINK_DIR/openzuma"
echo -e "${GREEN}✓${NC} Symlinked openzuma → $COMMAND_LINK_DISPLAY_DIR/openzuma"
...
# ============================================================================
# Seed bundled skills into ~/.openzuma/skills/
# ============================================================================

OPENZUMA_SKILLS_DIR="${OPENZUMA_HOME:-$HOME/.openzuma}/skills"
mkdir -p "$OPENZUMA_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.openzuma/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$OPENZUMA_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  openzuma status        # Check configuration"
if is_termux; then
    echo "  openzuma gateway       # Run gateway in foreground"
else
    echo "  openzuma gateway install # Install gateway service (messaging + cron)"
fi
echo "  openzuma cron list     # View scheduled jobs"
echo "  openzuma doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m openzuma_cli.main setup
fi
