...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("OPENZUMA_HOME", os.path.expanduser("~/.openzuma")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("OPENZUMA_HOME", Path.home() / ".openzuma")) / "skills" / "red-teaming" / "godmode"
...
# ═══════════════════════════════════════════════════════════════════
# Openzuma config paths
# ═══════════════════════════════════════════════════════════════════

OPENZUMA_HOME = Path(os.getenv("OPENZUMA_HOME", Path.home() / ".openzuma"))
CONFIG_PATH = OPENZUMA_HOME / "config.yaml"
PREFILL_PATH = OPENZUMA_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Openzuma models — already uncensored, just needs clean prompt
    "openzuma": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "openzuma" in model_lower or "nous" in model_lower:
        return "openzuma"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Openzuma config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.openzuma/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Openzuma for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Openzuma for changes to take effect.")
