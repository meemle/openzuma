...
## 5. GODMODE FAST — Openzuma 4 405B (Zero Refusal)

**Model:** `nousresearch/openzuma-4-405b`
**Strategy:** Openzuma models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Openzuma Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.openzuma/config.yaml`:
