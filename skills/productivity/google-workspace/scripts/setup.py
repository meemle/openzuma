#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for Openzuma Agent.
...
try:
    from openzuma_constants import display_openzuma_home, get_openzuma_home
except ModuleNotFoundError:
    OPENZUMA_AGENT_ROOT = Path(__file__).resolve().parents[4]
    if OPENZUMA_AGENT_ROOT.exists():
        sys.path.insert(0, str(OPENZUMA_AGENT_ROOT))
    from openzuma_constants import display_openzuma_home, get_openzuma_home

OPENZUMA_HOME = get_openzuma_home()
TOKEN_PATH = OPENZUMA_HOME / "google_token.json"
CLIENT_SECRET_PATH = OPENZUMA_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = OPENZUMA_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same Openzuma profile to refresh consent."
    )
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to Openzuma home."""
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_openzuma_home()}/google_token.json")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for Openzuma")
