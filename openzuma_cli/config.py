"""
Configuration management for Openzuma Agent.

Config files are stored in ~/.openzuma/ for easy access:
- ~/.openzuma/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.openzuma/.env         - API keys and secrets

This module provides:
- openzuma config          - Show current configuration
- openzuma config edit     - Open config in editor
- openzuma config set      - Set a specific value
- openzuma config wizard   - Re-run setup wizard
"""
...
from openzuma_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("OPENZUMA_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_openzuma_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Openzuma is running in package-manager-managed mode.

    Two signals: the OPENZUMA_MANAGED env var (set by the systemd service),
    or a .managed marker file in OPENZUMA_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade openzuma-agent"
...
def recommended_update_command() -> str:
    ...
    return get_managed_update_command() or "openzuma update"


def format_managed_message(action: str = "modify this Openzuma installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("OPENZUMA_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Openzuma installation is managed by NixOS "
            f"(OPENZUMA_MANAGED={env_hint}).\n"
            "Edit services.openzuma-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Openzuma installation is managed by Homebrew "
            f"(OPENZUMA_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade openzuma-agent"
        )

    return (
        f"Cannot {action}: this Openzuma installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Openzuma."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from OPENZUMA_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, openzuma_bin
    or None if container mode is not active, we're already inside the
    container, or OPENZUMA_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("OPENZUMA_DEV") == "1":
        return None
    ...
    container_mode_file = get_openzuma_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "openzuma-agent")
    exec_user = info.get("exec_user", "openzuma")
    openzuma_bin = info.get("openzuma_bin", "/data/current-package/bin/openzuma")

    return {
        "backend": backend,
        "container_name": container_name,
        "exec_user": exec_user,
        "openzuma_bin": openzuma_bin,
    }
...
# Re-export from openzuma_constants — canonical definition lives there.
from openzuma_constants import get_openzuma_home  # noqa: F811,E402

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_openzuma_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_openzuma_home() / ".env"
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the openzuma group can
    share state with the gateway service.

    The mode can be overridden via the OPENZUMA_HOME_MODE environment variable
    (e.g. OPENZUMA_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse OPENZUMA_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("OPENZUMA_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    When Openzuma runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("OPENZUMA_CONTAINER") or os.environ.get("OPENZUMA_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set OPENZUMA_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into OPENZUMA_HOME if the user doesn't have one yet."""
...
def ensure_openzuma_home():
    """Ensure ~/.openzuma directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_openzuma_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_openzuma_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"OPENZUMA_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "terminal": {
        ...
        # skipped. When empty, Openzuma auto-appends ``~/.bashrc`` if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Openzuma sources ``~/.bashrc`` in the login
        ...
        # docker_env lets you specify exact key-value pairs — useful when Openzuma
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Openzuma sends a stable profile-scoped userId to Camofox
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.openzuma/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # always goes to ~/.openzuma/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${OPENZUMA_SKILL_DIR} and ${OPENZUMA_SESSION_ID} in SKILL.md
    ...
    # This section is only needed for openzuma-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Openzuma Agent*" header.
    ...
    # stored approval from ~/.openzuma/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or OPENZUMA_ACCEPT_HOOKS=1.
    ...
    "cron": {
        ...
        # Also overridable via OPENZUMA_CRON_MAX_PARALLEL env var.
    ...
    "code_execution": {
        ...
        #     openzuma-agent's own python (sys.executable). Maximum isolation
    ...
    # Logging — controls file logging to ~/.openzuma/logs/.
    ...
    # Session storage — controls automatic cleanup of ~/.openzuma/state.db.
    ...
    "sessions": {
        ...
        # default of ``openzuma sessions prune``.
        "retention_days": 90,
...
OPTIONAL_ENV_VARS = {
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Openzuma auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @openzuma:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. OPENZUMA_BOT)",
    ...
    "API_SERVER_PORT": {
        "description": "Port for the API server (default: 8742).",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'openzuma-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Openzuma API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Openzuma API server URL (e.g. http://192.168.1.100:8742)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Openzuma API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    "WEBHOOK_PORT": {
        "description": "Port for the webhook HTTP server (default: 8744).",
        "prompt": "Webhook port",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    # OPENZUMA_TOOL_PROGRESS and OPENZUMA_TOOL_PROGRESS_MODE are deprecated —
    # now configured via display.tool_progress in config.yaml (off|new|all|verbose).
    # Gateway falls back to these env vars for backward compatibility.
    "OPENZUMA_TOOL_PROGRESS": {
    ...
    "OPENZUMA_PREFILL_MESSAGES_FILE": {
    ...
    "OPENZUMA_EPHEMERAL_SYSTEM_PROMPT": {
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.openzuma.config`` entries, then checks
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'openzuma setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Openzuma won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'openzuma doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("OPENZUMA_HOME", "~/.openzuma")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("OPENZUMA_TOOL_PROGRESS")
            old_mode = get_env_value("OPENZUMA_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in ("false", "0", "no"):
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from OPENZUMA_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in ("new", "all"):
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from OPENZUMA_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("OPENZUMA_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from OPENZUMA_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$OPENZUMA_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_openzuma_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `openzuma plugins enable <name>` to activate."
                    )
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in ("y", "yes"):
                ...
                print("  Set later with: openzuma config set <key> <value>")
    ...
    # metadata.openzuma.config in their SKILL.md frontmatter.
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.openzuma/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.openzuma/config.yaml."""
...
#   openai-codex (OAuth — openzuma auth) — OpenAI Codex
#   nous         (OAuth — openzuma auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.openzuma/config.yaml."""
    if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.openzuma/.env.
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Openzuma env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.openzuma/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.openzuma/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.openzuma/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.openzuma/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Openzuma — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Openzuma vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.openzuma/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ Openzuma Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  openzuma config edit     # Edit config file", Colors.DIM))
    print(color("  openzuma config set <key> <value>", Colors.DIM))
    print(color("  openzuma setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: openzuma config set <key> <value>")
            print()
            print("Examples:")
            print("  openzuma config set model anthropic/claude-sonnet-4")
            print("  openzuma config set terminal.backend docker")
            print("  openzuma config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'openzuma config migrate' to add them")
        ...
        print("  openzuma config           Show current configuration")
        print("  openzuma config edit      Open config in editor")
        print("  openzuma config set <key> <value>   Set a config value")
        print("  openzuma config check     Check for missing/outdated config")
        print("  openzuma config migrate   Update config with new options")
        print("  openzuma config path      Show config file path")
        print("  openzuma config env-path  Show .env file path")
        sys.exit(1)
