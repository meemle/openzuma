"""``openzuma logs`` — view and filter Openzuma log files.
...
Usage examples::
    ...
    openzuma logs                    # last 50 lines of agent.log
    openzuma logs -f                 # follow agent.log in real time
    openzuma logs errors             # last 50 lines of errors.log
    openzuma logs gateway -n 100    # last 100 lines of gateway.log
    openzuma logs --level WARNING    # only WARNING+ lines
    openzuma logs --session abc123   # filter by session ID substring
    openzuma logs --component tools  # only tool-related lines
    openzuma logs --since 1h         # lines from the last hour
    openzuma logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_openzuma_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Openzuma runs — try 'openzuma chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_openzuma_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_openzuma_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_openzuma_home()}/logs/")
        return

    print(f"Log files in {display_openzuma_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'openzuma chat' to generate logs)")
