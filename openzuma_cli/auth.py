"""
Multi-provider authentication system for Openzuma Agent.

Supports OAuth device code flows (Nous Portal, future: OpenAI Codex) and
traditional API key providers (OpenRouter, custom endpoints). Auth state
is persisted in ~/.openzuma/auth.json with cross-process file locking.
...
from openzuma_cli.config import get_openzuma_home, get_config_path, read_raw_config
from openzuma_constants import OPENROUTER_BASE_URL
...
DEFAULT_NOUS_CLIENT_ID = "openzuma-cli"
...
def get_anthropic_key() -> str:
    ...
    """
    from openzuma_cli.config import get_env_value
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from openzuma_cli.copilot_auth import resolve_copilot_token
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `openzuma model` to re-authenticate."
...
def _oauth_trace_enabled() -> bool:
    raw = os.getenv("OPENZUMA_OAUTH_TRACE", "").strip().lower()
...
# =============================================================================
# Auth Store — persistence layer for ~/.openzuma/auth.json
# =============================================================================

def _auth_file_path() -> Path:
    return get_openzuma_home() / "auth.json"
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    # not by the user explicitly configuring anthropic in Openzuma.
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `openzuma logout`.
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    """
    try:
        from openzuma_cli.config import validate_config_structure
        issues = validate_config_structure()
        if not issues:
            return ""

        lines = ["Config issue detected — run 'openzuma doctor' for full diagnostics:"]
        for ci in issues:
    ...
    *,
    ...
    if normalized != "auto":
        ...
        if _config_hint:
            ...
            msg += " Check 'openzuma model' for available providers, or run 'openzuma doctor' to diagnose config issues."
    ...
    raise AuthError(
        "No inference provider configured. Run 'openzuma model' to choose a "
        "provider and model, or set an API key (OPENROUTER_API_KEY, "
        "OPENAI_API_KEY, etc.) in ~/.openzuma/.env.",
...
def resolve_qwen_runtime_credentials(
    *,
    ...
    base_url = os.getenv("OPENZUMA_QWEN_BASE_URL", "").strip().rstrip("/") or DEFAULT_QWEN_BASE_URL
    return {
        "provider": "qwen-oauth",
        "base_url": base_url,
...
#
# Tokens live in ~/.openzuma/auth/google_oauth.json (managed by agent.google_oauth).
...
def get_gemini_oauth_auth_status() -> Dict[str, Any]:
    """Return a status dict for `openzuma auth list` / `openzuma status`."""
    try:
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.openzuma/auth.json (not ~/.codex/)
#
# Openzuma maintains its own Codex OAuth session separate from the Codex CLI
# and VS Code extension. This prevents refresh token rotation conflicts
# where one app's refresh invalidates the other's session.
# =============================================================================

def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from Openzuma auth store (~/.openzuma/auth.json).
    ...
    if not state:
        raise AuthError(
            "No Codex credentials stored. Run `openzuma auth` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "Codex auth state is missing tokens. Run `openzuma auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "Codex auth is missing access_token. Run `openzuma auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "Codex auth is missing refresh_token. Run `openzuma auth` to re-authenticate.",
            provider="openai-codex",
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None) -> None:
    """Save Codex OAuth tokens to Openzuma auth store (~/.openzuma/auth.json)."""
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    """Refresh Codex OAuth tokens without mutating Openzuma auth state."""
    ...
    if response.status_code != 200:
        ...
        if code == "refresh_token_reused":
            message = (
                ...
                "then run `openzuma auth` to re-authenticate."
            )
            relogin_required = True
...
def _refresh_codex_auth_tokens(
    ...
    Saves the new tokens to Openzuma auth store automatically.
    """
...
def resolve_codex_runtime_credentials(
    *,
    ...
    """Resolve runtime credentials from Openzuma's own Codex token store."""
    data = _read_codex_tokens()
    tokens = dict(data["tokens"])
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = float(os.getenv("OPENZUMA_CODEX_REFRESH_TIMEOUT_SECONDS", "20"))

    should_refresh = bool(force_refresh)
    if (not should_refresh) and refresh_if_expiring:
        should_refresh = _codex_access_token_is_expiring(access_token, refresh_skew_seconds)
    if should_refresh:
        # Re-read under lock to avoid racing with other Openzuma processes
    ...
    base_url = (
        os.getenv("OPENZUMA_CODEX_BASE_URL", "").strip().rstrip("/")
...
def _resolve_verify(
    *,
    ...
    effective_ca = (
        ...
        or os.getenv("OPENZUMA_CA_BUNDLE")
    ...
    *,
    ...
    for item in data:
        ...
        if isinstance(model_id, str) and model_id.strip():
            mid = model_id.strip()
            # Skip Openzuma models — they're not reliable for agentic tool-calling
            if "openzuma" in mid.lower():
                continue
...
def resolve_nous_access_token(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "Openzuma is not logged into Nous Portal.",
                provider="nous",
                relogin_required=True,
            )
        ...
        portal_base_url = (
            ...
            or os.getenv("OPENZUMA_PORTAL_BASE_URL")
...
def refresh_nous_oauth_from_state(
    ...
    *,
    ...
    return refresh_nous_oauth_pure(
        ...
        state.get("client_id", "openzuma-cli"),
...
def persist_nous_credentials(
    ...
    *,
    ...
    Historically ``openzuma auth add nous`` wrote a ``manual:device_code`` pool
    ...
    ``openzuma auth add nous --label <name>``).  It gets embedded in the
...
def resolve_nous_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError("Openzuma is not logged into Nous Portal.",
...
def get_nous_auth_status() -> Dict[str, Any]:
    """Status snapshot for `openzuma status` output.

    Checks the credential pool first (where the dashboard device-code flow
    and ``openzuma auth`` store credentials), then falls back to the legacy
...
def get_codex_auth_status() -> Dict[str, Any]:
    ...
    Checks the credential pool first (where `openzuma auth` stores credentials),
    then falls back to the legacy provider state.
    """
    # Check credential pool first — this is where `openzuma auth` and
    # `openzuma model` store device_code tokens.
    try:
        from agent.credential_pool import load_pool
...
def get_external_process_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    command = (
        os.getenv("OPENZUMA_COPILOT_ACP_COMMAND", "").strip()
        or os.getenv("COPILOT_CLI_PATH", "").strip()
        or "copilot"
    )
    raw_args = os.getenv("OPENZUMA_COPILOT_ACP_ARGS", "").strip()
...
def resolve_external_process_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if not resolved_command and not base_url.startswith("acp+tcp://"):
        raise AuthError(
            ...
            "Install GitHub Copilot CLI or set OPENZUMA_COPILOT_ACP_COMMAND/COPILOT_CLI_PATH.",
            provider=provider_id,
...
def _prompt_model_selection(
    ...
    """
    from openzuma_cli.models import _format_price_per_mtok
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from openzuma_cli.config import save_config, load_config
...
def login_command(args) -> None:
    """Deprecated: use 'openzuma model' or 'openzuma setup' instead."""
    print("The 'openzuma login' command has been removed.")
    print("Use 'openzuma auth' to manage credentials,")
    print("'openzuma model' to select a provider, or 'openzuma setup' for full setup.")
    raise SystemExit(0)


def _login_openai_codex(args, pconfig: ProviderConfig) -> None:
    """OpenAI Codex login via device code flow. Tokens stored in ~/.openzuma/auth.json."""

    # Check for existing Openzuma-owned credentials
    try:
        ...
        if isinstance(_resolved_key, str) and _resolved_key and not _codex_access_token_is_expiring(_resolved_key, 60):
            print("Existing Codex credentials found in Openzuma auth store.")
            try:
    ...
    if cli_tokens:
        ...
        print("Openzuma will create its own session to avoid conflicts with Codex CLI / VS Code.")
        try:
        ...
        if do_import in ("y", "yes"):
            ...
            base_url = os.getenv("OPENZUMA_CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
            config_path = _update_config_for_provider("openai-codex", base_url)
            print()
            print("Credentials imported. Note: if Codex CLI refreshes its token,")
            print("Openzuma will keep working independently with its own session.")
            print(f"  Config updated: {config_path} (model.provider=openai-codex)")
            return

    # Run a fresh device code flow — Openzuma gets its own OAuth session
    print()
    print("Signing in to OpenAI Codex...")
    print("(Openzuma creates its own session — won't affect Codex CLI or VS Code)")
    print()

    creds = _codex_device_code_login()

    # Save tokens to Openzuma auth store
...
def _nous_device_code_login(
    *,
    ...
    print(f"Starting Openzuma login via {pconfig.name}...")
    ...
    try:
        ...
        if exc.code == "subscription_required":
            ...
            print()
            print("After subscribing, run `openzuma model` again to finish setup.")
            raise SystemExit(1)
        raise
...
def _login_nous(args, pconfig: ProviderConfig) -> None:
    ...
    try:
        ...
        if not selected_model:
            ...
            print("  Run `openzuma model` again to switch to Nous Portal.")
            return
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target):
        ...
        if os.getenv("OPENROUTER_API_KEY"):
            print("Openzuma will use OpenRouter for inference.")
        else:
            print("Run `openzuma model` or configure an API key to use Openzuma.")
    else:
