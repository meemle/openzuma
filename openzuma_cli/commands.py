"""Slash command definitions and autocomplete for the Openzuma CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of Openzuma config/state", "Session",
    ...
    CommandDef("update", "Update Openzuma Agent to the latest version", "Info",
               gateway_only=True),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`openzuma_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/openzuma`` subcommand mapping, and
    ...
    """
    try:
        from openzuma_cli.plugins import get_plugin_commands
    except Exception:
        return []
    try:
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``openzuma skills
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /openzuma handler.

    Maps both canonical names and aliases so /openzuma bg do stuff works
    the same as /openzuma background do stuff.

    Plugin-registered slash commands are included so ``/openzuma <plugin-cmd>``
