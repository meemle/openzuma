"""
Openzuma Agent — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m openzuma_cli.main web          # Start on http://127.0.0.1:9219
    python -m openzuma_cli.main web --port 8080
"""
...
WEB_DIST = Path(os.environ["OPENZUMA_WEB_DIST"]) if "OPENZUMA_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
_log = logging.getLogger(__name__)

app = FastAPI(title="Openzuma Agent", version=__version__)
...
def _probe_gateway_health() -> tuple[bool, dict | None]:
    ...
    - ``http://gateway:8742``                (base URL — recommended)
    - ``http://gateway:8742/health``         (explicit health path)
    - ``http://gateway:8742/health/detailed`` (explicit detailed path)
...
# streamed to a per-action log file under ``~/.openzuma/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_openzuma_home() / "logs"

# Short ``name`` (from the URL) → absolute log file path.
_ACTION_LOG_FILES: Dict[str, str] = {
    "gateway-restart": "gateway-restart.log",
    "openzuma-update": "openzuma-update.log",
}
...
def _spawn_openzuma_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``openzuma <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``openzuma_cli.main`` module so the action
    ...
    cmd = [sys.executable, "-m", "openzuma_cli.main", *subcommand]
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "OPENZUMA_NONINTERACTIVE": "1"},
    }
...
async def restart_gateway():
    """Kick off a ``openzuma gateway restart`` in the background."""
    try:
        proc = _spawn_openzuma_action(["gateway", "restart"], "gateway-restart")
...
@app.post("/api/openzuma/update")
async def update_openzuma():
    """Kick off ``openzuma update`` in the background."""
    try:
        proc = _spawn_openzuma_action(["update"], "openzuma-update")
    except Exception as exc:
        _log.exception("Failed to spawn openzuma update")
        raise HTTPException(status_code=500, detail=f"Failed to start update: {exc}")
    return {
        "ok": True,
        "pid": proc.pid,
        "name": "openzuma-update",
    }
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Openzuma supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
# the canonical ``openzuma auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    Openzuma resolves Anthropic creds in this order at runtime:
    1. ``~/.openzuma/.anthropic_oauth.json`` — Openzuma-managed PKCE flow
    ...
    try:
        from agent.anthropic_adapter import (
            read_openzuma_oauth_credentials,
            read_claude_code_credentials,
            _OPENZUMA_OAUTH_FILE,
        )
    except ImportError:
        read_claude_code_credentials = None  # type: ignore
        read_openzuma_oauth_credentials = None  # type: ignore
        _OPENZUMA_OAUTH_FILE = None  # type: ignore

    openzuma_creds = None
    if read_openzuma_oauth_credentials:
        try:
            openzuma_creds = read_openzuma_oauth_credentials()
        except Exception:
            openzuma_creds = None
    if openzuma_creds and openzuma_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "openzuma_pkce",
            "source_label": f"Openzuma PKCE ({_OPENZUMA_OAUTH_FILE})",
            "token_preview": _truncate_token(openzuma_creds.get("accessToken")),
            "expires_at": openzuma_creds.get("expiresAt"),
            "has_refresh_token": bool(openzuma_creds.get("refreshToken")),
        }
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Openzuma even
    when they also have a separate Openzuma-managed PKCE login.
    """
    try:
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "flow": "pkce",
        "cli_command": "openzuma auth add anthropic",
    ...
    },
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": "openzuma auth add nous",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "openzuma auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "openzuma auth add qwen-oauth",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from openzuma_cli import auth as hauth
        if provider_id == "nous":
...
async def list_oauth_providers():
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("openzuma_pkce", "claude_code", ...)
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    # Anthropic and claude-code clear the same Openzuma-managed PKCE file
    # AND forget the Claude Code import. We don't touch ~/.claude/* directly
    # — that's owned by the Claude Code CLI; users can re-auth there if they
    # want to undo a disconnect.
    if provider_id in ("anthropic", "claude-code"):
        try:
            from agent.anthropic_adapter import _OPENZUMA_OAUTH_FILE
            if _OPENZUMA_OAUTH_FILE.exists():
                _OPENZUMA_OAUTH_FILE.unlink()
        except Exception:
            pass
        # Also clear the credential pool entry if present.
        try:
            from openzuma_cli.auth import clear_provider_auth
...
#          → persists to ~/.openzuma/.anthropic_oauth.json AND credential pool
...
# Guarded so openzuma web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Openzuma file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``openzuma auth add anthropic``.
    """
    from agent.anthropic_adapter import _OPENZUMA_OAUTH_FILE
    payload = {
    ...
    _OPENZUMA_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    _OPENZUMA_OAUTH_FILE.write_text(json.dumps(payload, indent=2), encoding="utf-8")
...
def _submit_anthropic_pkce(session_id: str, code_input: str) -> Dict[str, Any]:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "openzuma-dashboard/1.0",
        },
        method="POST",
    )
    try:
...
async def _start_device_code_flow(provider_id: str) -> Dict[str, Any]:
    ...
    from openzuma_cli import auth as hauth
    if provider_id == "nous":
        from openzuma_cli.auth import _request_device_code, PROVIDER_REGISTRY
        import httpx
        pconfig = PROVIDER_REGISTRY["nous"]
        portal_base_url = (
            os.getenv("OPENZUMA_PORTAL_BASE_URL")
...
def _nous_poller(session_id: str) -> None:
    ...
    from openzuma_cli.auth import _poll_for_token, refresh_nous_oauth_from_state
    ...
    try:
        ...
        from openzuma_cli.auth import persist_nous_credentials
...
async def get_logs(
    ...
    from openzuma_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_openzuma_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from openzuma_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
async def get_skills():
    ...
    from openzuma_cli.skills_config import get_disabled_skills
    config = load_config()
...
async def toggle_skill(body: SkillToggle):
    from openzuma_cli.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
def mount_spa(application: FastAPI):
    ...
    def _serve_index():
        ...
        token_script = (
            f'<script>window.__OPENZUMA_SESSION_TOKEN__="{_SESSION_TOKEN}";</script>'
        )
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",   "label": "Openzuma Teal",  "description": "Classic dark teal — the canonical Openzuma look"},
...
def _discover_user_themes() -> list:
    """Scan ~/.openzuma/dashboard-themes/*.yaml for user-created themes."""
    themes_dir = get_openzuma_home() / "dashboard-themes"
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as openzuma_cli.plugins):
    1. User plugins:    ~/.openzuma/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.openzuma/plugins/  (only if OPENZUMA_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    search_dirs = [
        (get_openzuma_home() / "plugins", "user"),
        (PROJECT_ROOT / "plugins" / "memory", "bundled"),
        (PROJECT_ROOT / "plugins", "bundled"),
    ]
    if os.environ.get("OPENZUMA_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".openzuma" / "plugins", "project"))
    ...
    for plugin in _get_dashboard_plugins():
        ...
        try:
            spec = importlib.util.spec_from_file_location(
                f"openzuma_dashboard_plugin_{plugin['name']}", api_path,
            )
...
def start_server(
    ...
    print(f"  Openzuma Web UI → http://{host}:{port}")
