"""
Dump command for openzuma CLI.

Outputs a compact, plain-text summary of the user's Openzuma setup
...
from openzuma_cli.config import get_openzuma_home, get_env_path, get_project_root, load_config
...
def _gateway_status() -> str:
    ...
    try:
        from openzuma_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(openzuma_home: Path) -> int:
...
def _cron_summary(openzuma_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = openzuma_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"openzuma_home:      {display_openzuma_home()}")
    ...
    toolsets = config.get("toolsets", ["openzuma-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(openzuma_home)}")
    lines.append(f"  skills:             {_count_skills(openzuma_home)}")
