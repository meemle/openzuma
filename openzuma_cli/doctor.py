"""
Doctor command for openzuma CLI.

Diagnoses issues with Openzuma Agent setup.
"""
...
from openzuma_cli.config import get_project_root, get_openzuma_home, get_env_path
from openzuma_constants import display_openzuma_home

PROJECT_ROOT = get_project_root()
OPENZUMA_HOME = get_openzuma_home()
_DHH = display_openzuma_home()  # user-facing display path (e.g. ~/.openzuma or ~/.openzuma/profiles/coder)

# Load environment variables from ~/.openzuma/.env so API key checks work
...
from openzuma_constants import OPENROUTER_MODELS_URL
...
from openzuma_constants import is_termux as _is_termux
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.openzuma/.env contains provider auth/base URL settings."""
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `openzuma`.
    os.environ.setdefault("OPENZUMA_INTERACTIVE", "1")
    ...
    print(color("│                 🩺 Openzuma Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    # Check ~/.openzuma/.env (primary location for user config)
    env_path = OPENZUMA_HOME / '.env'
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'openzuma setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'openzuma setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'openzuma setup' to create one")
                issues.append("Run 'openzuma setup' to create .env")
    
    # Check ~/.openzuma/config.yaml (primary) or project cli-config.yaml (fallback)
    config_path = OPENZUMA_HOME / 'config.yaml'
    if config_path.exists():
        ...
        try:
            ...
            if provider and provider != "auto":
                if canonical_provider is None or (known_providers and canonical_provider not in known_providers):
                    ...
                    issues.append(
                        ...
                        f"Fix: run 'openzuma config set model.provider <valid_provider>'"
                    )
            ...
            if canonical_provider and canonical_provider not in ("auto", "custom", "openrouter"):
                try:
                    from openzuma_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if pconfig and getattr(pconfig, "auth_type", "") == "api_key":
                        ...
                        if not configured:
                            check_fail(
                                ...
                                "(check ~/.openzuma/.env or run 'openzuma setup')",
                            )
                            issues.append(
                                f"No credentials found for provider '{canonical_provider}'. "
                                f"Run 'openzuma setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'openzuma config set model.provider <name>'"
                            )
                except Exception:
                    pass
    ...
    if config_path.exists():
        try:
            from openzuma_cli.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'openzuma setup' to migrate config")
                else:
                    issues.append("Run 'openzuma doctor --fix' or 'openzuma setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'openzuma doctor --fix'")
        except Exception:
            pass
    ...
    openzuma_home = OPENZUMA_HOME
    if openzuma_home.exists():
        check_ok(f"{_DHH} directory exists")
    else:
        if should_fix:
            openzuma_home.mkdir(parents=True, exist_ok=True)
    ...
    for subdir_name in expected_subdirs:
        subdir_path = openzuma_home / subdir_name
    ...
    soul_path = openzuma_home / "SOUL.md"
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Openzuma a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Openzuma Agent Persona\n\n"
                "<!-- Edit this file to customize how Openzuma communicates. -->\n\n"
                "You are Openzuma, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    memories_dir = openzuma_home / "memories"
    ...
    state_db_path = openzuma_home / "state.db"
    ...
    wal_path = openzuma_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'openzuma doctor --fix' to checkpoint")
    ...
    # =========================================================================
    # Check: Command installation (openzuma bin symlink)
    # =========================================================================
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "openzuma"
        ...
        _cmd_link = _cmd_link_dir / "openzuma"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(openzuma not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/openzuma → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/openzuma points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/openzuma → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/openzuma — run 'openzuma doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/openzuma exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/openzuma not found",
                    "(openzuma command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/openzuma → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/openzuma symlink — run 'openzuma doctor --fix'")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                issues.append(
                    ...
                    "Fix: run 'openzuma config set model.provider <provider>' to switch providers, "
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'openzuma setup' to configure missing API keys for full tool access")
    except Exception as e:
    ...
    hub_dir = OPENZUMA_HOME / "skills" / ".hub"
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: openzuma skills list)")
    ...
    try:
        import yaml as _yaml
        _mem_cfg_path = OPENZUMA_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                check_warn("Honcho config not found", "run: openzuma memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                check_fail("Honcho API key or base URL not set", "run: openzuma memory setup")
                issues.append("No Honcho API key — run 'openzuma memory setup'")
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                check_fail("Mem0 API key not set", "(set MEM0_API_KEY in .env or run openzuma memory setup)")
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: openzuma memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: openzuma memory setup")
        except Exception as _e:
    ...
    # =========================================================================
    try:
        from openzuma_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
            ...
            if wrapper_dir.is_dir():
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "openzuma -p" in content:
                            _m = _re.search(r"openzuma -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'openzuma doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
