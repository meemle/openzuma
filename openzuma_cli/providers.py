"""
Single source of truth for provider identity in Openzuma Agent.
...
2. **Openzuma overlays** — transport type, auth patterns, aggregator flags,
...
# -- Openzuma overlay ----------------------------------------------------------
# Openzuma-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class OpenzumaOverlay:
    """Openzuma-specific provider metadata layered on top of models.dev."""
...
OPENZUMA_OVERLAYS: Dict[str, OpenzumaOverlay] = {
    "openrouter": OpenzumaOverlay(
        transport="openai_chat",
        is_aggregator=True,
        extra_env_vars=("OPENAI_API_KEY",),
        base_url_env_var="OPENROUTER_BASE_URL",
    ),
    "nous": OpenzumaOverlay(
        transport="openai_chat",
        auth_type="oauth_device_code",
        base_url_override="https://inference-api.nousresearch.com/v1",
    ),
    "openai-codex": OpenzumaOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "qwen-oauth": OpenzumaOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="OPENZUMA_QWEN_BASE_URL",
    ),
    "google-gemini-cli": OpenzumaOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="cloudcode-pa://google",
    ),
    "copilot-acp": OpenzumaOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": OpenzumaOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": OpenzumaOverlay(
    ...
    "kimi-for-coding": OpenzumaOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "stepfun": OpenzumaOverlay(
        transport="openai_chat",
        extra_env_vars=("STEPFUN_API_KEY",),
        base_url_override="https://api.stepfun.ai/step_plan/v1",
        base_url_env_var="STEPFUN_BASE_URL",
    ),
    "minimax": OpenzumaOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-cn": OpenzumaOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": OpenzumaOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": OpenzumaOverlay(
        transport="openai_chat",
        base_url_env_var="DASHSCOPE_BASE_URL",
    ),
    "vercel": OpenzumaOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ),
    "opencode": OpenzumaOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": OpenzumaOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_GO_BASE_URL",
    ),
    "kilo": OpenzumaOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="KILOCODE_BASE_URL",
    ),
    "huggingface": OpenzumaOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "nvidia": OpenzumaOverlay(
        transport="openai_chat",
        base_url_override="https://integrate.api.nvidia.com/v1",
        base_url_env_var="NVIDIA_BASE_URL",
    ),
    "xiaomi": OpenzumaOverlay(
        transport="openai_chat",
        base_url_env_var="XIAOMI_BASE_URL",
    ),
    "arcee": OpenzumaOverlay(
        transport="openai_chat",
        base_url_override="https://api.arcee.ai/api/v1",
        base_url_env_var="ARCEE_BASE_URL",
    ),
    "ollama-cloud": OpenzumaOverlay(
        transport="openai_chat",
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "openzuma", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Openzuma overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Openzuma overlay
    ...
    overlay = OPENZUMA_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + openzuma extra
    ...
    if overlay is not None:
        # Openzuma-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "") -> str:
    ...
    # Direct provider checks for providers not in OPENZUMA_OVERLAYS
    if provider == "bedrock":
