"""
Profile management for multiple isolated Openzuma instances.

Each profile is a fully independent OPENZUMA_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.openzuma/profiles/<name>/`` by default.

The "default" profile is ``~/.openzuma`` itself — backward compatible,
zero migration needed.

Usage::

    openzuma profile create coder          # fresh profile + bundled skills
    openzuma profile create coder --clone  # also copy config, .env, SOUL.md
    openzuma profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    openzuma -p coder chat                 # or via flag
    openzuma profile use coder             # set as sticky default
    openzuma profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See openzuma_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
# Directories/files to exclude when exporting the default (~/.openzuma) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "openzuma-agent",         # repo checkout (multi-GB)
...
_RESERVED_NAMES = frozenset({
    "openzuma", "default", "test", "tmp", "root", "sudo",
})

# Openzuma subcommands that cannot be used as profile names/aliases
_OPENZUMA_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the openzuma root, NOT to the current OPENZUMA_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where OPENZUMA_HOME points outside
    ``~/.openzuma``, profiles live under ``OPENZUMA_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_openzuma_home() / "profiles"


def _get_default_openzuma_home() -> Path:
    """Return the default (pre-profile) OPENZUMA_HOME path.

    In standard deployments this is ``~/.openzuma``.
    In Docker/custom deployments where OPENZUMA_HOME is outside ``~/.openzuma``
    (e.g. ``/opt/data``), returns OPENZUMA_HOME directly.
    """
    from openzuma_constants import get_default_openzuma_root
    return get_default_openzuma_root()


def _get_active_profile_path() -> Path:
    """Return the path to the sticky active_profile file."""
    return _get_default_openzuma_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name == "default":
        return  # special alias for ~/.openzuma
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its OPENZUMA_HOME directory."""
    if name == "default":
        return _get_default_openzuma_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, openzuma subcommands, existing binaries in PATH.
    """
    if name in _RESERVED_NAMES:
        return f"'{name}' is a reserved name"
    if name in _OPENZUMA_SUBCOMMANDS:
        return f"'{name}' conflicts with a openzuma subcommand"
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_openzuma_home()
...
def create_profile(
    ...
    if name == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.openzuma)."
        )
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches OPENZUMA_HOME at module level.
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "OPENZUMA_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if name == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.openzuma).\n"
            "To remove everything, use: openzuma uninstall"
        )
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set OPENZUMA_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("OPENZUMA_HOME")
    try:
        os.environ["OPENZUMA_HOME"] = str(profile_dir)
        from openzuma_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["OPENZUMA_HOME"] = old_home
        elif "OPENZUMA_HOME" in os.environ:
            del os.environ["OPENZUMA_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.openzuma/active_profile``. Use ``"default"`` to clear.
    """
    validate_profile_name(name)
    if name != "default" and not profile_exists(name):
        raise FileNotFoundError(
            f"Profile '{name}' does not exist. "
            f"Create it with: openzuma profile create {name}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from OPENZUMA_HOME.

    Returns ``"default"`` if OPENZUMA_HOME is not set or points to ``~/.openzuma``.
    Returns the profile name if OPENZUMA_HOME points into ``~/.openzuma/profiles/<name>``.
    Returns ``"custom"`` if OPENZUMA_HOME is set to an unrecognized path.
    """
    from openzuma_constants import get_openzuma_home
    openzuma_home = get_openzuma_home()
    resolved = openzuma_home.resolve()

    default_resolved = _get_default_openzuma_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if name == "default":
        # The default profile IS ~/.openzuma itself — its parent is ~/ and its
        # directory name is ".openzuma", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: openzuma profile import <archive> --name <name>"
        )

    # Archives exported from the default profile have "default/" as top-level
    # dir.  Importing as "default" would target ~/.openzuma itself — disallow
    # that and guide the user toward a named profile.
    if inferred_name == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.openzuma). "
            "Specify a different name: openzuma profile import <archive> --name <name>"
        )
...
def generate_bash_completion() -> str:
    """Generate a bash completion script for openzuma profile names."""
    return '''# Openzuma Agent profile completion
# Add to ~/.bashrc: eval "$(openzuma completion bash)"

_openzuma_profiles() {
    local profiles_dir="$HOME/.openzuma/profiles"
...
_openzuma_completion() {
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_openzuma_profiles)" -- "$cur"))
        return
    fi
...
complete -F _openzuma_completion openzuma
'''


def generate_zsh_completion() -> str:
    """Generate a zsh completion script for openzuma profile names."""
    return '''#compdef openzuma
# Openzuma Agent profile completion
# Add to ~/.zshrc: eval "$(openzuma completion zsh)"

_openzuma() {
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.openzuma/profiles" ]]; then
        profiles+=("${(@f)$(ls $HOME/.openzuma/profiles 2>/dev/null)}")
    fi
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a OPENZUMA_HOME path string.

    Called early in the CLI entry point, before any openzuma modules
    are imported, to set the OPENZUMA_HOME environment variable.
    """
    ...
    if profile_name != "default" and not profile_dir.is_dir():
        raise FileNotFoundError(
            ...
            f"Create it with: openzuma profile create {profile_name}"
        )
