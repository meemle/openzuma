"""openzuma hooks — inspect and manage shell-script hooks.

Usage::

    openzuma hooks list
    openzuma hooks test <event> [--for-tool X] [--payload-file F]
    openzuma hooks revoke <command>
    openzuma hooks doctor

Consent records live under ``~/.openzuma/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.openzuma/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``openzuma hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: openzuma hooks {list|test|revoke|doctor}")
        print("Run 'openzuma hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.openzuma/config.yaml.")
        print("See `openzuma hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `openzuma hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `openzuma hooks test` and `openzuma hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Openzuma wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `openzuma hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `openzuma hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `openzuma hooks doctor`.")
