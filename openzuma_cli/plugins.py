"""
Openzuma Plugin System
====================

Discovers, loads, and manages plugins from four sources:

1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with openzuma-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.openzuma/plugins/<name>/``
3. **Project plugins** – ``./.openzuma/plugins/<name>/`` (opt-in via
   ``OPENZUMA_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``openzuma_agent.plugins``
   entry-point group.
...
ENTRY_POINTS_GROUP = "openzuma_agent.plugins"

_NS_PARENT = "openzuma_plugins"
...
class PluginManifest:
    ...
    # lookups and by ``openzuma plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``openzuma honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``openzuma <subcommand>``
    ...
    def register_skill(
        ...
        ``~/.openzuma/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def discover_and_load(self) -> None:
        ...
        #
        # Repo-shipped plugins live next to openzuma_cli/. Two layouts are
        ...
        # 2. User plugins (~/.openzuma/plugins/)
        user_dir = get_openzuma_home() / "plugins"
        manifests.extend(self._scan_directory(user_dir, source="user"))

        # 3. Project plugins (./.openzuma/plugins/)
        if _env_enabled("OPENZUMA_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".openzuma" / "plugins"
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with openzuma and must
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `openzuma plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``openzuma_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``openzuma_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``openzuma tools`` TUI so plugin-provided toolsets appear
