#!/usr/bin/env python3
"""
Openzuma CLI - Main entry point.

Usage:
    openzuma                     # Interactive chat (default)
    openzuma chat                # Interactive chat
    openzuma gateway             # Run gateway in foreground
    openzuma gateway start       # Start gateway as service
    openzuma gateway stop        # Stop gateway service
    openzuma gateway status      # Show gateway status
    openzuma gateway install     # Install gateway service
    openzuma gateway uninstall   # Uninstall gateway service
    openzuma setup               # Interactive setup wizard
    openzuma logout              # Clear stored authentication
    openzuma status              # Show status of all components
    openzuma cron                # Manage cron jobs
    openzuma cron list           # List cron jobs
    openzuma cron status         # Check if cron scheduler is running
    openzuma doctor              # Check configuration and dependencies
    openzuma honcho setup                    # Configure Honcho AI memory integration
    openzuma honcho status                   # Show Honcho config and connection status
    openzuma honcho sessions                 # List directory → session name mappings
    openzuma honcho map <name>               # Map current directory to a session name
    openzuma honcho peer                     # Show peer names and dialectic settings
    openzuma honcho peer --user NAME         # Set user peer name
    openzuma honcho peer --ai NAME           # Set AI peer name
    openzuma honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    openzuma honcho mode                     # Show current memory mode
    openzuma honcho mode [hybrid|honcho|local]  # Set memory mode
    openzuma honcho tokens                   # Show token budget settings
    openzuma honcho tokens --context N       # Set session.context() token cap
    openzuma honcho tokens --dialectic N     # Set dialectic result char cap
    openzuma honcho identity                 # Show AI peer identity representation
    openzuma honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    openzuma honcho migrate                  # Step-by-step migration guide: OpenClaw native → Openzuma + Honcho
    openzuma version             Show version
    openzuma update              Update to latest version
    openzuma uninstall           Uninstall Openzuma Agent
    openzuma acp                 Run as an ACP server for editor integration
    openzuma sessions browse     Interactive session picker with search

    openzuma claw migrate --dry-run  # Preview migration without changes
"""
...
def _add_accept_hooks_flag(parser) -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(equivalent to OPENZUMA_ACCEPT_HOOKS=1 / hooks_auto_accept: true)."
        ),
    )
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (openzuma tools, openzuma setup, openzuma model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'openzuma {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any openzuma module import.
#
# Many modules cache OPENZUMA_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("OPENZUMA_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.openzuma/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set OPENZUMA_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # 2. If no flag, check active_profile in the openzuma root
    if profile_name is None:
        try:
            from openzuma_constants import get_default_openzuma_root

            active_path = get_default_openzuma_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set OPENZUMA_HOME
    if profile_name is not None:
        try:
            from openzuma_cli.profiles import resolve_profile_env

            openzuma_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent openzuma from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["OPENZUMA_HOME"] = openzuma_home
...
# Load .env from ~/.openzuma/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from openzuma_cli.config import get_openzuma_home
from openzuma_cli.env_loader import load_openzuma_dotenv

load_openzuma_dotenv(project_env=PROJECT_ROOT / ".env")

# Initialize centralized file logging early — all `openzuma` subcommands
# (chat, setup, gateway, config, etc.) write to agent.log + errors.log.
try:
    from openzuma_logging import setup_logging as _setup_logging
...
try:
    from openzuma_cli.config import load_config as _load_config_early
    from openzuma_constants import apply_ipv4_preference as _apply_ipv4
...
from openzuma_constants import AI_GATEWAY_BASE_URL, OPENROUTER_BASE_URL
...
def _has_any_provider_configured() -> bool:
    ...
    from openzuma_cli.config import get_env_path, get_openzuma_home, load_config
    from openzuma_cli.auth import get_auth_status

    # Determine whether Openzuma itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from openzuma_cli.config import DEFAULT_CONFIG
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_openzuma_config = _model_name and _model_name != _DEFAULT_MODEL

    # Check env vars (may be set by .env or shell).
    # OPENAI_BASE_URL alone counts — local models (vLLM, llama.cpp, etc.)
    # often don't require an API key.
    from openzuma_cli.auth import PROVIDER_REGISTRY
    ...
    auth_file = get_openzuma_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Openzuma has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Openzuma to use their tokens.
    if _has_openzuma_config:
        try:
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, openzuma_bin
        cli_args: the original CLI arguments (everything after 'openzuma')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    openzuma_bin = container_info["openzuma_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo openzuma {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo openzuma {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(session_id: Optional[str]) -> None:
    ...
    print(f"  openzuma --tui --resume {target}")
    if title:
        print(f'  openzuma --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @openzuma/ink is missing or node_modules is behind package-lock.json (post-pull)."""
    ink = root / "node_modules" / "@openzuma" / "ink" / "package.json"
    if not ink.is_file():
        return True
...
def _find_bundled_tui(tui_dir: Path) -> Optional[Path]:
    """Directory whose dist/entry.js we should run: OPENZUMA_TUI_DIR first, else repo ui-tui."""
    env = os.environ.get("OPENZUMA_TUI_DIR")
    if env:
        p = Path(env)
...
def _openzuma_ink_bundle_stale(tui_dir: Path) -> bool:
    ink_root = tui_dir / "packages" / "openzuma-ink"
...
def _ensure_tui_node() -> None:
    ...
    ``OPENZUMA_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("OPENZUMA_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    openzuma_home = os.environ.get("OPENZUMA_HOME") or str(Path.home() / ".openzuma")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "OPENZUMA_HOME": openzuma_home},
            capture_output=True,
            text=True,
            check=False,
        )
    ...
    extras.extend([Path(openzuma_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (OPENZUMA_TUI_DIR or ui-tui, build when stale)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("OPENZUMA_NODE")
    ...
    # pre-built dist + node_modules (nix / full OPENZUMA_TUI_DIR) skips npm.
    if not tui_dev:
        ext_dir = os.environ.get("OPENZUMA_TUI_DIR")
        if ext_dir:
            p = Path(ext_dir)
            if (p / "dist" / "entry.js").exists() and not _tui_need_npm_install(p):
                node = _node_bin("node")
    ...
    if _tui_need_npm_install(tui_dir):
        if not os.environ.get("OPENZUMA_QUIET"):
    ...
    if tui_dev:
        if _openzuma_ink_bundle_stale(tui_dir):
            result = subprocess.run(
                [npm, "run", "build", "--prefix", "packages/openzuma-ink"],
                cwd=str(tui_dir),
                capture_output=True,
                text=True,
            )
            if result.returncode != 0:
                ...
                print("@openzuma/ink build failed.")
                if preview:
                    print(preview)
                sys.exit(1)
...
def _launch_tui(resume_session_id: Optional[str] = None, tui_dev: bool = False):
    ...
    env = os.environ.copy()
    env["OPENZUMA_PYTHON_SRC_ROOT"] = os.environ.get(
        "OPENZUMA_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("OPENZUMA_PYTHON", sys.executable)
    env.setdefault("OPENZUMA_CWD", os.getcwd())
    ...
    if resume_session_id:
        env["OPENZUMA_TUI_RESUME"] = resume_session_id
...
def cmd_chat(args):
    ...
    use_tui = getattr(args, "tui", False) or os.environ.get("OPENZUMA_TUI") == "1"
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'openzuma sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Openzuma isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  openzuma setup")
        print()
        ...
        if reply in ("", "y", "yes"):
            ...
            return
        print()
        print("You can run 'openzuma setup' at any time to configure.")
        sys.exit(1)
    ...
    # user's ~/.openzuma/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["OPENZUMA_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["OPENZUMA_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["OPENZUMA_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from openzuma_cli.gateway import gateway_command
...
def cmd_whatsapp(args):
    ...
    from openzuma_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Openzuma?")
        print()
    ...
    session_dir = get_openzuma_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in ("y", "yes"):
            ...
            print("  Start the gateway with: openzuma gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  openzuma gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Openzuma Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  openzuma gateway")
        ...
        print()
        print("  Or install as a service: openzuma gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'openzuma whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    from openzuma_cli.setup import run_setup_wizard
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``openzuma model``) and the setup wizard
    (``setup_model_provider`` in setup.py).  Handles the full flow:
    provider picker, credential prompting, model selection, and config
    persistence.
    """
    from openzuma_cli.auth import (
        resolve_provider,
        AuthError,
        format_auth_error,
    )
    ...
    effective_provider = (
        config_provider or os.getenv("OPENZUMA_INFERENCE_PROVIDER") or "auto"
    )
    try:
    ...
    from openzuma_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    # a leftover OPENAI_BASE_URL in ~/.openzuma/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.openzuma/.env if the active provider is not 'custom'.
    ...
    """
    from openzuma_cli.config import get_env_value, save_env_value, load_config
...
#
# Openzuma uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `openzuma model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `openzuma model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Openzuma only falls back to a lightweight backend (OpenRouter,")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``openzuma model`` flow, then route aux tasks to them here.
    """
    from openzuma_cli.config import load_config
...
def _aux_flow_provider_model(
    ...
    from openzuma_cli.auth import _prompt_model_selection
    from openzuma_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from openzuma_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from openzuma_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    from openzuma_cli.models import ai_gateway_model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from openzuma_cli.nous_subscription import prompt_enable_tool_gateway
    ...
    from openzuma_cli.models import (
        _PROVIDER_MODELS,
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from openzuma_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from openzuma_cli.codex_models import get_codex_model_ids
    ...
    _codex_token = None
    # Prefer credential pool (where `openzuma auth` stores device_code tokens),
    ...
    if not _codex_token:
        try:
            from openzuma_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from openzuma_cli.models import fetch_api_models
...
def _model_flow_google_gemini_cli(_config, current_model=""):
    ...
    Flow:
      ...
      5. Save to ~/.openzuma/config.yaml.
    """
    from openzuma_cli.auth import (
    ...
    from openzuma_cli.models import _PROVIDER_MODELS
...
def _model_flow_custom(config):
    ...
    """
    from openzuma_cli.auth import _save_model_choice, deactivate_provider
    from openzuma_cli.config import get_env_value, load_config, save_config
    ...
    from openzuma_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `openzuma model` to set a model.")
...
def _remove_custom_provider(config):
    ...
    try:
        ...
        idx = menu.show()
        from openzuma_cli.curses_ui import flush_stdin
...
def _model_flow_copilot(config, current_model=""):
    ...
    from openzuma_cli.config import save_env_value, load_config, save_config
    from openzuma_cli.models import (
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from openzuma_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from openzuma_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Openzuma turns to `copilot --acp`.")
    print("  Openzuma currently starts its own ACP subprocess for each request.")
    print("  Openzuma uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set OPENZUMA_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from openzuma_cli.config import get_env_value, save_env_value, load_config, save_config
    from openzuma_cli.models import fetch_api_models
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so openzuma knows where to find it
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "ollama-cloud":
        from openzuma_cli.models import fetch_ollama_cloud_models
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from openzuma_constants import display_openzuma_home as _dhh_fn

            print(
                f"    Openzuma will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from openzuma_cli.auth import get_anthropic_key
    ...
    if selected:
        ...
        # providers if the user switches without running 'openzuma model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate Openzuma CLI with a provider."""
    from openzuma_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from openzuma_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from openzuma_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from openzuma_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from openzuma_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from openzuma_cli.webhook import webhook_command
...
def cmd_hooks(args):
    ...
    from openzuma_cli.hooks import hooks_command
    hooks_command(args)


def cmd_doctor(args):
    """Check configuration and dependencies."""
    from openzuma_cli.doctor import run_doctor
...
def cmd_dump(args):
    ...
    from openzuma_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from openzuma_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from openzuma_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up Openzuma home directory to a zip file."""
    if getattr(args, "quick", False):
        from openzuma_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from openzuma_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a Openzuma backup from a zip file."""
    from openzuma_cli.backup import run_import
...
def cmd_version(args):
    """Show version."""
    print(f"Openzuma Agent v{__version__} ({__release_date__})")
    ...
    try:
        from openzuma_cli.banner import check_for_updates
...
def cmd_uninstall(args):
    """Uninstall Openzuma Agent."""
    _require_tty("uninstall")
    from openzuma_cli.uninstall import run_uninstall
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``openzuma update --gateway`` so interactive prompts (stash restore,
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``openzuma web``).
    ...
    if r1.returncode != 0:
        print(
            ...
            + ("" if fatal else " (openzuma web will not be available)")
        )
        if fatal:
...
def _update_via_zip(args):
    """Update Openzuma Agent by downloading a ZIP archive.
    ...
    try:
        tmp_dir = tempfile.mkdtemp(prefix="openzuma-update-")
        zip_path = os.path.join(tmp_dir, f"openzuma-agent-{branch}.zip")
        ...
        # GitHub ZIPs extract to openzuma-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"openzuma-agent-{branch}")
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "openzuma-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Openzuma behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes openzuma completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Openzuma couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Openzuma couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Openzuma behaves unexpectedly.")
    return True


# =========================================================================
# Fork detection and upstream management for `openzuma update`
# =========================================================================
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_openzuma_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_openzuma_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _invalidate_update_cache():
    ...
    ``openzuma update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from openzuma_constants import get_default_openzuma_root

    default_home = get_default_openzuma_root()
...
def _load_installable_optional_extras() -> list[str]:
    ...
    # Entries look like "openzuma-agent[matrix]" or "package-name[extra]".
...
class _UpdateOutputStream:
    """Stream wrapper used during ``openzuma update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.openzuma/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``openzuma update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``openzuma update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.openzuma/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``openzuma update --gateway``) the update is already
    ...
    try:
        ...
        # openzuma_cli.config.get_openzuma_home to simulate setup failure.
        from openzuma_cli.config import get_openzuma_home as _get_openzuma_home

        logs_dir = _get_openzuma_home() / "logs"
...
def cmd_update(args):
    """Update Openzuma Agent to the latest version.

    Thin wrapper around ``_cmd_update_impl``: installs hangup protection,
    runs the update, then restores stdio on the way out (even on
    ``sys.exit`` or unhandled exceptions).
    """
    from openzuma_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update Openzuma Agent")
        return
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        # the old bytecode (e.g. get_openzuma_home added to openzuma_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # modules in this process.  Reload openzuma_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_openzuma_home() added since the last release.
        try:
            import importlib
            import openzuma_constants as _hc
        ...
        try:
            from openzuma_cli.profiles import (
                list_profiles,
        ...
        if needs_migration:
            ...
            if gateway_mode:
                ...
                print(
                    "    Run 'openzuma config migrate' later to apply any new config/env options."
                )
                response = "n"
            else:
                try:
            ...
            if response in ("", "y", "yes"):
                ...
                if gateway_mode and missing_env:
                    print("  ℹ API keys require manual entry: openzuma config migrate")
            else:
                print()
                print("Skipped. Run 'openzuma config migrate' later to configure.")
        else:
        ...
        # When running as ``openzuma update --gateway`` (spawned by the gateway's
        # /update command), this process lives inside the gateway's systemd
        # cgroup.  ``systemctl restart openzuma-gateway`` kills everything in the
        ...
        if gateway_mode:
            _exit_code_path = get_openzuma_home() / ".update_exit_code"
            try:
        ...
        try:
            from openzuma_cli.gateway import (
                is_macos,
            ...
            # Discover all openzuma-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. openzuma-gateway.service or openzuma-gateway-coder.service
            ...
            if restarted_services or killed_pids:
                ...
                if killed_pids:
                    ...
                    print("    Restart manually: openzuma gateway run")
                    # Also restart for each profile if needed
                    if len(killed_pids) > 1:
                        print(
                            "    (or: openzuma -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Openzuma gateway unit files are still installed.
        # When both openzuma.service (from a pre-rename install) and the
        # current openzuma-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `openzuma update` surfaces the issue until the user migrates.
        try:
            from openzuma_cli.gateway import (
                has_legacy_openzuma_units,
                _find_legacy_openzuma_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_openzuma_units():
                print()
                print("⚠ Legacy Openzuma gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_openzuma_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (openzuma.service) fight the current")
                print("  openzuma-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    openzuma gateway migrate-legacy")
                print()
        ...
        print("  openzuma model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``openzuma -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    if action is None:
        # Bare `openzuma profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_openzuma_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → openzuma -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.openzuma)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  openzuma profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     openzuma -p {name} chat")
                else:
        ...
        from openzuma_cli.profiles import profile_exists
        ...
        if remove:
            ...
            if wrapper_path:
                ...
                if custom_name:
                    wrapper_path.write_text(f'#!/bin/sh\nexec openzuma -p {name} "$@"\n')
                print(f"✓ Alias created: {wrapper_path}")
                if not _is_wrapper_dir_in_path():
                    print(f"⚠ {_get_wrapper_dir()} is not in your PATH.")

    elif action == "rename":
        from openzuma_cli.profiles import rename_profile
        ...
        from openzuma_cli.profiles import export_profile
        ...
        from openzuma_cli.profiles import import_profile
...
def cmd_dashboard(args):
    ...
    if "OPENZUMA_WEB_DIST" not in os.environ:
        if not _build_web_ui(PROJECT_ROOT / "web", fatal=True):
            sys.exit(1)

    from openzuma_cli.web_server import start_server
...
def cmd_completion(args, parser=None):
    ...
    from openzuma_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_logs(args):
    """View and filter Openzuma log files."""
    from openzuma_cli.logs import tail_log, list_logs
...
def main():
    """Main entry point for openzuma CLI."""
    parser = argparse.ArgumentParser(
        prog="openzuma",
        description="Openzuma Agent - AI assistant with tool-calling capabilities",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    openzuma                        Start interactive chat
    openzuma chat -q "Hello"        Single query mode
    openzuma -c                     Resume the most recent session
    openzuma -c "my project"        Resume a session by name (latest in lineage)
    openzuma --resume <session_id>  Resume a specific session by ID
    openzuma setup                  Run setup wizard
    openzuma logout                 Clear stored authentication
    openzuma auth add <provider>    Add a pooled credential
    openzuma auth list              List pooled credentials
    openzuma auth remove <p> <t>    Remove pooled credential by index, id, or label
    openzuma auth reset <provider>  Clear exhaustion status for a provider
    openzuma model                  Select default model
    openzuma config                 View configuration
    openzuma config edit            Edit config in $EDITOR
    openzuma config set model gpt-4 Set a config value
    openzuma gateway                Run messaging gateway
    openzuma -s openzuma-agent-dev,github-auth
    openzuma -w                     Start in isolated git worktree
    openzuma gateway install        Install gateway background service
    openzuma sessions list          List past sessions
    openzuma sessions browse        Interactive session picker
    openzuma sessions rename ID T   Rename/title a session
    openzuma logs                   View agent.log (last 50 lines)
    openzuma logs -f                Follow agent.log in real time
    openzuma logs errors            View errors.log
    openzuma logs --since 1h        Lines from the last hour
    openzuma debug share             Upload debug report for support
    openzuma update                 Update to latest version

For more help on a command:
    openzuma <command> --help
""",
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "without a TTY prompt.  Equivalent to OPENZUMA_ACCEPT_HOOKS=1 or "
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Ignore ~/.openzuma/config.yaml and fall back to built-in defaults (credentials in .env are still loaded)",
    )
    parser.add_argument(
        "--ignore-rules",
        action="store_true",
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with Openzuma Agent",
    )
    ...
    chat_parser.add_argument(
        ...
        help=(
            "Auto-approve any unseen shell hooks declared in config.yaml "
            "without a TTY prompt (see also OPENZUMA_ACCEPT_HOOKS env var and "
    ...
    chat_parser.add_argument(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Ignore ~/.openzuma/config.yaml and fall back to built-in defaults (credentials in .env are still loaded). Useful for isolated CI runs, reproduction, and third-party integrations.",
    )
    chat_parser.add_argument(
        "--ignore-rules",
        action="store_true",
    ...
    model_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use for Nous login (default: openzuma-cli)",
    )
    model_parser.add_argument(
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy openzuma.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy Openzuma gateway unit files "
            "(e.g. openzuma.service) left over from older installs. Profile "
            "units (openzuma-gateway-<profile>.service) and unrelated "
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Openzuma Agent with an interactive wizard. "
        "Run a specific section: openzuma setup model|tts|terminal|gateway|tools|agent",
    )
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for Openzuma CLI",
    )
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: openzuma-cli)"
    )
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Openzuma Agent components",
    )
    ...
    hooks_parser = subparsers.add_parser(
        ...
        description=(
            "Inspect shell-script hooks declared in ~/.openzuma/config.yaml, "
            "test them against synthetic payloads, and manage the first-use "
            "consent allowlist at ~/.openzuma/shell-hooks-allowlist.json."
        ),
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Openzuma Agent setup",
    )
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your Openzuma setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Openzuma Agent. Use 'openzuma debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    openzuma debug share              Upload debug report and print URL
    openzuma debug share --lines 500  Include more log lines
    openzuma debug share --expire 30  Keep paste for 30 days
    openzuma debug share --local      Print report locally (no upload)
    openzuma debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'openzuma debug share'",
    )
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up Openzuma home directory to a zip file",
        description="Create a zip archive of your entire Openzuma configuration, "
        "skills, sessions, and data (excludes the openzuma-agent codebase). "
        "Use --quick for a fast snapshot of just critical state files.",
    )
    backup_parser.add_argument(
        "-o",
        "--output",
        help="Output path for the zip file (default: ~/openzuma-backup-<timestamp>.zip)",
    )
    backup_parser.add_argument(
        "-q",
        "--quick",
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="Restore a Openzuma backup from a zip file",
        description="Extract a previously created Openzuma backup into your "
        "Openzuma home directory, restoring configuration, skills, "
        "sessions, and data",
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Openzuma Agent configuration",
    )
    ...
    def cmd_pairing(args):
        from openzuma_cli.pairing import pairing_command
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.openzuma/skills/.bundled_manifest) "
            "so future 'openzuma update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from openzuma_cli.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from openzuma_cli.skills_hub import skills_command
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/openzuma-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
        "--force",
        "-f",
    ...
    _install_enable_group.add_argument(
        "--no-enable",
        action="store_true",
        help="Install disabled (skip confirmation prompt); enable later with `openzuma plugins enable <name>`",
    )
    ...
    def cmd_plugins(args):
        from openzuma_cli.plugins_cmd import plugins_command
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            from openzuma_constants import get_openzuma_home, display_openzuma_home

            mem_dir = get_openzuma_home() / "memories"
            ...
            if not existing:
                print(
                    f"\n  Nothing to reset — no memory files found in {display_openzuma_home()}/memories/\n"
                )
                return
            ...
            print(f"  Files were in: {display_openzuma_home()}/memories/\n")
        else:
            from openzuma_cli.memory_setup import memory_command
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'openzuma tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # openzuma tools list [--platform cli]
    ...
    # openzuma tools disable <name...> [--platform cli]
    ...
    # openzuma tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in ("list", "disable", "enable"):
            from openzuma_cli.tools_config import tools_disable_enable_command

            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from openzuma_cli.tools_config import tools_command
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Openzuma as an MCP server",
        description=(
            "Manage MCP server connections and run Openzuma as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'openzuma mcp add' to connect to a new server, or\n"
            "'openzuma mcp serve' to expose Openzuma conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Openzuma as an MCP server (expose conversations to other agents)",
    )
    ...
    def cmd_mcp(args):
        from openzuma_cli.mcp_config import mcp_command
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            # Launch openzuma --resume <id> by replacing the current process
            print(f"Resuming session: {selected_id}")
            openzuma_bin = shutil.which("openzuma")
            if openzuma_bin:
                os.execvp(openzuma_bin, ["openzuma", "--resume", selected_id])
            else:
                # Fallback: re-invoke via python -m
                os.execvp(
                    sys.executable,
                    [sys.executable, "-m", "openzuma_cli.main", "--resume", selected_id],
                )
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to Openzuma",
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to Openzuma",
    ...
    def cmd_claw(args):
        from openzuma_cli.claw import claw_command
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Openzuma Agent to the latest version",
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Openzuma Agent",
        description="Remove Openzuma Agent from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Openzuma Agent as an ACP (Agent Client Protocol) server",
        description="Start Openzuma Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    _add_accept_hooks_flag(acp_parser)

    def cmd_acp(args):
        """Launch Openzuma Agent as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated Openzuma instances",
    )
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Openzuma Agent web dashboard for managing config, API keys, and sessions",
    )
    dashboard_parser.add_argument(
        "--port", type=int, default=9219, help="Port (default 9219)"
    )
    dashboard_parser.add_argument(
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter Openzuma log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    openzuma logs                    Show last 50 lines of agent.log
    openzuma logs -f                 Follow agent.log in real time
    openzuma logs errors             Show last 50 lines of errors.log
    openzuma logs gateway -n 100     Show last 100 lines of gateway.log
    openzuma logs --level WARNING    Only show WARNING and above
    openzuma logs --session abc123   Filter by session ID
    openzuma logs --component tools  Only show tool-related lines
    openzuma logs --since 1h         Lines from the last hour
    openzuma logs --since 30m -f     Follow, starting from 30 min ago
    openzuma logs list               List available log files with sizes
""",
    )
    ...
    # e.g. ``openzuma -c Pokemon Agent Dev`` → ``openzuma -c 'Pokemon Agent Dev'``
    ...
    from openzuma_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'openzuma -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (openzuma hooks list, cron
    ...
    if (
        ...
        try:
            from openzuma_cli.plugins import discover_plugins
            discover_plugins()
        except Exception:
            logger.debug(
