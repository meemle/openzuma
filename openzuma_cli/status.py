"""
Status command for openzuma CLI.

Shows the status of all Openzuma Agent components.
"""
...
from openzuma_cli.auth import AuthError, resolve_provider
from openzuma_cli.colors import Colors, color
from openzuma_cli.config import get_env_path, get_env_value, get_openzuma_home, load_config
from openzuma_cli.models import provider_label
from openzuma_cli.nous_subscription import get_nous_subscription_features
from openzuma_cli.runtime_provider import resolve_requested_provider
...
def show_status(args):
    """Show status of all Openzuma Agent components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)
    
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ Openzuma Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    try:
        from openzuma_cli.auth import get_nous_auth_status, get_codex_auth_status, get_qwen_auth_status
    ...
    print(
        ...
        f"{'logged in' if nous_logged_in else 'not logged in (run: openzuma model)'}"
    )
    if nous_logged_in:
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: openzuma model)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: openzuma model)"
    ...
    if not terminal_env:
        ...
        # (openzuma status doesn't go through cli.py's config loading)
        try:
            _cfg = load_config()
    ...
    try:
        from openzuma_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   openzuma gateway")
    ...
    jobs_file = get_openzuma_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_openzuma_home() / "sessions" / "sessions.json"
    ...
    if deep:
        ...
        try:
            ...
            sock.settimeout(1)
            result = sock.connect_ex(('127.0.0.1', 18889))
            sock.close()
            # Port in use = gateway likely running
            port_in_use = result == 0
            # This is informational, not necessarily bad
            print(f"  Port 18889:   {'in use' if port_in_use else 'available'}")
        except OSError:
            pass
    
    print()
    print(color("─" * 60, Colors.DIM))
    print(color("  Run 'openzuma doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'openzuma setup' to configure", Colors.DIM))
    print()
