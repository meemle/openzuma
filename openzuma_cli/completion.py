"""Shell completion script generation for openzuma CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_openzuma_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Openzuma Agent bash completion
# Add to ~/.bashrc:
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef openzuma
# Openzuma Agent zsh completion
# Add to ~/.zshrc:
...
_openzuma() {{
    ...
    _arguments -C \\
        ...
        '(-p --profile){{-p,--profile}}[Profile name]:profile:_openzuma_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'openzuma command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Openzuma Agent fish completion",
        "# Add to your config:",
        "#   openzuma completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __openzuma_profiles",
        "    echo default",
        "    if test -d $HOME/.openzuma/profiles",
        "        ls $HOME/.openzuma/profiles 2>/dev/null",
        "    end",
        "end",
        "",
        "# Disable file completion by default",
        "complete -c openzuma -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c openzuma -f -s p -l profile"
        " -d 'Profile name' -xa '(__openzuma_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__openzuma_profiles)' -d 'Profile name'"
                )
