...
def auth_add_command(args) -> None:
    ...
    if provider == "anthropic":
        ...
        creds = anthropic_mod.run_openzuma_oauth_login_pure()
        if not creds:
        ...
        entry = PooledCredential(
            ...
            priority=0,
            source=f"{SOURCE_MANUAL}:openzuma_pkce",
    ...
    if provider == "openai-codex":
        # Clear any existing suppression marker so a re-link after `openzuma auth
    ...
    raise SystemExit(f"`openzuma auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    # Unified removal dispatch.  Every credential source Openzuma reads from
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `openzuma auth` is called bare."""
