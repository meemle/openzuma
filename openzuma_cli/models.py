...
Add, remove, or reorder entries here — both `openzuma setup` and
`openzuma` provider-selection will pick up the change automatically.
"""
...
from openzuma_cli import __version__ as _OPENZUMA_VERSION

# Identify ourselves so endpoints fronted by Cloudflare's Browser Integrity
# Check (error 1010) don't reject the default ``Python-urllib/*`` signature.
_OPENZUMA_USER_AGENT = f"openzuma-cli/{_OPENZUMA_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `openzuma model`
    flow without maintaining a separate static list.
    """
    from openzuma_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
def check_nous_free_tier() -> bool:
    ...
    try:
        from openzuma_cli.auth import get_provider_auth_state, resolve_nous_runtime_credentials
...
# this list:  openzuma model, /model, /provider, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `openzuma model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `openzuma model` TUI
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model a user would be offered first in the ``openzuma model`` picker.

    Used as a fallback when the user has configured a provider but never
    selected a model (e.g. ``openzuma auth add openai-codex`` without
    ``openzuma model``).
    """
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    openzuma-agent is tool-calling-first — every provider path assumes the model
    ...
    *,
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — openzuma-agent
...
def fetch_ai_gateway_models(
    ...
    *,
    ...
    from openzuma_constants import AI_GATEWAY_BASE_URL
...
def fetch_ai_gateway_pricing(
    ...
    *,
    ...
    """Fetch Vercel AI Gateway /v1/models and return openzuma-shaped pricing.

    Vercel uses ``input`` / ``output`` field names; openzuma's picker expects
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``openzuma model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from openzuma_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def parse_model_input(raw: str, current_provider: str) -> tuple[str, str]:
    ...
    Supports ``provider:model`` syntax to switch providers at runtime::
        ...
        nous:openzuma-3                           →  ("nous", "openzuma-3")
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Openzuma' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``openzuma_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Openzuma should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    try:
        from openzuma_cli.auth import resolve_api_key_provider_credentials
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a Openzuma release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "nous":
        ...
        try:
            from openzuma_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
...
def copilot_default_headers() -> dict[str, str]:
    ...
    """
    try:
        from openzuma_cli.copilot_auth import copilot_request_headers
        return copilot_request_headers(is_agent_turn=True)
    except ImportError:
        return {
            "Editor-Version": COPILOT_EDITOR_VERSION,
            "User-Agent": "OpenzumaAgent/1.0",
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Openzuma' default Claude IDs elsewhere use
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _OPENZUMA_USER_AGENT}
    if api_key:
        headers["Authorization"] = f"Bearer {api_key}"
...
def _ollama_cloud_cache_path() -> Path:
    ...
    return get_openzuma_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "custom":
        ...
        message = (
            ...
            f"Openzuma will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Openzuma cannot verify the model name."
