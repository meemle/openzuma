"""
Unified tool configuration for Openzuma Agent.

`openzuma tools` and `openzuma setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.openzuma/config.yaml under
...
from openzuma_cli.nous_subscription import (
...
from openzuma_cli.cli_output import (  # noqa: E402 — late import block
...
# They're still in _OPENZUMA_CORE_TOOLS (available at runtime if enabled),
...
def _get_effective_configurable_toolsets():
    ...
    try:
        from openzuma_cli.plugins import discover_plugins, get_plugin_toolsets
...
from openzuma_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
    ...
    if post_setup_key in ("agent_browser", "browserbase"):
        ...
        if not node_modules.exists() and shutil.which("npm"):
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_openzuma_home()}/openzuma-agent && npm install")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in openzuma-agent directory)")
...
def _get_platform_tools(
    ...
    *,
    ...
    # "openzuma-cli" (which include all _OPENZUMA_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # (e.g. "openzuma-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
    ...
    # A plugin toolset is "known" for a platform once `openzuma tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by openzuma tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (openzuma-cli, openzuma-telegram, etc.)
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `openzuma tools` and `openzuma setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ Openzuma Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://openzuma-agent.nousresearch.com/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    print()
    from openzuma_constants import display_openzuma_home
    print(color(f"  Tool configuration saved to {display_openzuma_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'openzuma' or gateway restart.", Colors.DIM))
    print()
