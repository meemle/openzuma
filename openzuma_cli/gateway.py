...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and Openzuma will log in directly",
        ...
        "vars": [
            ...
            {"name": "MATRIX_USER_ID", "prompt": "User ID (@bot:server — required for password login)", "password": False,
             "help": "Full Matrix user ID, e.g. @openzuma:matrix.example.org"},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. openzuma) and copy the bot token",
        ...
        "vars": [
            ...
            {"name": "MATTERMOST_HOME_CHANNEL", "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)", "password": False,
             "help": "Channel ID where Openzuma delivers cron results and notifications."},
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your Openzuma agent",
        ...
        "vars": [
            {"name": "EMAIL_ADDRESS", "prompt": "Email address", "password": False,
             "help": "The email address Openzuma will use (e.g., openzuma@gmail.com)."},
    ...
    },
    {
        ...
        "vars": [
            ...
            {"name": "WECOM_CALLBACK_PORT", "prompt": "Callback server port (default: 8745)", "password": False,
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Openzuma connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: openzuma pairing generate bluebubbles",
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_openzuma_home() / "whatsapp" / "session" / "creds.json"
...
def _setup_whatsapp():
    ...
    from openzuma_cli.main import cmd_whatsapp
    import argparse
...
def _setup_dingtalk():
    ...
    if method == 0:
        ...
        try:
            from openzuma_cli.dingtalk_auth import dingtalk_qr_auth
...
def _setup_feishu():
    ...
    if used_qr:
        ...
        if connection_mode == "webhook":
            print_info("  Webhook defaults: 127.0.0.1:8865/feishu/webhook")
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info("  Unknown users can request access; approve with `openzuma pairing approve`.")
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info("    signal-cli link -n \"OpenzumaAgent\"")
