"""openzuma claw — OpenClaw migration commands.

Usage:
    openzuma claw migrate              # Preview then migrate (always shows preview first)
    openzuma claw migrate --dry-run    # Preview only, no changes
    openzuma claw migrate --yes        # Skip confirmation prompt
    openzuma claw migrate --preset full --overwrite  # Full migration, overwrite conflicts
    openzuma claw cleanup              # Archive leftover OpenClaw directories
    openzuma claw cleanup --dry-run    # Preview what would be archived
"""
...
from openzuma_cli.config import get_openzuma_home, get_config_path, load_config, save_config
...
def _warn_if_openclaw_running(auto_yes: bool) -> None:
    ...
    print_info(
        ...
        "Openzuma may try to use the same token, causing disconnects."
    )
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    """Check if a Openzuma gateway is running with connected platforms.
    ...
    print()
    print_error(
        "Openzuma gateway is running with active connections: "
        + ", ".join(connected)
    )
    print_info(
    ...
    print_info("Recommendation: stop the gateway first with 'openzuma stop'.")
    print()
...
def _find_migration_script() -> Path | None:
    """Find the openclaw_to_openzuma.py script in known locations."""
...
def _load_migration_module(script_path: Path):
    ...
    spec = importlib.util.spec_from_file_location("openclaw_to_openzuma", script_path)
...
def claw_command(args):
    """Route openzuma claw subcommands."""
    ...
    if action == "migrate":
        ...
        print("Usage: openzuma claw <command> [options]")
        print()
        print("Commands:")
        print("  migrate          Migrate settings from OpenClaw to Openzuma")
        print("  cleanup          Archive leftover OpenClaw directories after migration")
        print()
        print("Run 'openzuma claw <command> --help' for options.")


def _cmd_migrate(args):
    """Run the OpenClaw → Openzuma migration."""
    ...
    if not source_dir.is_dir():
        ...
        print_info("You can specify a custom path: openzuma claw migrate --source /path/to/.openclaw")
        return
    ...
    print_info(f"Target:      {openzuma_home}")
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info("To execute, re-run with: openzuma claw migrate --yes")
            return
    ...
    # 'openzuma claw cleanup' separately.
...
def _cmd_cleanup(args):
    ...
    if running:
        ...
        if not auto_yes:
            ...
            if not prompt_yes_no("Proceed anyway?", default=False):
                print_info("Aborted. Stop OpenClaw first, then re-run: openzuma claw cleanup")
                return
    ...
    for source_dir in dirs_to_check:
        ...
        if dry_run:
            ...
            print_info("To execute, re-run with: openzuma claw cleanup --yes")
        else:
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  openzuma claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        ...
        if skipped_keys:
            ...
            print_info("  openzuma claw migrate --migrate-secrets")
            print()
            print_info("Or add your key manually:")
            print_info("  openzuma config set OPENROUTER_API_KEY sk-or-v1-...")
