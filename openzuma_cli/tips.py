...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or openzuma -c.",
    ...
    "Ctrl+Z suspends Openzuma to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "openzuma -c resumes your most recent CLI session. openzuma -c \"project name\" resumes by title.",
    "openzuma -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "openzuma -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "openzuma chat -t web,terminal enables only specific toolsets for a focused session.",
    "openzuma chat -s github-pr-workflow preloads a skill at launch.",
    "openzuma chat -q \"query\" runs a single non-interactive query and exits.",
    "openzuma chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "openzuma chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "openzuma --yolo bypasses all dangerous command approval prompts for the entire session.",
    "openzuma chat --source telegram tags the session for filtering in openzuma sessions list.",
    "openzuma -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "openzuma doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "openzuma dump outputs a compact setup summary — great for bug reports.",
    "openzuma config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "openzuma config edit opens config.yaml in your default editor.",
    "openzuma config check scans for missing or stale configuration options.",
    "openzuma sessions browse opens an interactive session picker with search.",
    "openzuma sessions stats shows session counts by platform and database size.",
    "openzuma sessions prune --older-than 30 cleans up old sessions.",
    "openzuma skills search react --source skills-sh searches the skills.sh public directory.",
    "openzuma skills check scans installed hub skills for upstream updates.",
    "openzuma skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "openzuma skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "openzuma mcp add github --command npx adds MCP servers from the command line.",
    "openzuma mcp serve runs Openzuma itself as an MCP server for other agents.",
    "openzuma auth add lets you add multiple API keys for credential pool rotation.",
    "openzuma completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "openzuma logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "openzuma backup creates a zip backup of your entire Openzuma home directory.",
    "openzuma profile create coder creates an isolated profile that becomes its own command.",
    "openzuma profile create work --clone copies your current config and keys to a new profile.",
    "openzuma update syncs new bundled skills to ALL profiles automatically.",
    "openzuma gateway install sets up Openzuma as a system service (systemd/launchd).",
    "openzuma memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "openzuma webhook subscribe creates event-driven webhook routes with HMAC validation.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as openzuma -w).",
    ...
    "OPENZUMA_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Openzuma tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'openzuma profile create coder' creates the 'coder' command.",
    "openzuma profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Openzuma prints a resume command with session ID and stats.",
    "openzuma sessions export backup.jsonl exports all sessions for backup or analysis.",
    "openzuma -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "openzuma skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .openzuma/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: openzuma cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Openzuma runs on 18 platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, email, and more.",
    "openzuma gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Openzuma via a local macOS server.",
    ...
    "Context files (.openzuma.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "SOUL.md at ~/.openzuma/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    "Openzuma loads project context from .openzuma.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "MCP servers auto-generate toolsets at runtime — openzuma tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.openzuma/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "openzuma chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "openzuma plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "openzuma acp runs Openzuma as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "OPENZUMA_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "openzuma login supports OAuth-based auth for Nous and OpenAI Codex providers.",
    ...
    "openzuma status --deep runs deeper diagnostic checks across all components.",

    # --- Hidden Gems & Power-User Tricks ---
    "BOOT.md at ~/.openzuma/BOOT.md runs automatically on every gateway start — use it for startup checks.",
    "Cron jobs can attach a Python script (--script) whose stdout is injected into the prompt as context.",
    "Cron scripts live in ~/.openzuma/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Openzuma your own.",
    "SOUL.md is auto-seeded with a default personality on first run. Edit ~/.openzuma/SOUL.md to customize.",
    ...
    "Before a gateway session reset, Openzuma auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.openzuma/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at OPENZUMA_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "OPENZUMA_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in OPENZUMA_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.openzuma/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'openzuma profile create ls' would be rejected.",
    "openzuma profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "openzuma claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "OPENZUMA_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via openzuma tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.openzuma/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.openzuma/skills/.hub/quarantine/ holds skills pending security review.",
]
