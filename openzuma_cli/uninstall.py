"""
Openzuma Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.openzuma/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Openzuma PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing openzuma-agent or openzuma PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Openzuma Agent" comment and following line
                if '# Openzuma Agent' in line or '# openzuma-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('openzuma' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing openzuma
                if 'openzuma' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the openzuma wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "openzuma",
        Path("/usr/local/bin/openzuma"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains openzuma_cli reference)
                content = wrapper.read_text()
                if 'openzuma_cli' in content or 'openzuma-agent' in content:
                    wrapper.unlink()
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``openzuma gateway run`` processes
    ...
    try:
        from openzuma_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from openzuma_cli.gateway import get_launchd_plist_path
...
def _is_default_openzuma_home(openzuma_home: Path) -> bool:
    """Return True when ``openzuma_home`` points at the default (non-profile) root."""
    try:
        from openzuma_constants import get_default_openzuma_root
        return openzuma_home.resolve() == get_default_openzuma_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its OPENZUMA_HOME directory.

    We shell out to ``openzuma -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current OPENZUMA_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m openzuma_cli.main` so we don't depend on a `openzuma`
    #    wrapper that may be half-removed mid-uninstall.
    openzuma_invocation = [_sys.executable, "-m", "openzuma_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                openzuma_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's OPENZUMA_HOME directory.
    try:
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.openzuma/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.openzuma/ for future reinstall
    """
    project_root = get_project_root()
    openzuma_home = get_openzuma_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie OPENZUMA_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_openzuma_home(openzuma_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Openzuma Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {openzuma_home / 'config.yaml'}")
    print(f"  Secrets: {openzuma_home / '.env'}")
    print(f"  Data:    {openzuma_home / 'cron/'}, {openzuma_home / 'sessions/'}, {openzuma_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their OPENZUMA_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Openzuma data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Openzuma code but keep your configuration and data.")
    ...
    log_info("Removing openzuma command...")
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.openzuma/, just remove the openzuma-agent subdir
            if openzuma_home in project_root.parents or project_root.parent == openzuma_home:
    ...
    # 5. Optionally remove ~/.openzuma/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile OPENZUMA_HOME dirs live under
        ...
        try:
            if openzuma_home.exists():
                shutil.rmtree(openzuma_home)
                log_success(f"Removed {openzuma_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {openzuma_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {openzuma_home}")
    ...
    if not full_uninstall:
        ...
        print(f"  {openzuma_home}/")
        print()
        print("To reinstall later with your existing settings:")
        print(color("  curl -fsSL https://raw.githubusercontent.com/NousResearch/openzuma-agent/main/scripts/install.sh | bash", Colors.DIM))
        print()
    
    print(color("Reload your shell to complete the process:", Colors.YELLOW))
    print("  source ~/.bashrc  # or ~/.zshrc")
    print()
    print("Thank you for using Openzuma Agent! ⚕")
    print()
