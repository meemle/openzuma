...
- ``openzuma_cli.providers``        -- canonical provider identity + overlays
- ``openzuma_cli.model_normalize``  -- per-provider name formatting
...
from openzuma_cli.providers import (
...
_OPENZUMA_MODEL_WARNING = (
    "Nous Research Openzuma 3 & 4 models are NOT agentic and are not designed "
    "for use with Openzuma Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Nous Research Openzuma 3 / Openzuma 4 chat families.
# The previous substring check (`"openzuma" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``openzuma-brain:qwen3-14b-ctx16k`` that just
# happen to carry "openzuma" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   NousResearch/Openzuma-3-Llama-3.1-70B, openzuma-4-405b, openrouter/openzuma3:70b
# Negative examples it must NOT match:
#   openzuma-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NOUS_OPENZUMA_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])openzuma[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)


def is_nous_openzuma_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous Openzuma 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_NOUS_OPENZUMA_NON_AGENTIC_RE.search(model_name))


def _check_openzuma_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous Openzuma 3/4 chat model."""
    if is_nous_openzuma_non_agentic(model_name):
        return _OPENZUMA_MODEL_WARNING
    return ""
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'openzuma model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'openzuma doctor' — config issues detected:"
    ...
    # Mirrors the same logic in openzuma_cli.runtime_provider.resolve_runtime_provider;
    ...
    openzuma_warn = _check_openzuma_model_warning(new_model)
    if openzuma_warn:
        warnings.append(openzuma_warn)
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from openzuma_cli/models.py (OPENROUTER_MODELS,
    ...
    # Build curated model lists keyed by openzuma provider ID
    ...
    # --- 1. Check Openzuma-mapped providers ---
    for openzuma_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        pconfig = PROVIDER_REGISTRY.get(openzuma_id)
        ...
        # show up in the picker without requiring a Openzuma release.
        model_ids = curated.get(openzuma_id, [])
        if openzuma_id in _MODELS_DEV_PREFERRED:
            model_ids = _merge_with_models_dev(openzuma_id, model_ids)
        total = len(model_ids)
    ...
    # --- 2. Check Openzuma-only providers (nous, openai-codex, copilot, opencode-go) ---
    from openzuma_cli.providers import OPENZUMA_OVERLAYS
    from openzuma_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Openzuma provider ID.
    # OPENZUMA_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Openzuma IDs ("copilot").
    _mdev_to_openzuma = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in OPENZUMA_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Openzuma slug — e.g. "github-copilot" → "copilot"
        openzuma_slug = _mdev_to_openzuma.get(pid, pid)
        if openzuma_slug.lower() in seen_slugs:
            continue
        ...
        if not has_creds:
            try:
                from openzuma_cli.auth import _load_auth_store
                store = _load_auth_store()
                providers_store = store.get("providers", {})
                pool_store = store.get("credential_pool", {})
                if store and (
                    pid in providers_store or openzuma_slug in providers_store
                    or pid in pool_store or openzuma_slug in pool_store
                ):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(openzuma_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", openzuma_slug, exc)
        ...
        # configured.
        if not has_creds and openzuma_slug == "anthropic":
            try:
                ...
                if (openzuma_creds and openzuma_creds.get("accessToken")) or \
        ...
        # Use curated list — look up by Openzuma slug, fall back to overlay key
        model_ids = curated.get(openzuma_slug, []) or curated.get(pid, [])
        # Merge with models.dev for preferred providers (same rationale as above).
        if openzuma_slug in _MODELS_DEV_PREFERRED:
            model_ids = _merge_with_models_dev(openzuma_slug, model_ids)
        total = len(model_ids)
        top = model_ids[:max_models]

        results.append({
            "slug": openzuma_slug,
            "name": get_label(openzuma_slug),
            "is_current": openzuma_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "openzuma",
        ...
        seen_slugs.add(openzuma_slug.lower())

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or OPENZUMA_OVERLAYS (keeps /model in sync
    # with `openzuma model`).
    try:
        from openzuma_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Openzuma's canonical write key (matches
            ...
            # Openzuma writes ``models:`` as a dict keyed by model id
            # (see openzuma_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            # active model. Openzuma's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, openzuma_cli/config.py) already consume that dict.
