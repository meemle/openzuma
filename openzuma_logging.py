"""Centralized logging setup for Openzuma Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.openzuma/logs/`` (profile-aware via ``get_openzuma_home()``).
...
from openzuma_constants import get_config_path, get_openzuma_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_openzuma_session_injector", False):
    ...
    _session_record_factory._openzuma_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``openzuma logs --component``.
COMPONENT_PREFIXES = {
    "gateway": ("gateway",),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("openzuma_cli", "cli"),
    "cron": ("cron",),
}
...
def setup_logging(
    *,
    ...
    """Configure the Openzuma logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    openzuma_home
        Override for the Openzuma home directory.  Falls back to
        ``get_openzuma_home()`` (profile-aware).
    log_level
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_openzuma_verbose", False):
                return

    handler = logging.StreamHandler()
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._openzuma_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    inherit the openzuma group. However, both _open() (initial creation) and
    ...
    def __init__(self, *args, **kwargs):
        from openzuma_cli.config import is_managed
