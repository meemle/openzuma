"""
Openzuma MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Openzuma-specific extra)

Usage:
    openzuma mcp serve
    openzuma mcp serve --verbose
...
logger = logging.getLogger("openzuma.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using OPENZUMA_HOME."""
    try:
        from openzuma_constants import get_openzuma_home
        return get_openzuma_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("OPENZUMA_HOME", Path.home() / ".openzuma")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from openzuma_constants import get_openzuma_home
        directory_file = get_openzuma_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("OPENZUMA_HOME", Path.home() / ".openzuma")
...
class EventBridge:
    ...
    This is the Openzuma equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from openzuma_constants import get_openzuma_home
            db_file = get_openzuma_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("OPENZUMA_HOME", Path.home() / ".openzuma")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Openzuma MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "openzuma",
        instructions=(
            "Openzuma Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Openzuma MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
