# Contributing to Openzuma Agent

Thank you for contributing to Openzuma Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — Windows, macOS, different Linux distros, different terminal emulators. We want Openzuma to work everywhere.
...
Bundled skills (in `skills/`) ship with every Openzuma install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `openzuma skills browse` (labeled "official") and install it with `openzuma skills install` (no third-party warning, builtin trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Nous Research Discord](https://discord.gg/NousResearch). Users can install it with `openzuma skills install`.
...
```bash
mkdir -p ~/.openzuma/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.openzuma/config.yaml
touch ~/.openzuma/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.openzuma/.env
```
...
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/openzuma" ~/.local/bin/openzuma

# Verify
openzuma doctor
openzuma chat -q "Hello"
```
...
├── cli.py                    # OpenzumaCLI class — interactive TUI, prompt_toolkit integration
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (openzuma-cli, openzuma-telegram, etc.)
├── openzuma_state.py           # SQLite session database with FTS5 full-text search, session titles
...
│
├── openzuma_cli/               # CLI command implementations
...
│
├── skills/                   # Bundled skills (copied to ~/.openzuma/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── environments/             # RL training environments (Atropos integration)
├── tests/                    # Test suite
├── website/                  # Documentation site (openzuma-agent.nousresearch.com)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.openzuma/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.openzuma/`)

| Path | Purpose |
|------|---------|
| `~/.openzuma/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.openzuma/.env` | API keys and secrets |
| `~/.openzuma/auth.json` | OAuth credentials (Nous Portal) |
| `~/.openzuma/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.openzuma/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.openzuma/state.db` | SQLite session database |
| `~/.openzuma/sessions/` | JSON session logs |
| `~/.openzuma/cron/` | Scheduled job data |
| `~/.openzuma/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`openzuma_state.py`) with full-text search and unique session titles. JSON logs go to `~/.openzuma/sessions/`.
...
Four fields are supported under `metadata.openzuma`:
...
The user may skip setup and keep loading the skill. Openzuma only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `openzuma setup` or update `~/.openzuma/.env` locally.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Openzuma tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `openzuma --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Openzuma uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.openzuma/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `openzuma_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `openzuma_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

Openzuma runs on Linux, macOS, and Windows. When writing code that touches the OS:
...
Openzuma has terminal access. Security matters.
...
2. **Test manually**: Run `openzuma` and exercise the code path you changed
