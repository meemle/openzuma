# Openzuma Agent Has Had "Routines" Since March
...
Openzuma equivalent — works today:
```bash
...
Openzuma equivalent — works today:
```bash
openzuma webhook subscribe auth-watch \
  --events "pull_request" \
...
Every use case in their blog post — backlog triage, docs drift, deploy verification, alert correlation, library porting, bespoke PR review — has a working Openzuma implementation. No new features needed. It's been shipping since March 2026.

---

## What's Different

| | Claude Code Routines | Openzuma Agent |
|---|---|---|
...
## Things Openzuma Does That Routines Can't
...
Openzuma has no daily limit. Run 500 automations a day if you want. The only constraint is your API budget, and you choose which models to use for which tasks.
...
Openzuma Agent is open source and free. The automation infrastructure — cron scheduler, webhook platform, skill system, multi-platform delivery — is built in.

```bash
pip install openzuma-agent
openzuma setup
```
...
```bash
openzuma gateway setup    # enable webhooks
openzuma webhook subscribe pr-review \
  --events "pull_request" \
...
Full automation templates gallery: [openzuma-agent.nousresearch.com/docs/guides/automation-templates](https://openzuma-agent.nousresearch.com/docs/guides/automation-templates)
...
*Openzuma Agent is built by [Nous Research](https://nousresearch.com). Open source, model-agnostic, runs on your infrastructure.*
