"""CLI entry point for the openzuma-agent ACP adapter.

Loads environment variables from ``~/.openzuma/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from OPENZUMA_HOME (default ``~/.openzuma``)."""
    from openzuma_cli.env_loader import load_openzuma_dotenv

    openzuma_home = get_openzuma_home()
    loaded = load_openzuma_dotenv(openzuma_home=openzuma_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", openzuma_home / ".env"
        )
...
def main() -> None:
    ...
    logger.info("Starting openzuma-agent ACP adapter")
    ...
    import acp
    from .server import OpenzumaACPAgent

    agent = OpenzumaACPAgent()
    try:
