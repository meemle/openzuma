"""Persistent slash-command worker — one OpenzumaCLI per TUI session.
...
def _run(cli: OpenzumaCLI, command: str) -> str:
...
def main():
    ...
    os.environ["OPENZUMA_SESSION_KEY"] = args.session_key
    os.environ["OPENZUMA_INTERACTIVE"] = "1"

    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = OpenzumaCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
