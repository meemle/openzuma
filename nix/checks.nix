...
{ inputs, ... }: {
  perSystem = { pkgs, system, lib, ... }:
    let
      ...
      configKeys = pkgs.runCommand "openzuma-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${openzumaVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "openzuma-cross-eval" { } (
          if failures != [] then
        ...
        package-contents = pkgs.runCommand "openzuma-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${openzuma-agent}/bin/openzuma || (echo "FAIL: openzuma binary missing"; exit 1)
          test -x ${openzuma-agent}/bin/openzuma-agent || (echo "FAIL: openzuma-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${openzuma-agent}/bin/openzuma version 2>&1 | grep -qi "openzuma" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "openzuma-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in openzuma openzuma-agent openzuma-acp; do
            test -x ${openzuma-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "openzuma-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking openzuma --help ==="
          ${openzuma-agent}/bin/openzuma --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${openzuma-agent}/bin/openzuma --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "openzuma-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${openzuma-agent}/share/openzuma-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${openzuma-agent}/share/openzuma-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "OPENZUMA_BUNDLED_SKILLS" ${openzuma-agent}/bin/openzuma || \
            (echo "FAIL: OPENZUMA_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: OPENZUMA_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-tui = pkgs.runCommand "openzuma-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${openzuma-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${openzuma-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          test -d ${openzuma-agent}/ui-tui/node_modules || (echo "FAIL: node_modules missing"; exit 1)
          echo "PASS: node_modules present"

          grep -q "OPENZUMA_TUI_DIR" ${openzuma-agent}/bin/openzuma || \
            (echo "FAIL: OPENZUMA_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: OPENZUMA_TUI_DIR set in wrapper"
        ...
        # Verify OPENZUMA_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        openzuma-node = pkgs.runCommand "openzuma-node-version" { } ''
          set -e
          echo "=== Checking OPENZUMA_NODE in wrapper ==="
          grep -q "OPENZUMA_NODE" ${openzuma-agent}/bin/openzuma || \
            (echo "FAIL: OPENZUMA_NODE not set in wrapper"; exit 1)
          echo "PASS: OPENZUMA_NODE present in wrapper"

          OPENZUMA_NODE=$(sed -n "s/^export OPENZUMA_NODE='\(.*\)'/\1/p" ${openzuma-agent}/bin/openzuma)
          test -x "$OPENZUMA_NODE" || (echo "FAIL: OPENZUMA_NODE=$OPENZUMA_NODE not executable"; exit 1)
          echo "PASS: OPENZUMA_NODE executable at $OPENZUMA_NODE"

          NODE_MAJOR=$("$OPENZUMA_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All OPENZUMA_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify OPENZUMA_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "openzuma-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(OPENZUMA_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking OPENZUMA_MANAGED guards ==="
          check_blocked "config set" ${openzuma-agent}/bin/openzuma config set model foo
          check_blocked "config edit" ${openzuma-agent}/bin/openzuma config edit
        ...
        in pkgs.runCommand "openzuma-config-roundtrip" {
          ...
          merge_and_load() {
            local openzuma_home="$1"
            export OPENZUMA_HOME="$openzuma_home"
            ${configMergeScript} ${nixSettings} "$openzuma_home/config.yaml"
            ${openzumaVenv}/bin/python3 -c '
import json, sys
