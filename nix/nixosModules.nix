# nix/nixosModules.nix — NixOS module for openzuma-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: openzuma runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $OPENZUMA_HOME/.env
# and read by openzuma at startup — no container recreation needed for env changes.
#
# Tool resolution: the openzuma wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.openzuma-agent;
    openzuma-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-openzuma-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "openzuma-config-attrs";
      description = "Openzuma YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "openzuma-config.yaml" configJson;
    ...
    documentDerivation = pkgs.runCommand "openzuma-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'OPENZUMA_DOC_EOF'\n${value}\nOPENZUMA_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "openzuma-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/openzuma";
    ...
    # openzuma user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "openzuma-container-entrypoint" ''
      set -eu

      OPENZUMA_UID="''${OPENZUMA_UID:?OPENZUMA_UID must be set}"
      OPENZUMA_GID="''${OPENZUMA_GID:?OPENZUMA_GID must be set}"

      # ── Group: ensure a group with GID=$OPENZUMA_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$OPENZUMA_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="openzuma"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$OPENZUMA_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$OPENZUMA_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$OPENZUMA_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$OPENZUMA_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="openzuma"
        TARGET_HOME="/home/openzuma"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$OPENZUMA_UID" -g "$OPENZUMA_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$OPENZUMA_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$OPENZUMA_UID:$OPENZUMA_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure OPENZUMA_HOME is owned by the target user
      if [ -n "''${OPENZUMA_HOME:-}" ] && [ -d "$OPENZUMA_HOME" ]; then
        chown -R "$OPENZUMA_UID:$OPENZUMA_GID" "$OPENZUMA_HOME"
      fi
      ...
      if [ ! -f /var/lib/openzuma-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/openzuma-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/openzuma ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/openzuma
        chmod 0440 /etc/sudoers.d/openzuma
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$OPENZUMA_UID" --regid="$OPENZUMA_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $OPENZUMA_HOME/.env.
    ...
    # Default: /var/lib/openzuma/workspace → /data/workspace.
  ...
  in {
    options.services.openzuma-agent = with lib; {
      enable = mkEnableOption "Openzuma Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = openzuma-agent;
        description = "The openzuma-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/openzuma";
        description = "State directory. Contains .openzuma/ subdir (OPENZUMA_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Openzuma config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $OPENZUMA_HOME/.env at activation time.
          Openzuma reads this file on every startup via load_openzuma_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $OPENZUMA_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $OPENZUMA_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `openzuma gateway`.";
      };
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the openzuma CLI to environment.systemPackages and export
          OPENZUMA_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.openzuma symlink to the service
            stateDir. These users are automatically added to the openzuma group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.openzuma-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the openzuma CLI to system PATH and export OPENZUMA_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.openzuma/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ cfg.package ];
        environment.variables.OPENZUMA_HOME = "${cfg.stateDir}/.openzuma";
      })
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.openzuma-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed openzuma
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure openzuma is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.openzuma        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.openzuma/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.openzuma/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.openzuma/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.openzuma/memories 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."openzuma-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.openzuma
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.openzuma ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.openzuma ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.openzuma -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories; do
            mkdir -p "${cfg.stateDir}/.openzuma/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.openzuma/$_subdir"
            chmod 2770 "${cfg.stateDir}/.openzuma/$_subdir"
            find "${cfg.stateDir}/.openzuma/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m 0640 -D ${configFile} ${cfg.stateDir}/.openzuma/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.openzuma/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.openzuma/config.yaml
            chmod 0640 ${cfg.stateDir}/.openzuma/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.openzuma/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.openzuma/.managed
          chmod 0644 ${cfg.stateDir}/.openzuma/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.openzuma/.container-mode <<'OPENZUMA_CONTAINER_MODE_EOF'
# Written by NixOS activation script. Do not edit manually.
backend=${cfg.container.backend}
container_name=${containerName}
exec_user=${cfg.user}
openzuma_bin=${containerDataDir}/current-package/bin/openzuma
OPENZUMA_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.openzuma/.container-mode
            chmod 0644 ${cfg.stateDir}/.openzuma/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.openzuma/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.openzuma";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.openzuma" ]; then
                  rm -f "${symlinkPath}"
                  echo "openzuma-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.openzuma -> stateDir/.openzuma for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.openzuma";
                target = "${cfg.stateDir}/.openzuma";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "openzuma-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.openzuma/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.openzuma/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.openzuma/auth.json
              fi
            ''}
          ''}
          ...
          # Openzuma reads $OPENZUMA_HOME/.env at startup via load_openzuma_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.openzuma/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'OPENZUMA_NIX_ENV_EOF'
${envFileContent}
OPENZUMA_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
                echo "" >> "$ENV_FILE"
                cat "${f}" >> "$ENV_FILE"
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.openzuma-agent = {
          description = "Openzuma Agent Gateway";
          ...
          environment = {
            HOME = cfg.stateDir;
            OPENZUMA_HOME = "${cfg.stateDir}/.openzuma";
            OPENZUMA_MANAGED = "true";
          ...
          serviceConfig = {
            ...
            # $OPENZUMA_HOME/.env by the activation script. load_openzuma_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${cfg.package}/bin/openzuma"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the openzuma group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.openzuma-agent = {
          description = "Openzuma Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              OPENZUMA_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              OPENZUMA_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env OPENZUMA_UID="$OPENZUMA_UID" \
                --env OPENZUMA_GID="$OPENZUMA_GID" \
                --env OPENZUMA_HOME=${containerDataDir}/.openzuma \
                --env OPENZUMA_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/openzuma gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
