...
Checklist for integrating a new messaging platform into the Openzuma gateway.
...
And add it to the `openzuma-gateway` composite:

```python
"openzuma-gateway": {
    "includes": [..., "openzuma-your-platform"]
}
```
...
## 12. Status Display (`openzuma_cli/status.py`)
...
## 13. Gateway Setup Wizard (`openzuma_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ openzuma_cli/ toolsets.py \
  --include="*.py" -l | sort -u
